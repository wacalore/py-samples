...
//
// For non-refunding months (all except Feb/May/Aug/Nov):
//   - 2Y, 3Y, 5Y, 7Y originals settle and enter the index
//   - 10Y REOPEN also settles but same CUSIP (include for extension calc)
//
// For refunding months (Feb, May, Aug, Nov):
//   - 10Y ORIGINAL (new CUSIP) settles
//   - 20Y and 30Y REOPENS settle
//
auctionSchedule:([]
    ...
    typicalOfferSize:69e9 58e9 70e9 44e9 42e9 13e9 22e9;  // Typical OFFER amount (public market)
    typicalReopenSize:0N 0N 0N 0N 35e9 13e9 22e9          // Typical REOPEN size (10Y/20Y/30Y only)
  )
...
filterEligibleForIndex:{[bonds;asOfDate;indexName]
    ...
    // Start with bonds outstanding as of date (use issueDate, not auctionDate)
...
buildIndexComposition:{[bonds;asOfDate;indexName;prices;outstandings]
    ...
    // Use outstanding for index weighting (total amount issued)
    // This gives correct extension calculations (~0.06 for non-refunding, ~0.05 for refunding)
...
// Predict new issuance for a target month (for entering count)
// Returns table of expected auctions:
//   - Refunding months (Feb, May, Aug, Nov): 10Y, 20Y, 30Y (3 bonds)
//   - Non-refunding months: 2Y, 3Y, 5Y, 7Y (4 bonds)
predictNewIssuance:{[bonds;targetMonth;asOfDate]
    isRefunding:.tsyindex.isRefundingMonth targetMonth;
    // Refunding: 10Y/20Y/30Y, Non-refunding: 2Y/3Y/5Y/7Y (no 10Y reopening)
    terms:$[isRefunding; `10Y`20Y`30Y; `2Y`3Y`5Y`7Y];
    .tsyindex.predictNewIssuanceTerms[bonds;targetMonth;asOfDate;terms]}

// For extension calculation: includes 10Y reopening in non-refunding months
predictNewIssuanceForExtension:{[bonds;targetMonth;asOfDate]
    isRefunding:.tsyindex.isRefundingMonth targetMonth;
    // Refunding: only 10Y/20Y/30Y
    // Non-refunding: all issuance including 10Y reopening
    terms:$[isRefunding; `10Y`20Y`30Y; `2Y`3Y`5Y`7Y`10Y];
    .tsyindex.predictNewIssuanceTerms[bonds;targetMonth;asOfDate;terms]}
...
forecastMonth:{[bonds;currentComp;targetMonth;indexName;asOfDate]
    ...
    // 3. Add new issuance for extension calculation
    // Refunding months (Feb/May/Aug/Nov): only 10Y, 20Y, 30Y
    // Non-refunding months: all issuance including 10Y reopening
    newIssues:.tsyindex.predictNewIssuanceForExtension[bonds;targetMonth;asOfDate];
    ...
    if[0 < count newIssues;
        ...
        newRows:([]
            ...
            parAmount:newIssues`estimatedSize;
            marketValue:newIssues`estimatedSize;
...
extensionFromPrices:{[bonds;priceData;indexName]
    ...
    results:{[bonds;priceData;indexName;asOfDate]
        ...
        newInProj:select from projComp where cusip like "NEW_*";
...
extensionDetail:{[bonds;prices;asOfDate;indexName]
    ...
    // Calculate rebalancing effect and aging effect
    rebalEffect:decomp[`extensionFromNew] + decomp`extensionFromRolloff;
    agingEffect:decomp`extensionFromAging;
    ...
    // Summary
    summary:`date`forecastMonth`bondCount`totalMV`dv01Net`avgCoupon`avgPrice`currentDuration`projectedDuration`rebalancingEffect`agingEffect`totalDurationChange`extensionFromNew`extensionFromRolloff`bondsEntering`bondsLeaving`rebalByBucket!(
        asOfDate;
        nextMonth;
        stats`bondCount;
        stats`totalMV;
    ...
    `summary`composition`rolloffs`newIssues`buckets!(summary;comp;rolloffs;newIssues;buckets)}
