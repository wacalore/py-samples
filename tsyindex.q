...
filterEligibleForIndex:{[bonds;asOfDate;indexName]
    ...
    // Filter by minimum public float (eligibility uses publicFloat, not outstanding)
    // publicFloat = outstanding - SOMA holdings
    eligible:select from eligible where publicFloat >= rules`minOutstanding;
...
// Bonds exit in the month exactly 12 months before their maturity month,
// but only for bonds maturing on or before the 15th.
// Bonds maturing after the 15th exit in the following month's rebalance.
predictRolloff:{[composition;targetMonth;indexName]
    // Target maturity month is 12 months ahead
    targetMaturityMonth:targetMonth + 12;

    // Bonds maturing in that month, on or before the 15th
    rolloffs:select from composition where
        (`month$maturityDate) = targetMaturityMonth,
        (`dd$maturityDate) <= 15;
...
forecastMonth:{[bonds;currentComp;targetMonth;indexName;asOfDate]
    ...
    // 3. Add new issuance (only net new: 10Y, 20Y, 30Y quarterly auctions)
    // Monthly auctions (2Y-7Y) are replacements for maturing bonds, not net additions
    newIssues:.tsyindex.predictNewIssuance[bonds;targetMonth;asOfDate];
