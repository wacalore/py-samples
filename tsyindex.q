...
// Get most recent auction size for a term (from actual data)
// Returns the outstanding amount from the most recent auction of that term
getRecentAuctionSize:{[bonds;asOfDate;oTerm]
    recent:select from bonds where origTerm = oTerm, auctionDate <= asOfDate;
    if[0 = count recent;
        // Fallback to static typical sizes if no data
        :$[oTerm ~ `2Y; 75e9; oTerm ~ `3Y; 75e9; oTerm ~ `5Y; 75e9;
           oTerm ~ `7Y; 47e9; oTerm ~ `10Y; 42e9; oTerm ~ `20Y; 13e9; 22e9]];
    recent:`auctionDate xdesc recent;
    first recent`outstanding}
...
predictNewIssuanceTerms:{[bonds;targetMonth;asOfDate;terms]
    ...
    newIssues:{[bonds;targetMonth;monthEnd;asOfDate;term]
        ...
        // Use most recent auction size instead of static typical size
        estSize:.tsyindex.getRecentAuctionSize[bonds;asOfDate;term];
