...
// Bonds settling BEFORE this date enter that month's index
// Bonds settling ON or AFTER this date enter the NEXT month's index
determinationDate:{[ym]
    lbd:lastBizDayOfMonth ym;
    nBizDaysBefore[lbd;2]}

// Check if a settlement date qualifies for a month's index
// settlementDate must be BEFORE the determination date (strictly less than)
settlesForMonth:{[ym;settlementDate]
    settlementDate < determinationDate ym}
...
// Returns bonds issuing in the window between:
//   - Prior month's prior determination date (exclusive, already in prior index)
//   - Prior month's determination date (exclusive, cutoff for this index)
// Example for January index: bonds issuing between Nov 26 (exclusive) and Dec 29 (exclusive)
bondsEnteringForIndexWithAuctions:{[bonds;indexMonth;indexName;auctions]
    // The determination date cutoff for this index
    detDate:determinationDate indexMonth - 1;
    // The prior determination date (bonds before this are already in prior index)
...
// startDate: exclusive lower bound (bonds after this date)
// endDate: exclusive upper bound (bonds before this date)
// announceCutoff: only include auctions announced by this date (for predictions)
// auctions: (optional) pre-loaded auction cache - pass (::) to load from file
// Returns ALL auctions (both original and reopenings) settling in the date range
bondsEnteringByDateRangeWithAuctions:{[bonds;indexName;startDate;endDate;announceCutoff;auctions]
    ...
    // Filter to auctions settling in the date range
    if[0 < count auctionData;
        auctionData:select from auctionData where issueDate > startDate, issueDate < endDate];

    // Cross-reference with bonds table to exclude TIPS/FRN (bonds table only has coupon Notes/Bonds)
    if[0 < count auctionData;
    ...
    // Also get from bonds cache for bonds not in auction data
    enteringCache:select from bonds where issueDate > startDate, issueDate < endDate;
...
extensionDetailWithAuctions:{[bonds;prices;asOfDate;indexName;weighting;auctions]
    ...
    // Apply same weighting to projected composition
    // Note: new issues have 100% publicFloat at issuance, so no adjustment needed for them
    projComp:.tsyindex.applyWeighting[projComp;bonds;weighting];
...
// extensionTimeSeries: Calculate index extension for each date in a prices table
...
extensionTimeSeries:{[bonds;pricesTable;indexName;weighting]
    ...
    // Load auction cache ONCE at the start (not per date)
    ...
    // Process each date
    calcOneDate:{[bonds;pricesTable;indexName;weighting;auctions;dt]
        // Extract prices for this date as dict
        dayPrices:exec cusip!price from pricesTable where date = dt;

        // Calculate extension using WithAuctions version (with error handling)
        result:.[.tsyindex.extensionDetailWithAuctions; (bonds;dayPrices;dt;indexName;weighting;auctions);
    ...
    results:calcOneDate[bonds;pricesTable;indexName;weighting;auctions;] each dates;
    raze results}
