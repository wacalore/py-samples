...
outstandingAsOf:{[auctions;asOfDate]
    // Sum offeringAmt for auctions that have settled by asOfDate
    settled:select totalOut:sum offeringAmt by cusip from auctions where issueDate <= asOfDate;
...
extensionDetailWithAuctions:{[bonds;prices;asOfDate;indexName;weighting;auctions]
    ...
    // Apply SOMA-adjusted weighting if requested (skip for now until we have historical SOMA)
    // comp:.tsyindex.applyWeighting[comp;bonds;weighting];
    // For now, just use outstanding-based weighting
    stats:.tsyindex.indexStats comp;
    ...
    // Skip SOMA weighting for projected comp until we have historical SOMA data
