...
optimizeAlphasCore:{[R;deltaCtx;rf;lamCorr;lamTO;lo;hi;nIter]
    ...
    w:projectConstrained[lo;hi;n#1f%n];

    // State
    state:`w`obj`iter`converged!(w;alphaObj[p;w];0;0b);

    // SLSQP step with backtracking line search
    step:{[p;lo;hi;s]
        if[s`converged; :s];
        wCur:s`w;
        objCur:s`obj;
        ...
        cnt:0;
        while[(objNew < objCur) and (alpha > 1e-10) and (cnt < 20);
            alpha:0.5 * alpha;
            wNew:projectConstrained[lo;hi;wCur + alpha * dir];
            objNew:alphaObj[p;wNew];
            cnt+:1];
...
alphaReport:{[alphas;returns;weights]
    ...
    `perAlpha`combined`corrMatrix!(
        perAlpha;
        `sharpe`return`volatility`portfolioTO`sumOfPartsTO`nettingBenefit!(
            portSharpeVal;annFactor*portMu;sqrt[annFactor]*portVol;portTO;sumAlphaTO;1-portTO%sumAlphaTO);
        Corr)}
