// =============================================================================
// PORTFOLIO OPTIMIZER
// =============================================================================
// Optimization methods for portfolio construction
// Version: 0.1.0

\d .optimizer

// -----------------------------------------------------------------------------
// UTILITY FUNCTIONS
// -----------------------------------------------------------------------------

// Annualization factor (default 252 trading days)
annFactor:252

// Portfolio return given weights and asset returns matrix
// w: weight vector, R: matrix where each row is asset returns
portRet:{[w;R] sum w * avg each R}

// Portfolio volatility given weights and covariance matrix
portVol:{[w;C] sqrt sum w * .kdbtools.mm[C;w]}

// Portfolio Sharpe ratio (annualized)
// Returns annualize by multiplying by periods (252)
// Volatility annualizes by multiplying by sqrt(periods)
portSharpe:{[w;R;C;rf]
    ret:annFactor * sum w * avg each R;
    vol:sqrt[annFactor] * portVol[w;C];
    (ret - rf) % vol}

// Median return (more robust to outliers)
medianRet:{[R] med each R}

// Portfolio median Sharpe
portMedianSharpe:{[w;R;C;rf]
    ret:annFactor * sum w * medianRet R;
    vol:sqrt[annFactor] * portVol[w;C];
    (ret - rf) % vol}

// Normalize weights to sum to 1
normalize:{x % sum x}

// Clip weights to bounds
clipWeights:{[lo;hi;w] lo | w & hi}

// Project weights to simplex (sum to 1, non-negative)
projectSimplex:{[w]
    n:count w;
    u:desc w;
    cssv:sums u;
    rho:last where (u + (1 - cssv) % 1 + til n) > 0;
    theta:(cssv[rho] - 1) % rho + 1;
    0f | w - theta}

// -----------------------------------------------------------------------------
// TABLE CONVERSION HELPERS
// -----------------------------------------------------------------------------

// Convert a table of returns to matrix format for optimizer
// Each column becomes one asset's return series (one row in matrix)
// Usage: R:.optimizer.fromTable[t]
//        R:.optimizer.fromTable[delete date from t]
fromTable:{[t] value flip t}

// Convert table, excluding specified columns (e.g., date, sym)
// Usage: R:.optimizer.fromTableEx[`date`sym;t]
fromTableEx:{[excludeCols;t] fromTable (cols[t] except excludeCols)#t}

// Convert long-format table (date;sym;ret) to matrix
// Each unique sym becomes one row
// Usage: R:.optimizer.fromLongTable[`sym;`ret;t]
fromLongTable:{[symCol;retCol;t] value exec retCol by symCol from t}

// Get asset names from table (for labeling results)
// Usage: assets:.optimizer.assetNames[`date;t]
assetNames:{[excludeCols;t] cols[t] except excludeCols}

// Attach asset names to optimization result weights
// Usage: .optimizer.labelWeights[`date;t;result]
labelWeights:{[excludeCols;t;result]
    assets:assetNames[excludeCols;t];
    result,enlist[`weightsByAsset]!enlist assets!result`weights}

// Convenience wrapper: optimize directly from table
// Usage: .optimizer.optimizeTable[t;`date;0.02;`maxSharpe;10000]
//        .optimizer.optimizeTable[t;`date;0.02;`constrained;0.05;0.4;10000]
optimizeTable:{[t;excludeCols;rf;method;args]
    R:fromTableEx[excludeCols;t];
    assets:assetNames[excludeCols;t];
    result:$[method~`maxSharpe;        maxSharpe[R;rf;args 0];
             method~`constrained;      maxSharpeConstrained[R;rf;args 0;args 1;args 2];
             method~`slsqp;            maxSharpeSLSQP[R;rf;args 0;args 1;args 2];
             method~`hillClimb;        maxSharpeHillClimb[R;rf;args 0;args 1];
             method~`hybrid;           maxSharpeHybrid[R;rf;args 0;args 1;args 2];
             method~`median;           maxMedianSharpe[R;rf;args 0];
             method~`riskParity;       riskParity[R;args 0;args 1];
             method~`minVariance;      minVariance[R;args 0];
             method~`minCDaR;          minCDaR[R;args 0;args 1];
             method~`minCDaRConstr;    minCDaRConstrained[R;args 0;args 1;args 2;args 3];
             method~`maxCER;           maxCER[R;args 0;args 1];
             method~`maxCERAnalytical; maxCERAnalytical[R;args 0];
             method~`maxCERConstr;     maxCERConstrained[R;args 0;args 1;args 2;args 3];
             method~`minEntropic;      minEntropicRisk[R;args 0;args 1];
             method~`minEntropicConstr;minEntropicRiskConstrained[R;args 0;args 1;args 2;args 3];
             method~`maxKappa;         maxKappa[R;args 0;args 1;args 2];
             method~`maxKappaConstr;   maxKappaConstrained[R;args 0;args 1;args 2;args 3;args 4];
             method~`maxSortino;       maxSortino[R;args 0];
             method~`maxSortinoConstr; maxSortinoConstrained[R;args 0;args 1;args 2];
             '`unknownMethod];
    result,enlist[`assets]!enlist assets!result`weights}

// -----------------------------------------------------------------------------
// VECTORIZED HELPERS (OPTIMIZED)
// -----------------------------------------------------------------------------

// Generate random weight matrix: nIter rows, n columns, each row sums to 1
randWeightMatrix:{[nIter;n]
    raw:(nIter;n)#(nIter*n)?1f;
    {x%sum x} each raw}

// Vectorized portfolio variances: w'Cw for each row of W
// W: nIter x n, C: n x n -> returns nIter vector
portVarsVec:{[W;C]
    WC:.kdbtools.mm[W;C];          // nIter x n
    sum each WC * W}                // diagonal of W*C*W'

// Vectorized Sharpe ratios for weight matrix
// W: nIter x n, mu: n vector (mean returns), C: n x n covariance
sharpesVec:{[W;mu;C;rf]
    rets:annFactor * sum each W *\: mu;   // nIter annualized returns
    vars:portVarsVec[W;C];                 // nIter variances
    vols:sqrt[annFactor] * sqrt vars;      // nIter annualized vols
    (rets - rf) % vols}

// Vectorized CER: E[r] - 0.5*lambda*Var[r] for all portfolios
cersVec:{[W;mu;C;lambda]
    rets:annFactor * sum each W *\: mu;
    vars:annFactor * portVarsVec[W;C];
    rets - 0.5 * lambda * vars}

// Vectorized portfolio returns matrix: nIter x T
// W: nIter x n, R: n x T -> returns nIter x T matrix
portRetsMatrix:{[W;R] .kdbtools.mm[W;R]}

// Vectorized entropic risk for all portfolios
// Returns nIter vector of entropic risks
entropicRisksVec:{[W;R;theta]
    PR:portRetsMatrix[W;R];               // nIter x T
    (1 % theta) * log each avg each exp neg theta * PR}

// Vectorized LPM (Lower Partial Moment) for all portfolios
// Returns nIter vector of LPMs
lpmsVec:{[W;R;order;mar]
    PR:portRetsMatrix[W;R];               // nIter x T
    shortfalls:mar - PR;                   // nIter x T
    avg each (0f | shortfalls) xexp order}

// Vectorized Kappa ratios for all portfolios
kappasVec:{[W;R;order;mar]
    PR:portRetsMatrix[W;R];
    excessRets:(avg each PR) - mar;
    lpms:lpmsVec[W;R;order;mar];
    // Handle division by zero
    excessRets % lpms xexp 1 % order}

// -----------------------------------------------------------------------------
// MAX SHARPE - VECTORIZED RANDOM SEARCH
// -----------------------------------------------------------------------------

// Find max Sharpe portfolio via vectorized random search
maxSharpe:{[R;rf;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Generate all random portfolios as matrix
    W:randWeightMatrix[nIter;n];

    // Vectorized Sharpe computation
    sharpes:sharpesVec[W;mu;C;rf];

    // Find best
    bestIdx:sharpes?max sharpes;
    bestW:W bestIdx;

    `weights`sharpe`return`volatility!(
        bestW;
        sharpes bestIdx;
        annFactor * sum bestW * mu;
        sqrt[annFactor] * sqrt portVarsVec[enlist bestW;C] 0)}

// -----------------------------------------------------------------------------
// MAX MEDIAN SHARPE - VECTORIZED
// -----------------------------------------------------------------------------

// Vectorized median Sharpe
medianSharpesVec:{[W;medR;C;rf]
    rets:annFactor * sum each W *\: medR;
    vars:portVarsVec[W;C];
    vols:sqrt[annFactor] * sqrt vars;
    (rets - rf) % vols}

// Find max median Sharpe portfolio (more robust to outliers)
maxMedianSharpe:{[R;rf;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    medR:med each R;

    W:randWeightMatrix[nIter;n];
    sharpes:medianSharpesVec[W;medR;C;rf];

    bestIdx:sharpes?max sharpes;
    bestW:W bestIdx;

    `weights`sharpe`medianReturn`volatility!(
        bestW;
        sharpes bestIdx;
        annFactor * sum bestW * medR;
        sqrt[annFactor] * sqrt portVarsVec[enlist bestW;C] 0)}

// -----------------------------------------------------------------------------
// MAX SHARPE WITH WEIGHT CONSTRAINTS - VECTORIZED
// -----------------------------------------------------------------------------

// Generate constrained random weight matrix
randWeightMatrixConstrained:{[nIter;n;lo;hi]
    raw:{[lo;hi;n;i] lo + (hi-lo)*n?1f}[lo;hi;n] each til nIter;
    projectBoundedSimplex[lo;hi] each raw}

// Find max Sharpe with weight limits
maxSharpeConstrained:{[R;rf;lo;hi;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    W:randWeightMatrixConstrained[nIter;n;loVec;hiVec];
    sharpes:sharpesVec[W;mu;C;rf];

    bestIdx:sharpes?max sharpes;
    bestW:W bestIdx;

    `weights`sharpe`return`volatility`bounds!(
        bestW;
        sharpes bestIdx;
        annFactor * sum bestW * mu;
        sqrt[annFactor] * sqrt portVarsVec[enlist bestW;C] 0;
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// GRADIENT-BASED OPTIMIZATION (OPTIMIZED)
// -----------------------------------------------------------------------------

// ANALYTICAL gradient of Sharpe ratio (much faster than numerical)
// Sharpe = (w'μ - rf) / sqrt(w'Cw)
// ∂Sharpe/∂w = (μ*σ - (r-rf)*Cw/σ) / σ² = (μ - sharpe*Cw/σ) / σ
// rf is annual, convert to daily for gradient computation
sharpeGradAnalytical:{[mu;C;rf;w]
    Cw:.kdbtools.mm[C;w];
    variance:sum w * Cw;
    sigma:sqrt variance;
    ret:sum w * mu;
    rfDaily:rf % annFactor;
    sharpe:(ret - rfDaily) % sigma;
    (mu - sharpe * Cw % sigma) % sigma}

// Compute Sharpe for weight vector (daily scale, for optimization)
// rf is annual, convert to daily for comparison with daily returns
sharpeRaw:{[mu;C;rf;w] (sum[w*mu] - rf % annFactor) % sqrt sum w * .kdbtools.mm[C;w]}

// Project w onto bounded simplex: {sum(w)=1, lo<=w<=hi}
// Finds theta such that sum clip(w-theta, lo, hi) = 1 via bisection
// f(theta) = sum clip(...) is monotone non-increasing → bisection converges
projectBoundedSimplex:{[lo;hi;w]
    slo:sum lo;
    if[slo > 1 + 1e-10; :lo * 1 % slo];
    shi:sum hi;
    if[shi < 1 - 1e-10; :hi * 1 % shi];
    wc:lo | w & hi;
    if[abs[1 - sum wc] < 1e-12; :wc];
    tL:(min w) - max hi;
    tH:(max w) - min lo;
    st:50 {[lo;hi;w;s]
        m:0.5 * s[0] + s 1;
        $[1f < sum lo | (w - m) & hi; (m;s 1); (s 0;m)]
    }[lo;hi;w]/(tL;tH);
    theta:0.5 * st[0] + st 1;
    lo | (w - theta) & hi}

projectConstrained:{[lo;hi;w] projectBoundedSimplex[lo;hi;w]}

// Backtracking line search for Sharpe optimization (projected Armijo)
// p: `mu`C`rf`lo`hi dict; gradCur: gradient at wCur
// Uses projected Armijo condition: f(wNew) <= f(wCur) + c1 * g'*(wNew-wCur)
sharpeLineSearch:{[p;wCur;objCur;gradCur;dir]
    alpha:1f;
    wNew:projectConstrained[p`lo;p`hi;wCur + alpha * dir];
    objNew:neg sharpeRaw[p`mu;p`C;p`rf;wNew];
    cnt:0;
    while[(objNew > objCur + 1e-4 * sum gradCur * wNew - wCur) and (alpha > 1e-12) and (cnt < 40);
        alpha:0.5 * alpha;
        wNew:projectConstrained[p`lo;p`hi;wCur + alpha * dir];
        objNew:neg sharpeRaw[p`mu;p`C;p`rf;wNew];
        cnt+:1];
    (wNew;objNew)}

// L-BFGS two-loop recursion: compute search direction from gradient history
// Returns H*q (approximate inverse-Hessian times gradient)
// sHist/yHist: lists of (s_k, y_k) pairs, most recent last
// q: current gradient
lbfgsDir:{[sHist;yHist;q]
    m:count sHist;
    if[0=m; :q];   // No history — fall back to steepest descent
    // Forward loop: compute alphas
    alphas:m#0f;
    r:q;
    i:m-1;
    while[i >= 0;
        rho:1f % sum (yHist i) * sHist i;
        a:rho * sum (sHist i) * r;
        alphas[i]:a;
        r:r - a * yHist i;
        i-:1];
    // Initial Hessian approximation: H0 = (s'y / y'y) * I
    sk:last sHist; yk:last yHist;
    gamma:(sum sk * yk) % sum yk * yk;
    r:gamma * r;
    // Backward loop
    i:0;
    while[i < m;
        rho:1f % sum (yHist i) * sHist i;
        b:rho * sum (yHist i) * r;
        r:r + (alphas[i] - b) * sHist i;
        i+:1];
    r}

// Max Sharpe via L-BFGS projected gradient with backtracking line search
maxSharpeSLSQP:{[R;rf;lo;hi;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    // Analytical tangency portfolio as starting point (project to bounds)
    excessMu:mu - rf % annFactor;
    tpw:n#0f;
    // Solve C*w = excessMu via CG (50 iterations, cheap)
    tpState:`w`r`p!(n#1f%n; excessMu - .kdbtools.mm[C;n#1f%n]; excessMu - .kdbtools.mm[C;n#1f%n]);
    cgStep:{[C;s]
        Ap:.kdbtools.mm[C;s`p];
        alpha:(sum s[`r]*s`r) % sum s[`p]*Ap;
        wNew:s[`w] + alpha * s`p;
        rNew:s[`r] - alpha * Ap;
        beta:(sum rNew*rNew) % sum s[`r]*s`r;
        pNew:rNew + beta * s`p;
        `w`r`p!(wNew;rNew;pNew)};
    tpState:50 cgStep[C]/tpState;
    tpw:tpState`w;
    tpw:$[0 < sum tpw; tpw % sum tpw; n#1f%n];  // Normalize (handle all-negative case)

    // Pack parameters into dict to avoid 8-param limit
    p:`mu`C`rf`lo`hi!(mu;C;rf;loVec;hiVec);

    // Initialize with multiple starting points, keep best
    starts:(n#1f%n;                              // Equal weight
            loVec + (hiVec-loVec) * n?1f;        // Random in bounds
            loVec + 0.5 * hiVec - loVec;         // Midpoint
            tpw);                                // Tangency portfolio
    starts:projectConstrained[loVec;hiVec] each starts;
    startObjs:{[p;w] neg sharpeRaw[p`mu;p`C;p`rf;w]}[p] each starts;
    w:starts startObjs?min startObjs;

    // Optimization state (includes L-BFGS history)
    grad0:neg sharpeGradAnalytical[mu;C;rf;w];
    state:`w`obj`iter`converged`grad`sHist`yHist!(w;min startObjs;0;0b;grad0;();());
    lbfgsMem:10;  // L-BFGS memory depth

    // Single optimization step: L-BFGS with steepest descent fallback
    step:{[p;lbfgsMem;s]
        if[s`converged; :s];

        wCur:s`w;
        objCur:s`obj;
        gradCur:s`grad;
        mu:p`mu; C:p`C; rf:p`rf;

        // Check convergence via projected gradient
        // At bounds: only count gradient if it points into the interior
        atLo:wCur <= (p`lo) + 1e-12;
        atHi:wCur >= (p`hi) - 1e-12;
        projGrad:gradCur;
        // For minimizing -Sharpe: gradient pointing towards lower -Sharpe (i.e., grad < 0)
        // At lower bound: only movable if grad < 0 (descent wants to increase w, which is feasible)
        // At upper bound: only movable if grad > 0 (descent wants to decrease w, which is feasible)
        projGrad:?[atLo and projGrad > 0; 0f; projGrad];
        projGrad:?[atHi and projGrad < 0; 0f; projGrad];
        pgNorm:sqrt sum projGrad*projGrad;
        if[pgNorm < 1e-10; :s,enlist[`converged]!enlist 1b];

        // Try L-BFGS direction first
        rawDir:lbfgsDir[s`sHist;s`yHist;gradCur];
        isDesc:(sum rawDir * gradCur) > 0;
        hasHistory:0 < count s`sHist;
        lbfgsDir0:$[isDesc; neg rawDir; neg gradCur];  // Descent direction
        dn:sqrt sum lbfgsDir0*lbfgsDir0;
        if[dn < 1e-15; :s,enlist[`converged]!enlist 1b];
        // Normalize steepest descent steps for reasonable initial step size;
        // preserve L-BFGS scale when history exists (encodes curvature)
        if[(not hasHistory) or not isDesc; lbfgsDir0:lbfgsDir0 % dn];

        res:sharpeLineSearch[p;wCur;objCur;gradCur;lbfgsDir0];
        wNew:res 0; objNew:res 1;

        // If L-BFGS failed, fall back to normalized steepest descent
        if[objNew >= objCur;
            sdDir:neg gradCur;
            sdNorm:sqrt sum sdDir*sdDir;
            if[sdNorm > 1e-15;
                sdDir:sdDir % sdNorm;
                res:sharpeLineSearch[p;wCur;objCur;gradCur;sdDir];
                wNew:res 0; objNew:res 1]];

        // Converge if neither direction improved
        wDiff:max abs wNew - wCur;
        if[(objNew >= objCur) or (wDiff < 1e-12);
            :s,enlist[`converged]!enlist 1b];

        // Update L-BFGS history
        gradNew:neg sharpeGradAnalytical[mu;C;rf;wNew];
        sk:wNew - wCur;
        yk:gradNew - gradCur;
        syk:sum sk * yk;
        sHist:s`sHist; yHist:s`yHist;
        // Only update if curvature condition holds; reset history if violated
        $[syk > 1e-10 * (sqrt sum sk*sk) * sqrt sum yk*yk;
            [sHist:$[lbfgsMem <= count sHist; 1 _ sHist; sHist],enlist sk;
             yHist:$[lbfgsMem <= count yHist; 1 _ yHist; yHist],enlist yk];
            [sHist:(); yHist:()]];   // Reset on curvature failure

        `w`obj`iter`converged`grad`sHist`yHist!(wNew;objNew;s[`iter]+1;0b;gradNew;sHist;yHist)};

    state:nIter step[p;lbfgsMem]/state;
    w:state`w;

    `weights`sharpe`return`volatility`bounds`iterations`converged!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w];
        `lo`hi!(loVec;hiVec);
        state`iter;
        state`converged)}

// Unconstrained analytical solution (allows negative weights)
// w* = C^-1 * (μ - rf) / sum(C^-1 * (μ - rf))
maxSharpeAnalytical:{[R;rf]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Solve C * w = (mu - rf/252) using LU decomposition approximation
    // Since we don't have matrix inverse, use gradient descent on quadratic
    excessMu:mu - rf % annFactor;

    // Iteratively solve C*w = excessMu
    w:n#1f%n;
    solveStep:{[C;b;w]
        Cw:.kdbtools.mm[C;w];
        resid:b - Cw;
        // Gradient of ||Cw - b||^2 is 2*C'*(Cw-b) = 2*C*(Cw-b) for symmetric C
        grad:.kdbtools.mm[C;resid];
        // Step size from exact line search on quadratic
        Cg:.kdbtools.mm[C;grad];
        alpha:(sum grad * resid) % sum grad * Cg;
        w + alpha * grad};

    w:500 solveStep[C;excessMu]/w;

    // Normalize to sum to 1
    w:w % sum w;

    `weights`sharpe`return`volatility!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w])}

// Numerical gradient (kept for comparison/fallback)
sharpeGradNumerical:{[mu;C;rf;w;eps]
    n:count w;
    portSh:{[mu;C;rf;w] (sum[w*mu] - rf) % sqrt sum w * .kdbtools.mm[C;w]};
    base:portSh[mu;C;rf;w];
    {[mu;C;rf;w;eps;base;ps;i]
        wUp:@[w;i;+;eps];
        wUp:wUp % sum wUp;
        (ps[mu;C;rf;wUp] - base) % eps
    }[mu;C;rf;w;eps;base;portSh] each til n}

// -----------------------------------------------------------------------------
// DV01-CONSTRAINED SHARPE OPTIMIZATION
// -----------------------------------------------------------------------------

// Objective: maximize Sharpe ratio subject to net and gross DV01 penalties
// p: `mu`C`rf`dv01s`netLimit`grossLimit`lamDV01 dict
// Returns negative of penalized Sharpe (for minimization)
dv01Obj:{[p;w]
    base:sharpeRaw[p`mu;p`C;p`rf;w];
    dw:w * p`dv01s;
    netDV01:sum dw;
    grossDV01:sum abs dw;
    netViol:0f | (abs netDV01) - p`netLimit;
    grossViol:0f | grossDV01 - p`grossLimit;
    (neg base) + (p[`lamDV01] * netViol * netViol) + (p[`lamDV01] * grossViol * grossViol)}

// Analytical gradient of DV01-penalized Sharpe
// Base Sharpe gradient + penalty gradients
dv01Grad:{[p;w]
    mu:p`mu; C:p`C; rf:p`rf; dv01s:p`dv01s; lam:p`lamDV01;
    // Base Sharpe gradient (of negative Sharpe for minimization)
    baseGrad:neg sharpeGradAnalytical[mu;C;rf;w];
    // Net DV01 penalty gradient
    // P_net = lam * max(0, |d'w| - L)^2
    // dP/dw = 2*lam*viol*sign(d'w)*d
    dw:w * dv01s;
    netDV01:sum dw;
    netViol:0f | (abs netDV01) - p`netLimit;
    netSign:$[netDV01 >= 0; 1f; -1f];
    netGrad:(2 * lam * netViol * netSign) * dv01s;
    // Gross DV01 penalty gradient
    // P_gross = lam * max(0, sum|d*w| - L)^2
    // dP/dw_i = 2*lam*viol*sign(w_i*d_i)*d_i
    grossDV01:sum abs dw;
    grossViol:0f | grossDV01 - p`grossLimit;
    dwSign:signum dw;
    // Handle zero: when w_i*d_i = 0, subgradient is 0
    grossGrad:(2 * lam * grossViol * dwSign) * dv01s;
    // Total: minimize neg(Sharpe - penalties) = neg Sharpe + penalties
    // baseGrad is grad of neg Sharpe; penalty grads reduce Sharpe, so add them
    baseGrad + netGrad + grossGrad}

// Backtracking line search for DV01-penalized objective
// Returns (wNew;objNew) where objNew = dv01Obj (negative penalized Sharpe)
dv01LineSearch:{[p;lo;hi;wCur;objCur;dir;dirDeriv]
    alpha:1f;
    wNew:projectConstrained[lo;hi;wCur + alpha * dir];
    objNew:dv01Obj[p;wNew];
    cnt:0;
    while[(objNew > objCur + (1e-4 * alpha * dirDeriv)) and (alpha > 1e-12) and (cnt < 30);
        alpha:0.5 * alpha;
        wNew:projectConstrained[lo;hi;wCur + alpha * dir];
        objNew:dv01Obj[p;wNew];
        cnt+:1];
    (wNew;objNew)}

// Core L-BFGS engine for DV01-constrained Sharpe maximization
// cfg: `lo`hi`netLimit`grossLimit`lamDV01`nIter dict
// Returns dict with weights and metrics
maxSharpeDV01Core:{[R;rf;dv01s;cfg]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;
    lo:cfg`lo; hi:cfg`hi; netLimit:cfg`netLimit;
    grossLimit:cfg`grossLimit; lamDV01:cfg`lamDV01; nIter:cfg`nIter;

    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    // Pack params
    p:`mu`C`rf`dv01s`netLimit`grossLimit`lamDV01!(mu;C;rf;dv01s;netLimit;grossLimit;lamDV01);

    // Analytical tangency as starting point
    excessMu:mu - rf % annFactor;
    tpState:`w`r`p!(n#1f%n; excessMu - .kdbtools.mm[C;n#1f%n]; excessMu - .kdbtools.mm[C;n#1f%n]);
    cgStep:{[C;s]
        Ap:.kdbtools.mm[C;s`p];
        a:(sum s[`r]*s`r) % sum s[`p]*Ap;
        wNew:s[`w] + a * s`p;
        rNew:s[`r] - a * Ap;
        beta:(sum rNew*rNew) % sum s[`r]*s`r;
        pNew:rNew + beta * s`p;
        `w`r`p!(wNew;rNew;pNew)};
    tpState:50 cgStep[C]/tpState;
    tpw:tpState`w;
    tpw:$[0 < sum tpw; tpw % sum tpw; n#1f%n];

    // Multiple starting points
    starts:(n#1f%n;
            loVec + (hiVec - loVec) * n?1f;
            loVec + 0.5 * hiVec - loVec;
            tpw);
    starts:projectConstrained[loVec;hiVec] each starts;
    startObjs:dv01Obj[p;] each starts;
    w:starts startObjs?min startObjs;

    // L-BFGS state
    grad0:dv01Grad[p;w];
    state:`w`obj`iter`converged`grad`sHist`yHist!(w;min startObjs;0;0b;grad0;();());
    lbfgsMem:10;

    // L-BFGS step with steepest descent fallback
    step:{[p;lo;hi;lbfgsMem;s]
        if[s`converged; :s];
        wCur:s`w; objCur:s`obj; gradCur:s`grad;

        // Projected gradient convergence
        atLo:wCur <= lo + 1e-12;
        atHi:wCur >= hi - 1e-12;
        projGrad:gradCur;
        projGrad:?[atLo and projGrad > 0; 0f; projGrad];
        projGrad:?[atHi and projGrad < 0; 0f; projGrad];
        pgNorm:sqrt sum projGrad * projGrad;
        if[pgNorm < 1e-10; :s,enlist[`converged]!enlist 1b];

        // L-BFGS direction
        rawDir:lbfgsDir[s`sHist;s`yHist;gradCur];
        isDesc:(sum rawDir * gradCur) > 0;
        lDir:$[isDesc; neg rawDir; neg gradCur];
        dn:sqrt sum lDir * lDir;
        if[dn < 1e-15; :s,enlist[`converged]!enlist 1b];
        lDir:lDir % dn;
        dd:sum gradCur * lDir;

        res:dv01LineSearch[p;lo;hi;wCur;objCur;lDir;dd];
        wNew:res 0; objNew:res 1;

        // Steepest descent fallback
        if[objNew >= objCur;
            sdDir:neg gradCur;
            sdNorm:sqrt sum sdDir * sdDir;
            if[sdNorm > 1e-15;
                sdDir:sdDir % sdNorm;
                sdDD:sum gradCur * sdDir;
                res:dv01LineSearch[p;lo;hi;wCur;objCur;sdDir;sdDD];
                wNew:res 0; objNew:res 1]];

        wDiff:max abs wNew - wCur;
        if[(objNew >= objCur) or (wDiff < 1e-12);
            :s,enlist[`converged]!enlist 1b];

        // Update L-BFGS history
        gradNew:dv01Grad[p;wNew];
        sk:wNew - wCur;
        yk:gradNew - gradCur;
        syk:sum sk * yk;
        sHist:s`sHist; yHist:s`yHist;
        $[syk > 1e-10 * (sqrt sum sk * sk) * sqrt sum yk * yk;
            [sHist:$[lbfgsMem <= count sHist; 1 _ sHist; sHist],enlist sk;
             yHist:$[lbfgsMem <= count yHist; 1 _ yHist; yHist],enlist yk];
            [sHist:(); yHist:()]];

        `w`obj`iter`converged`grad`sHist`yHist!(wNew;objNew;s[`iter]+1;0b;gradNew;sHist;yHist)};

    state:nIter step[p;loVec;hiVec;lbfgsMem]/state;
    w:state`w;

    // Compute final metrics
    dw:w * dv01s;
    `weights`sharpe`return`volatility`netDV01`grossDV01`converged`iterations!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w];
        sum dw;
        sum abs dw;
        state`converged;
        state`iter)}

// Max Sharpe with DV01 constraints via penalty ramping
// R: n×T return matrix; rf: annual risk-free; dv01s: n-vector of DV01 per unit
// cfg: optional dict with netDV01, grossDV01, lo, hi, nIter, lamDV01, lamRamp, nRamp, totalNotional
maxSharpeDV01:{[R;rf;dv01s;cfg]
    if[(::)~cfg; cfg:()!()];
    n:count R;
    netLimit:$[`netDV01 in key cfg; cfg`netDV01; 1e15];
    grossLimit:$[`grossDV01 in key cfg; cfg`grossDV01; 1e15];
    lo:$[`lo in key cfg; cfg`lo; -1f];
    hi:$[`hi in key cfg; cfg`hi; 1f];
    nIter:$[`nIter in key cfg; cfg`nIter; 500];
    lamDV01:$[`lamDV01 in key cfg; cfg`lamDV01; 100f];
    lamRamp:$[`lamRamp in key cfg; cfg`lamRamp; 10f];
    nRamp:$[`nRamp in key cfg; cfg`nRamp; 3];
    totalNotional:$[`totalNotional in key cfg; cfg`totalNotional; 0n];

    // Penalty ramping: solve nRamp rounds with increasing lamDV01
    // Warm-start each round from previous solution
    coreCfg:`lo`hi`netLimit`grossLimit`lamDV01`nIter!(lo;hi;netLimit;grossLimit;lamDV01;nIter);
    result:maxSharpeDV01Core[R;rf;dv01s;coreCfg];
    i:1;
    while[i < nRamp;
        lamDV01:lamDV01 * lamRamp;
        coreCfg[`lamDV01]:lamDV01;
        result:maxSharpeDV01Core[R;rf;dv01s;coreCfg];
        i+:1];

    w:result`weights;
    // Compute notional if totalNotional provided
    notional:$[null totalNotional; count[w]#0n; w * totalNotional];

    // Feasibility checks (within 1% tolerance of limit)
    netTol:1f | 0.01 * netLimit;
    grossTol:1f | 0.01 * grossLimit;

    result,`notional`netDV01Limit`grossDV01Limit`netDV01Feasible`grossDV01Feasible!(
        notional;
        netLimit;
        grossLimit;
        (abs result`netDV01) <= netLimit + netTol;
        result[`grossDV01] <= grossLimit + grossTol)}

// Hill climbing with analytical gradient and momentum
// Uses normalized gradient for stable convergence
maxSharpeHillClimb:{[R;rf;nIter;lr]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Start with inverse volatility (better than equal weight)
    vars:{x[y;y]}[C] each til n;
    w:normalize 1f % sqrt vars;

    // Gradient ascent with momentum and normalized gradient
    beta:0.8;  // Momentum coefficient
    state:`w`v!(w;n#0f);

    step:{[mu;C;rf;lr;beta;s]
        g:sharpeGradAnalytical[mu;C;rf;s`w];
        gNorm:g % sqrt sum g*g;  // Normalize gradient
        v:(beta * s`v) + (1-beta) * gNorm;
        wNew:s[`w] + lr * v;
        wNew:projectSimplex wNew;
        `w`v!(wNew;v)};

    state:nIter step[mu;C;rf;lr;beta]/state;
    w:state`w;

    `weights`sharpe`return`volatility!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w])}

// Hill climbing with weight constraints (analytical gradient + momentum)
// Uses normalized gradient for stable convergence
maxSharpeHillClimbConstrained:{[R;rf;lo;hi;nIter;lr]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    // Start with projected inverse volatility
    vars:{x[y;y]}[C] each til n;
    w:projectBoundedSimplex[loVec;hiVec;1f % sqrt vars];

    // Gradient ascent with momentum, normalized gradient, and projection
    beta:0.8;
    state:`w`v!(w;n#0f);

    step:{[mu;C;rf;lr;beta;lo;hi;s]
        g:sharpeGradAnalytical[mu;C;rf;s`w];
        gNorm:g % sqrt sum g*g;  // Normalize gradient
        v:(beta * s`v) + (1-beta) * gNorm;
        wNew:s[`w] + lr * v;
        wNew:projectBoundedSimplex[lo;hi;wNew];
        `w`v!(wNew;v)};

    state:nIter step[mu;C;rf;lr;beta;loVec;hiVec]/state;
    w:state`w;

    `weights`sharpe`return`volatility`bounds!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w];
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// COMBINED APPROACH: RANDOM START + HILL CLIMBING
// -----------------------------------------------------------------------------

// Best of both: random search to find good starting point, then refine
// Uses normalized gradient for stable convergence
maxSharpeHybrid:{[R;rf;nRandom;nClimb;lr]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Random search phase
    randResult:maxSharpe[R;rf;nRandom];
    w:randResult`weights;

    // Refine with analytical gradient + momentum (normalized)
    beta:0.8;
    state:`w`v!(w;n#0f);

    step:{[mu;C;rf;lr;beta;s]
        g:sharpeGradAnalytical[mu;C;rf;s`w];
        gNorm:g % sqrt sum g*g;  // Normalize gradient
        v:(beta * s`v) + (1-beta) * gNorm;
        wNew:s[`w] + lr * v;
        wNew:projectSimplex wNew;
        `w`v!(wNew;v)};

    state:nClimb step[mu;C;rf;lr;beta]/state;
    w:state`w;

    `weights`sharpe`return`volatility!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w])}

// Hybrid with constraints
// Uses normalized gradient for stable convergence
maxSharpeHybridConstrained:{[R;rf;lo;hi;nRandom;nClimb;lr]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    // Random search phase
    randResult:maxSharpeConstrained[R;rf;lo;hi;nRandom];
    w:randResult`weights;

    // Refine with analytical gradient + momentum (normalized)
    beta:0.8;
    state:`w`v!(w;n#0f);

    step:{[mu;C;rf;lr;beta;lo;hi;s]
        g:sharpeGradAnalytical[mu;C;rf;s`w];
        gNorm:g % sqrt sum g*g;  // Normalize gradient
        v:(beta * s`v) + (1-beta) * gNorm;
        wNew:s[`w] + lr * v;
        wNew:projectBoundedSimplex[lo;hi;wNew];
        `w`v!(wNew;v)};

    state:nClimb step[mu;C;rf;lr;beta;loVec;hiVec]/state;
    w:state`w;

    `weights`sharpe`return`volatility`bounds!(
        w;
        portSharpe[w;R;C;rf];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w];
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// EQUAL RISK CONTRIBUTION (RISK PARITY)
// -----------------------------------------------------------------------------

// Simple risk parity: weights inverse to volatility
riskParity:{[R;nIter;lr]
    n:count R;
    C:.kdbtools.covmat flip R;

    // Extract diagonal (variances)
    vars:{x[y;y]}[C] each til n;
    vols:sqrt vars;
    w:normalize 1f % vols;

    `weights`sharpe`return`volatility!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C])}

// -----------------------------------------------------------------------------
// MINIMUM VARIANCE PORTFOLIO
// -----------------------------------------------------------------------------

// Minimum variance portfolio (uses inverse covariance)
// For long-only, use iterative projection
minVariance:{[R;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;

    // Start with risk parity as initial guess
    vars:{x[y;y]}[C] each til n;
    w:normalize 1f % sqrt vars;

    // Gradient descent on portfolio variance
    step:{[C;n;w]
        grad:2 * .kdbtools.mm[C;w];
        wNew:w - 0.1 * grad;
        wNew:0f | wNew;  // Long only
        wNew % sum wNew};

    w:nIter step[C;n]/w;

    `weights`sharpe`return`volatility!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C])}

// -----------------------------------------------------------------------------
// ADVANCED RISK MEASURES
// -----------------------------------------------------------------------------

// Drawdown series from cumulative returns
drawdowns:{[cumRets] cumRets - maxs cumRets}

// Maximum drawdown
maxDD:{[cumRets] min drawdowns cumRets}

// Conditional Drawdown at Risk (average of worst alpha% drawdowns)
CDaR:{[alpha;cumRets]
    dd:drawdowns cumRets;
    nTail:1 | floor alpha * count dd;
    avg nTail # asc dd}

// Portfolio cumulative returns
portCumRet:{[w;R] sums sum w * R}

// Portfolio CVaR: average of worst alpha% single-period returns
portCVaR:{[alpha;w;R]
    Rw:sum w * R;
    nTail:1 | floor alpha * count Rw;
    avg nTail # asc Rw}

// CVaR subgradient: avg of R_k(t) over tail set {t : Rw(t) <= VaR}
portCVaRGrad:{[alpha;w;R]
    Rw:sum w * R;
    nTail:1 | floor alpha * count Rw;
    varThreshold:(asc Rw) nTail - 1;
    tailIdx:where Rw <= varThreshold;
    if[0 = count tailIdx; :(count R) # 0f];
    {[R;idx;k] avg R[k] idx}[R;tailIdx] each til count R}

// CDaR subgradient: d(dd(t))/dw_k = cumR_k(t) - cumR_k(peakIdx(t))
portCDaRGrad:{[alpha;w;R]
    Rw:sum w * R;
    cumRw:sums Rw;
    dd:cumRw - maxs cumRw;
    nTail:1 | floor alpha * count dd;
    tailIdx:nTail # iasc dd;
    if[0 = count tailIdx; :(count R) # 0f];
    cumR:sums each R;
    peakIdx:{$[x[z] >= x[y]; z; y]}[cumRw] scan til count cumRw;
    {[cumR;peakIdx;tailIdx;k]
        avg cumR[k][tailIdx] - cumR[k] peakIdx tailIdx
    }[cumR;peakIdx;tailIdx] each til count R}

// Certainty Equivalent Return: E[r] - 0.5 * lambda * Var[r]
CER:{[lambda;w;R;C]
    ret:annFactor * sum w * avg each R;
    pvar:annFactor * sum w * .kdbtools.mm[C;w];
    ret - 0.5 * lambda * pvar}

// Entropic risk measure: (1/theta) * log(E[exp(-theta * returns)])
entropicRisk:{[theta;portRets]
    (1 % theta) * log avg exp neg theta * portRets}

// Lower Partial Moment of order n below MAR
LPM:{[n;mar;rets]
    shortfall:mar - rets;
    avg (0f | shortfall) xexp n}

// Kappa ratio: (E[r] - MAR) / LPM(n)^(1/n)
kappa:{[n;mar;rets]
    excessRet:(avg rets) - mar;
    lpm:LPM[n;mar;rets];
    excessRet % lpm xexp 1 % n}

// -----------------------------------------------------------------------------
// MIN CDAR OPTIMIZER
// -----------------------------------------------------------------------------

// Minimize Conditional Drawdown at Risk
// alpha: confidence level (e.g., 0.05 for worst 5% of drawdowns)
minCDaR:{[R;alpha;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;

    // Generate random portfolios
    W:randWeightMatrix[nIter;n];

    // Compute CDaR for each portfolio
    cdars:{[alpha;R;w] CDaR[alpha;portCumRet[w;R]]}[alpha;R] each W;

    // Find minimum (least negative = best)
    bestIdx:cdars?max cdars;
    bestW:W bestIdx;

    `weights`cdar`sharpe`return`volatility`maxDD!(
        bestW;
        cdars bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        maxDD portCumRet[bestW;R])}

// Min CDaR with constraints
minCDaRConstrained:{[R;alpha;lo;hi;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    W:randWeightMatrixConstrained[nIter;n;loVec;hiVec];
    cdars:{[alpha;R;w] CDaR[alpha;portCumRet[w;R]]}[alpha;R] each W;

    bestIdx:cdars?max cdars;
    bestW:W bestIdx;

    `weights`cdar`sharpe`return`volatility`maxDD`bounds!(
        bestW;
        cdars bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        maxDD portCumRet[bestW;R];
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// MAX CER (CERTAINTY EQUIVALENT RETURN) OPTIMIZER
// -----------------------------------------------------------------------------

// Maximize Certainty Equivalent Return (VECTORIZED)
// lambda: risk aversion coefficient (higher = more risk averse)
// Typical values: 1-10, where 2-4 is moderate
maxCER:{[R;lambda;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    W:randWeightMatrix[nIter;n];
    cers:cersVec[W;mu;C;lambda];          // Vectorized!

    bestIdx:cers?max cers;
    bestW:W bestIdx;

    `weights`cer`sharpe`return`volatility`lambda!(
        bestW;
        cers bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * mu;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        lambda)}

// Max CER with gradient descent (analytical solution exists)
maxCERAnalytical:{[R;lambda]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Analytical: w* = (1/lambda) * C^-1 * mu (then normalize for long-only)
    // Approximate with gradient ascent
    w:n#1f%n;  // Equal weight start

    step:{[mu;C;lambda;w]
        // Gradient of CER: mu - lambda * C * w
        grad:mu - lambda * .kdbtools.mm[C;w];
        gNorm:grad % sqrt sum grad*grad;
        wNew:w + 0.01 * gNorm;
        wNew:0f | wNew;
        wNew % sum wNew};

    w:500 step[mu;C;lambda]/w;

    `weights`cer`sharpe`return`volatility`lambda!(
        w;
        CER[lambda;w;R;C];
        portSharpe[w;R;C;0f];
        annFactor * sum w * mu;
        sqrt[annFactor] * sqrt sum w * .kdbtools.mm[C;w];
        lambda)}

// Max CER with constraints (VECTORIZED)
maxCERConstrained:{[R;lambda;lo;hi;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;
    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    W:randWeightMatrixConstrained[nIter;n;loVec;hiVec];
    cers:cersVec[W;mu;C;lambda];          // Vectorized!

    bestIdx:cers?max cers;
    bestW:W bestIdx;

    `weights`cer`sharpe`return`volatility`lambda`bounds!(
        bestW;
        cers bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * mu;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        lambda;
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// MIN ENTROPIC RISK OPTIMIZER
// -----------------------------------------------------------------------------

// Minimize Entropic Risk Measure (VECTORIZED)
// theta: risk aversion parameter (higher = more risk averse to tail losses)
// Typical values: 1-20
minEntropicRisk:{[R;theta;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;

    W:randWeightMatrix[nIter;n];
    eRisks:entropicRisksVec[W;R;theta];   // Vectorized!

    // Minimize (find most negative = lowest risk)
    bestIdx:eRisks?min eRisks;
    bestW:W bestIdx;

    `weights`entropicRisk`sharpe`return`volatility`theta!(
        bestW;
        eRisks bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        theta)}

// Min Entropic Risk with constraints (VECTORIZED)
minEntropicRiskConstrained:{[R;theta;lo;hi;nIter]
    n:count R;
    C:.kdbtools.covmat flip R;
    loVec:$[0 > type lo; n#lo; lo];
    hiVec:$[0 > type hi; n#hi; hi];

    W:randWeightMatrixConstrained[nIter;n;loVec;hiVec];
    eRisks:entropicRisksVec[W;R;theta];   // Vectorized!

    bestIdx:eRisks?min eRisks;
    bestW:W bestIdx;

    `weights`entropicRisk`sharpe`return`volatility`theta`bounds!(
        bestW;
        eRisks bestIdx;
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        theta;
        `lo`hi!(loVec;hiVec))}

// -----------------------------------------------------------------------------
// MAX KAPPA (GENERALIZED SHARPE) OPTIMIZER
// -----------------------------------------------------------------------------

// Maximize Kappa ratio (generalized Sharpe using LPM) - VECTORIZED
// n: LPM order (1=shortfall prob, 2=semi-variance, 3=skewness-aware)
// mar: Minimum Acceptable Return (daily, e.g., 0 or rf/252)
maxKappa:{[R;order;mar;nIter]
    nAssets:count R;
    C:.kdbtools.covmat flip R;

    W:randWeightMatrix[nIter;nAssets];
    kappas:kappasVec[W;R;order;mar];      // Vectorized!
    // Handle infinities (replace with large negative for argmax)
    kappas:@[kappas;where kappas=0w;:;neg 0w];

    // Find maximum
    bestIdx:kappas?max kappas;
    bestW:W bestIdx;

    portRets:sum bestW * R;

    `weights`kappa`lpm`sharpe`return`volatility`order`mar!(
        bestW;
        kappas bestIdx;
        LPM[order;mar;portRets];
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        order;
        mar)}

// Max Kappa with constraints (VECTORIZED)
maxKappaConstrained:{[R;order;mar;lo;hi;nIter]
    nAssets:count R;
    C:.kdbtools.covmat flip R;
    loVec:$[0 > type lo; nAssets#lo; lo];
    hiVec:$[0 > type hi; nAssets#hi; hi];

    W:randWeightMatrixConstrained[nIter;nAssets;loVec;hiVec];
    kappas:kappasVec[W;R;order;mar];      // Vectorized!
    kappas:@[kappas;where kappas=0w;:;neg 0w];

    bestIdx:kappas?max kappas;
    bestW:W bestIdx;

    portRets:sum bestW * R;

    `weights`kappa`lpm`sharpe`return`volatility`order`mar`bounds!(
        bestW;
        kappas bestIdx;
        LPM[order;mar;portRets];
        portSharpe[bestW;R;C;0f];
        annFactor * sum bestW * avg each R;
        sqrt[annFactor] * sqrt sum bestW * .kdbtools.mm[C;bestW];
        order;
        mar;
        `lo`hi!(loVec;hiVec))}

// Sortino ratio is Kappa with order=2, mar=0
maxSortino:{[R;nIter] maxKappa[R;2;0f;nIter]}
maxSortinoConstrained:{[R;lo;hi;nIter] maxKappaConstrained[R;2;0f;lo;hi;nIter]}

// -----------------------------------------------------------------------------
// EXAMPLE AND DOCUMENTATION
// -----------------------------------------------------------------------------

// Example function demonstrating all optimizer functions
// Run: .optimizer.example[]
example:{[]
    -1 "";
    -1 "=== PORTFOLIO OPTIMIZER EXAMPLE ===";
    -1 "";

    // Generate sample data: 5 assets, 252 trading days
    -1 "1. DATA SETUP";
    -1 "   R: matrix where each row is one asset's return series";
    -1 "   Shape: n_assets x n_periods (e.g., 5 x 252)";
    -1 "";

    system "S 42";  // Set seed for reproducibility
    n:5; T:252;

    // Simulate returns with different characteristics
    dailyMu:0.0001 * 3 2 1 2.5 1.5;           // Daily expected returns
    dailyVol:0.01 * 1.5 1.2 0.5 1.8 0.8;      // Daily volatilities
    R:{[mu;vol;T] mu + vol * (T?1f) - 0.5}'[dailyMu;dailyVol;T];

    rf:0.02;  // 2% annual risk-free rate

    -1 "   Generated: ",string[n]," assets, ",string[T]," periods";
    -1 "   Risk-free rate: ",string[rf]," (annual)";
    -1 "";

    // Show asset characteristics
    -1 "   Asset characteristics (annualized):";
    {-1 "     Asset ",string[x],": return=",string[252*avg y],", vol=",string sqrt[252]*dev y}'[til n;R];
    -1 "";

    // ---------------------------------------------------------------------
    -1 "2. OPTIMIZATION FUNCTIONS";
    -1 "";

    // maxSharpe - Unconstrained max Sharpe via random search
    -1 "   maxSharpe[R;rf;nIter]";
    -1 "   - R: return matrix (n_assets x n_periods)";
    -1 "   - rf: annual risk-free rate";
    -1 "   - nIter: number of random portfolios to try";
    r1:maxSharpe[R;rf;10000];
    -1 "   Result: Sharpe=",string[r1`sharpe],", weights=",-3!r1`weights;
    -1 "";

    // maxSharpeConstrained - With weight limits
    -1 "   maxSharpeConstrained[R;rf;lo;hi;nIter]";
    -1 "   - lo: minimum weight per asset (scalar or vector)";
    -1 "   - hi: maximum weight per asset (scalar or vector)";
    r2:maxSharpeConstrained[R;rf;0.05;0.4;10000];
    -1 "   Result: Sharpe=",string[r2`sharpe],", weights=",-3!r2`weights;
    -1 "";

    // maxSharpeHillClimb - Gradient-based optimization
    -1 "   maxSharpeHillClimb[R;rf;nIter;lr]";
    -1 "   - nIter: number of gradient steps";
    -1 "   - lr: learning rate (try 0.01 to 0.05)";
    r3:maxSharpeHillClimb[R;rf;500;0.02];
    -1 "   Result: Sharpe=",string[r3`sharpe],", weights=",-3!r3`weights;
    -1 "";

    // maxSharpeHybrid - Random search + gradient refinement
    -1 "   maxSharpeHybrid[R;rf;nRandom;nClimb;lr]";
    -1 "   - nRandom: random search iterations";
    -1 "   - nClimb: gradient refinement steps";
    r4:maxSharpeHybrid[R;rf;5000;200;0.02];
    -1 "   Result: Sharpe=",string[r4`sharpe],", weights=",-3!r4`weights;
    -1 "";

    // maxMedianSharpe - Robust to outliers
    -1 "   maxMedianSharpe[R;rf;nIter]";
    -1 "   - Uses median returns instead of mean (more robust)";
    r5:maxMedianSharpe[R;rf;10000];
    -1 "   Result: Sharpe=",string[r5`sharpe],", weights=",-3!r5`weights;
    -1 "";

    // riskParity - Equal risk contribution
    -1 "   riskParity[R;nIter;lr]";
    -1 "   - Weights inversely proportional to volatility";
    r6:riskParity[R;100;0.01];
    -1 "   Result: Sharpe=",string[r6`sharpe],", weights=",-3!r6`weights;
    -1 "";

    // minVariance - Minimum volatility portfolio
    -1 "   minVariance[R;nIter]";
    -1 "   - Minimizes portfolio variance (long-only)";
    r7:minVariance[R;500];
    -1 "   Result: Sharpe=",string[r7`sharpe],", vol=",string[r7`volatility];
    -1 "            weights=",-3!r7`weights;
    -1 "";

    // ---------------------------------------------------------------------
    -1 "3. ADVANCED RISK OPTIMIZERS";
    -1 "";

    // minCDaR - Minimize Conditional Drawdown at Risk
    -1 "   minCDaR[R;alpha;nIter]";
    -1 "   - alpha: confidence level (e.g., 0.05 = worst 5% of drawdowns)";
    -1 "   - Minimizes expected drawdown in tail scenarios";
    r8:minCDaR[R;0.05;10000];
    -1 "   Result: CDaR=",string[r8`cdar],", MaxDD=",string[r8`maxDD];
    -1 "            Sharpe=",string[r8`sharpe];
    -1 "";

    // maxCER - Maximize Certainty Equivalent Return
    -1 "   maxCER[R;lambda;nIter]";
    -1 "   - lambda: risk aversion (1-10, higher=more conservative)";
    -1 "   - Balances return vs variance based on risk preference";
    r9:maxCER[R;3;10000];
    -1 "   Result: CER=",string[r9`cer],", Sharpe=",string[r9`sharpe];
    -1 "";

    // maxCERAnalytical - Faster gradient-based CER
    -1 "   maxCERAnalytical[R;lambda]";
    -1 "   - Gradient descent (faster, no nIter needed)";
    r10:maxCERAnalytical[R;3];
    -1 "   Result: CER=",string[r10`cer],", Sharpe=",string[r10`sharpe];
    -1 "";

    // minEntropicRisk - Minimize Entropic Risk
    -1 "   minEntropicRisk[R;theta;nIter]";
    -1 "   - theta: tail aversion (1-20, higher=more tail-averse)";
    -1 "   - Penalizes large losses exponentially";
    r11:minEntropicRisk[R;10;10000];
    -1 "   Result: EntropicRisk=",string[r11`entropicRisk],", Sharpe=",string[r11`sharpe];
    -1 "";

    // maxKappa - Maximize Kappa (generalized Sharpe)
    -1 "   maxKappa[R;order;mar;nIter]";
    -1 "   - order: LPM order (2=semi-variance, 3=skewness-aware)";
    -1 "   - mar: minimum acceptable return (e.g., 0 or rf/252)";
    r12:maxKappa[R;2;0f;10000];
    -1 "   Result: Kappa=",string[r12`kappa],", LPM=",string[r12`lpm];
    -1 "            Sharpe=",string[r12`sharpe];
    -1 "";

    // maxSortino - Maximize Sortino ratio
    -1 "   maxSortino[R;nIter]";
    -1 "   - Sortino = Kappa with order=2, mar=0";
    -1 "   - Only penalizes downside volatility";
    r13:maxSortino[R;10000];
    -1 "   Result: Sortino=",string[r13`kappa],", Sharpe=",string[r13`sharpe];
    -1 "";

    // Constrained example
    -1 "   Constrained variants (add lo;hi before nIter):";
    -1 "   minCDaRConstrained, maxCERConstrained, minEntropicRiskConstrained";
    -1 "   maxKappaConstrained, maxSortinoConstrained";
    r14:maxSortinoConstrained[R;0.05;0.4;10000];
    -1 "   maxSortinoConstrained[R;0.05;0.4;10000]: Sortino=",string[r14`kappa];
    -1 "";

    // ---------------------------------------------------------------------
    -1 "4. RESULT DICTIONARY KEYS";
    -1 "";
    -1 "   All functions return a dictionary with:";
    -1 "   - weights: portfolio weights (sum to 1)";
    -1 "   - sharpe: annualized Sharpe ratio";
    -1 "   - return: annualized expected return";
    -1 "   - volatility: annualized volatility";
    -1 "   - bounds: (constrained only) lo/hi weight limits";
    -1 "   Advanced optimizers also return their objective (cdar, cer, etc.)";
    -1 "";

    // ---------------------------------------------------------------------
    -1 "5. RECOMMENDED USAGE";
    -1 "";
    -1 "   For diversified portfolios:";
    -1 "     .optimizer.maxSharpeConstrained[R;rf;0.05;0.3;20000]";
    -1 "";
    -1 "   For robust optimization:";
    -1 "     .optimizer.maxSharpeHybridConstrained[R;rf;0.05;0.3;10000;500;0.02]";
    -1 "";
    -1 "   For drawdown control:";
    -1 "     .optimizer.minCDaRConstrained[R;0.05;0.05;0.3;10000]";
    -1 "";
    -1 "   For downside risk focus (Sortino):";
    -1 "     .optimizer.maxSortinoConstrained[R;0.05;0.3;10000]";
    -1 "";
    -1 "   For tail risk aversion:";
    -1 "     .optimizer.minEntropicRiskConstrained[R;10;0.05;0.3;10000]";
    -1 "";
    -1 "   For mean-variance with risk aversion:";
    -1 "     .optimizer.maxCERAnalytical[R;3]  // fast gradient-based";
    -1 "";

    // ---------------------------------------------------------------------
    -1 "6. WORKING WITH TABLES";
    -1 "";
    -1 "   Most data comes as tables. Use these helpers to convert:";
    -1 "";

    // Create sample table
    t:([] date:2024.01.01+til 100; AAPL:100?0.02-0.01; GOOG:100?0.02-0.01; MSFT:100?0.02-0.01);
    -1 "   Sample table:";
    -1 "   t:([] date:dates; AAPL:rets; GOOG:rets; MSFT:rets)";
    -1 "";

    -1 "   METHOD 1: Manual conversion (most control)";
    -1 "   ------------------------------------------";
    -1 "   // Convert table to matrix (exclude date column)";
    -1 "   R:.optimizer.fromTableEx[enlist`date;t]";
    Rt:fromTableEx[enlist`date;t];
    -1 "   // Shape: ",string[count Rt]," assets x ",string[count first Rt]," periods";
    -1 "";
    -1 "   // Optimize";
    -1 "   result:.optimizer.maxSharpeConstrained[R;0.02;0.1;0.5;10000]";
    rt1:maxSharpeConstrained[Rt;0.02;0.1;0.5;10000];
    -1 "";
    -1 "   // Add asset labels to weights";
    -1 "   result:.optimizer.labelWeights[enlist`date;t;result]";
    rt1:labelWeights[enlist`date;t;rt1];
    -1 "   result`weightsByAsset:";
    -1 "     ",raze{string[x],"| ",string[y],"  "}'[key rt1`weightsByAsset;value rt1`weightsByAsset];
    -1 "";

    -1 "   METHOD 2: All-in-one wrapper";
    -1 "   -----------------------------";
    -1 "   result:.optimizer.optimizeTable[t;enlist`date;rf;method;args]";
    -1 "";
    -1 "   Methods and args:";
    -1 "     `maxSharpe         enlist nIter              e.g. enlist 10000";
    -1 "     `constrained       (lo;hi;nIter)             e.g. (0.05;0.4;10000)";
    -1 "     `slsqp             (lo;hi;nIter)             e.g. (0.05;0.4;500)";
    -1 "     `hillClimb         (nIter;lr)                e.g. (500;0.02)";
    -1 "     `hybrid            (nRandom;nClimb;lr)       e.g. (5000;200;0.02)";
    -1 "     `median            enlist nIter              e.g. enlist 10000";
    -1 "     `riskParity        (nIter;lr)                e.g. (100;0.01)";
    -1 "     `minVariance       enlist nIter              e.g. enlist 500";
    -1 "     `minCDaR           (alpha;nIter)             e.g. (0.05;5000)";
    -1 "     `minCDaRConstr     (alpha;lo;hi;nIter)       e.g. (0.05;0.05;0.3;5000)";
    -1 "     `maxCER            (lambda;nIter)            e.g. (2;5000)";
    -1 "     `maxCERAnalytical  enlist lambda             e.g. enlist 2";
    -1 "     `maxCERConstr      (lambda;lo;hi;nIter)      e.g. (2;0.05;0.3;5000)";
    -1 "     `minEntropic       (theta;nIter)             e.g. (5;5000)";
    -1 "     `minEntropicConstr (theta;lo;hi;nIter)       e.g. (5;0.05;0.3;5000)";
    -1 "     `maxKappa          (order;mar;nIter)         e.g. (2;0f;5000)";
    -1 "     `maxKappaConstr    (order;mar;lo;hi;nIter)   e.g. (2;0f;0.05;0.3;5000)";
    -1 "     `maxSortino        enlist nIter              e.g. enlist 5000";
    -1 "     `maxSortinoConstr  (lo;hi;nIter)             e.g. (0.05;0.3;5000)";
    -1 "";

    -1 "   Example:";
    -1 "   result:.optimizer.optimizeTable[t;enlist`date;0.02;`constrained;(0.1;0.5;10000)]";
    rt2:optimizeTable[t;enlist`date;0.02;`constrained;(0.1;0.5;10000)];
    -1 "   result`assets:";
    -1 "     ",raze{string[x],"| ",string[y],"  "}'[key rt2`assets;value rt2`assets];
    -1 "";

    -1 "=== END EXAMPLE ===";
    -1 "";

    // Return summary table
    ([] func:`maxSharpe`maxSharpeConstrained`maxSharpeHillClimb`maxSharpeHybrid`maxMedianSharpe`riskParity`minVariance`minCDaR`maxCER`maxCERAnalytical`minEntropicRisk`maxKappa`maxSortino;
       sharpe:r1[`sharpe],r2[`sharpe],r3[`sharpe],r4[`sharpe],r5[`sharpe],r6[`sharpe],r7[`sharpe],r8[`sharpe],r9[`sharpe],r10[`sharpe],r11[`sharpe],r12[`sharpe],r13[`sharpe])}

// -----------------------------------------------------------------------------
// ROBUST OPTIMIZER (handles sparse, non-normal alphas)
// -----------------------------------------------------------------------------

// Symmetric winsorize at percentile bounds (e.g., 0.05 = clip at 5th and 95th)
pctWinsorize:{[pct;x] v:x where not null x; lo:v (iasc v) `long$pct*n:count v; hi:v (iasc v) `long$(1-pct)*n; lo|x&hi}

// Rank normalize to 0-1
rankNorm:{[x] v:x; nonnull:where not null v; v[nonnull]:(iasc iasc v nonnull) % count nonnull; v}

// Fill rate (fraction non-null)
fillRate:{1 - (sum null x) % count x}

// Pairwise correlation (only overlapping non-null)
pairwiseCor:{[x;y] valid:where not null[x] & not null y; $[2>count valid;0f;cor[x valid;y valid]]}

// Spearman correlation (rank-based, robust to outliers)
spearmanCor:{[x;y] valid:where not null[x] & not null y; $[2>count valid;0f;cor[iasc iasc x valid;iasc iasc y valid]]}

// Build correlation matrix using pairwise overlapping periods
pairwiseCorMat:{[R] n:count R; raze {[R;n;i] {[R;i;j] $[i=j;1f;pairwiseCor[R i;R j]]}[R;i] each til n}[R;n] each til n}

// Build Spearman correlation matrix (returns n x n matrix)
spearmanCorMat:{[R] n:count R; (n;n)#raze {[R;i;n] {[R;i;j] $[i=j;1f;spearmanCor[R i;R j]]}[R;i] each til n}[R;;n] each til n}

// Shrink correlation toward identity (reduces estimation error)
shrinkCor:{[shrinkage;corMat] n:count corMat; I:{x=y}'[til n;til n]; ((1-shrinkage)*corMat) + shrinkage*I}

// MAD-based volatility (robust to outliers)
madVol:{[x] v:x where not null x; 1.4826 * med abs v - med v}

// Robust covariance from Spearman cor + MAD-based vol
robustCov:{[R;shrinkage] vols:madVol each R; corMat:spearmanCorMat R; shrunkenCor:shrinkCor[shrinkage;corMat]; vols *\: vols * shrunkenCor}

// Robust optimizer for alpha tables
// t: table with alpha columns (and optional non-alpha columns to exclude)
// exclude: list of column names to exclude (e.g., `date`sym)
// params: dictionary with optional settings
//   `minFill    - minimum fill rate to include alpha (default 0.5)
//   `winsorPct  - winsorization percentile (default 0.05)
//   `rankNorm   - rank normalize signals (default 1b)
//   `shrinkage  - correlation shrinkage (default 0.2)
//   `method     - optimization method: `sharpe`sortino`riskParity`minVar (default `sortino)
//   `lo         - min weight (default 0.0)
//   `hi         - max weight (default 1.0)
//   `nIter      - iterations (default 5000)
robustOptimize:{[t;exclude;params]
    defaults:`minFill`winsorPct`rankNorm`shrinkage`method`lo`hi`nIter`annualize!(0.5;0.05;0b;0.2;`sortino;0f;1f;5000;1b);
    p:defaults,params;
    allCols:cols t;
    alphaCols:allCols except exclude;
    alphas:t alphaCols;
    nAlphas:count alphas;
    alphaNames:alphaCols;
    fr:fillRate each alphas;
    validIdx:where fr >= p`minFill;
    if[0=count validIdx; '"No alphas meet minimum fill rate"];
    R:alphas validIdx;
    validNames:alphaNames validIdx;
    R:{0f^fills x} each R;
    R:pctWinsorize[p`winsorPct] each R;
    R:$[p`rankNorm; rankNorm each R; R];
    loVec:$[0>type p`lo; (count R)#p`lo; p`lo];
    hiVec:$[0>type p`hi; (count R)#p`hi; p`hi];
    lo1:first loVec; hi1:first hiVec;
    result:$[p[`method]~`sharpe;maxSharpeConstrained[R;0f;loVec;hiVec;p`nIter];p[`method]~`sortino;maxSortinoConstrained[R;loVec;hiVec;p`nIter];p[`method]~`riskParity;riskParity[R;p`nIter;0.01];p[`method]~`minVar;minVariance[R;p`nIter];p[`method]~`cdar;minCDaRConstrained[R;0.05;lo1;hi1;p`nIter];maxSortinoConstrained[R;loVec;hiVec;p`nIter]];
    fullWeights:nAlphas#0f;
    fullWeights[validIdx]:result`weights;
    weightsByAlpha:alphaNames!fullWeights;
    result:$[p`annualize;result;@[;`sharpe;%[;sqrt 252f]]@[;`return;%[;252f]]@[;`volatility;%[;sqrt 252f]] result];
    result,`weightsByAlpha`validAlphas`excludedAlphas`fillRates`params!(weightsByAlpha;validNames;alphaNames except validNames;alphaNames!fr;p)}

// Simplified interface for common case
robustOptimizeSimple:{[t;exclude]
    robustOptimize[t;exclude;()!()]}

// Show available functions
help:{[]
    -1 "";
    -1 "=== .optimizer FUNCTIONS ===";
    -1 "";
    -1 "TABLE HELPERS (convert tables to matrix format):";
    -1 "  fromTable[t]                       - Table to matrix (all columns)";
    -1 "  fromTableEx[`date;t]               - Exclude columns, then convert";
    -1 "  fromLongTable[`sym;`ret;t]         - Long format (sym,ret) to matrix";
    -1 "  optimizeTable[t;`date;rf;`maxSharpe;enlist 10000]  - Direct from table";
    -1 "";
    -1 "SHARPE MAXIMIZATION:";
    -1 "  maxSharpe[R;rf;nIter]              - Random search";
    -1 "  maxSharpeConstrained[R;rf;lo;hi;n] - With weight bounds";
    -1 "  maxSharpeSLSQP[R;rf;lo;hi;nIter]   - Gradient-based (like scipy)";
    -1 "  maxSharpeHillClimb[R;rf;nIter;lr]  - Gradient descent";
    -1 "  maxSharpeHybrid[R;rf;nR;nC;lr]     - Random + gradient";
    -1 "  maxMedianSharpe[R;rf;nIter]        - Median-based (robust)";
    -1 "";
    -1 "RISK-BASED:";
    -1 "  riskParity[R;nIter;lr]             - Equal risk contribution";
    -1 "  minVariance[R;nIter]               - Minimum volatility";
    -1 "";
    -1 "ADVANCED RISK OPTIMIZERS:";
    -1 "  minCDaR[R;alpha;nIter]             - Min Conditional Drawdown at Risk";
    -1 "  maxCER[R;lambda;nIter]             - Max Certainty Equivalent Return";
    -1 "  maxCERAnalytical[R;lambda]         - CER via gradient (faster)";
    -1 "  minEntropicRisk[R;theta;nIter]     - Min Entropic Risk Measure";
    -1 "  maxKappa[R;order;mar;nIter]        - Max Kappa (generalized Sharpe)";
    -1 "  maxSortino[R;nIter]                - Max Sortino (Kappa order=2, mar=0)";
    -1 "";
    -1 "CONSTRAINED VARIANTS (add lo;hi before nIter):";
    -1 "  minCDaRConstrained, maxCERConstrained, minEntropicRiskConstrained";
    -1 "  maxKappaConstrained, maxSortinoConstrained";
    -1 "  maxSharpeHillClimbConstrained, maxSharpeHybridConstrained";
    -1 "";
    -1 "RISK MEASURE HELPERS:";
    -1 "  drawdowns[cumRets]                 - Drawdown series";
    -1 "  maxDD[cumRets]                     - Maximum drawdown";
    -1 "  CDaR[alpha;cumRets]                - Conditional Drawdown at Risk";
    -1 "  CER[lambda;w;R;C]                  - Certainty Equivalent Return";
    -1 "  entropicRisk[theta;portRets]       - Entropic risk measure";
    -1 "  LPM[n;mar;rets]                    - Lower Partial Moment";
    -1 "  kappa[n;mar;rets]                  - Kappa ratio";
    -1 "";
    -1 "ROBUST OPTIMIZER (handles sparse, non-normal alphas):";
    -1 "  robustOptimize[t;exclude;params]   - Full robust pipeline";
    -1 "  robustOptimizeSimple[t;exclude]    - With default params";
    -1 "  alphaWeightProtocol[t;cfg]         - Walk-forward compare static/exp/roll/blend";
    -1 "";
    -1 "  params dictionary keys:";
    -1 "    `minFill    - min fill rate (default 0.5)";
    -1 "    `winsorPct  - winsorization pct (default 0.05)";
    -1 "    `rankNorm   - rank normalize (default 0b, use 1b for signals not returns)";
    -1 "    `shrinkage  - correlation shrinkage (default 0.2, not yet used)";
    -1 "    `method     - `sharpe`sortino`riskParity`minVar`cdar (minVar ignores lo/hi)";
    -1 "    `lo`hi      - weight bounds (default 0,1)";
    -1 "    `nIter      - iterations (default 5000)";
    -1 "    `annualize  - annualize metrics (default 1b, set 0b for dollar returns)";
    -1 "";
    -1 "UTILITIES:";
    -1 "  example[]                          - Run example with sample data";
    -1 "  help[]                             - Show this help";
    -1 "  alphaWeightProtocol output keys: `summary`best`series`weights";
    -1 "";
    -1 "PARAMETERS:";
    -1 "  R      - Return matrix: n_assets rows x n_periods columns";
    -1 "  rf     - Annual risk-free rate (e.g., 0.02 for 2%)";
    -1 "  nIter  - Number of iterations";
    -1 "  lr     - Learning rate for gradient descent (0.01-0.05)";
    -1 "  lo/hi  - Min/max weight bounds (scalar or vector)";
    -1 "  alpha  - CDaR confidence level (e.g., 0.05 for worst 5%)";
    -1 "  lambda - Risk aversion for CER (1-10, higher=more conservative)";
    -1 "  theta  - Entropic risk aversion (1-20, higher=more tail-averse)";
    -1 "  order  - LPM order (2=semi-variance, 3=skewness-aware)";
    -1 "  mar    - Minimum acceptable return (e.g., 0 or rf/252)";
    -1 "";
    -1 "Run .optimizer.example[] for a complete demonstration.";
    -1 "";}

// =============================================================================
// ALPHA PORTFOLIO OPTIMIZER
// =============================================================================
// Optimally blend multiple alpha signals.
// Maximizes Sharpe while penalizing PORTFOLIO-LEVEL turnover and alpha correlation.
// Portfolio turnover = avg_t Σ_j |Σ_i w_i × Δsig_i(t,j)| — captures netting across alphas.
//
// Inputs:
//   alphas  - list of tables, each: ([] time; ricRoot; sig; prevSig)
//   returns - table: ([] time; ricRoot; pxDiff)
//
// Objective: maximize Sharpe(Rw) - lamCorr*w'Corrw - lamTO*portfolioTO(w)
// =============================================================================

// Convert list of alpha signal tables + returns table → alpha returns matrix
// Each alpha's return at time t = Σ_j prevSig(t,j) × pxDiff(t,j) across ricRoot
// Returns: table ([] time; a0; a1; ... aK)
alphaReturns:{[alphas;returns]
    // Compute P&L per alpha: join with returns, multiply prevSig × pxDiff, sum by time
    pnls:{[ret;alpha]
        // Join on time and ricRoot
        j:alpha lj `time`ricRoot xkey ret;
        // P&L per row
        j:update pnl:prevSig * pxDiff from j;
        // Sum across ricRoot per time
        exec sum pnl by time from j
    }[returns;] each alphas;
    // Get union of all times
    times:asc distinct raze key each pnls;
    // Build matrix: one column per alpha
    mat:flip (`$"a",/:string til count alphas)!(pnls @\: times);
    // Fill nulls with 0 (missing = no position = no P&L)
    mat:0f^mat;
    ([] time:times),'mat}

// Build aligned delta matrix for portfolio-level turnover
// Returns dict: `deltas (K × N matrix), `timeIdx (N-vector), `nTimes (scalar)
// where N = total (time,ricRoot) observations aligned across all alphas
alphaDeltas:{[alphas]
    K:count alphas;
    // Compute signal changes per alpha: (time;ricRoot) -> sig - prevSig
    dTables:{update delta:sig - prevSig from x} each alphas;
    // Get union of all (time,ricRoot) pairs
    allKeys:asc distinct raze {exec (time,'ricRoot) from x} each dTables;
    // Build lookup per alpha
    lookups:{exec ((time,'ricRoot))!delta from x} each dTables;
    // Align: K × N matrix (fill missing with 0)
    dMat:0f^ lookups @\: allKeys;
    // Time index: map each element to its time
    timeVals:allKeys[;0];
    uTimes:asc distinct timeVals;
    timeIdx:uTimes?timeVals;
    nTimes:count uTimes;
    `dMat`timeIdx`nTimes!(dMat;timeIdx;nTimes)}

// Portfolio-level turnover for given weights
// TO = avg_t Σ_j |Σ_i w_i × delta_i(t,j)|
portfolioTO:{[deltaCtx;w]
    // Net position change per (time,ricRoot): Σ_i w_i × delta_i
    net:sum w * deltaCtx`dMat;
    // Sum |net| per time, then average
    absNet:abs net;
    toByTime:(deltaCtx[`nTimes]#0f);
    toByTime:@[toByTime;deltaCtx`timeIdx;+;absNet];
    avg toByTime}

// Portfolio-level turnover gradient (subgradient via sign)
// d/dw_k TO = avg_t Σ_j sign(net_j(t)) × delta_k(t,j)
portfolioTOGrad:{[deltaCtx;w]
    // Net position change
    net:sum w * deltaCtx`dMat;
    signs:signum net;
    // For each alpha k: sum(sign * delta_k) grouped by time, averaged
    K:count deltaCtx`dMat;
    nT:deltaCtx`nTimes;
    tidx:deltaCtx`timeIdx;
    {[dM;signs;tidx;nT;k]
        contrib:signs * dM k;
        byTime:nT#0f;
        byTime:@[byTime;tidx;+;contrib];
        avg byTime
    }[deltaCtx`dMat;signs;tidx;nT] each til K}

// Per-alpha turnover (for reporting, not used in optimization)
alphaTurnover:{[alphas]
    {[alpha]
        to:exec sum abs sig - prevSig by time from alpha;
        avg to
    } each alphas}

// Objective: Sharpe × (1 - lamCorr × w'Corrw) - lamTO × portfolioTO
// Correlation penalty is multiplicative on Sharpe — scale-invariant:
//   lamCorr=0.3 means "sacrifice up to 30% of Sharpe for diversification"
// Turnover penalty is additive on annualized Sharpe:
//   lamTO=0.5 means "sacrifice 0.5 ann. Sharpe per unit of portfolio turnover"
alphaGrad:{[p;w]
    // Annualized Sharpe and its gradient
    af:p`annFactor;
    sharpe:sqrt[af] * sharpeRaw[p`mu;p`C;p`rf;w];
    sg:sqrt[af] * sharpeGradAnalytical[p`mu;p`C;p`rf;w];
    // Correlation concentration: w'Corr*w
    corrConc:sum w * .kdbtools.mm[p`Corr;w];
    // d/dw [Sharpe × (1 - lamCorr × w'Corrw)]
    //   = gradSharpe × (1 - lamCorr × w'Corrw) - Sharpe × lamCorr × 2×Corr×w
    corrFactor:1 - p[`lamCorr] * corrConc;
    corrGrad:sharpe * p[`lamCorr] * 2 * .kdbtools.mm[p`Corr;w];
    // Portfolio turnover gradient (subgradient)
    toGrad:p[`lamTO] * portfolioTOGrad[p`deltaCtx;w];
    // Combined
    grad:(sg * corrFactor) - corrGrad - toGrad;
    // CVaR penalty subgradient (negative CVaR reduces objective)
    if[p[`lamCVaR] > 0; grad+:p[`lamCVaR] * portCVaRGrad[p`cvarAlpha;w;p`R]];
    // CDaR penalty subgradient (negative CDaR reduces objective)
    if[p[`lamDD] > 0; grad+:p[`lamDD] * portCDaRGrad[p`cdarAlpha;w;p`R]];
    grad}

alphaObj:{[p;w]
    af:p`annFactor;
    sharpe:sqrt[af] * sharpeRaw[p`mu;p`C;p`rf;w];
    corrConc:sum w * .kdbtools.mm[p`Corr;w];
    toPen:p[`lamTO] * portfolioTO[p`deltaCtx;w];
    obj:(sharpe * 1 - p[`lamCorr] * corrConc) - toPen;
    // CVaR penalty (portCVaR returns negative for risky portfolios → reduces obj)
    if[p[`lamCVaR] > 0; obj+:p[`lamCVaR] * portCVaR[p`cvarAlpha;w;p`R]];
    // CDaR penalty (CDaR returns negative for risky portfolios → reduces obj)
    if[p[`lamDD] > 0; obj+:p[`lamDD] * CDaR[p`cdarAlpha; portCumRet[w;p`R]]];
    obj}

// Backtracking line search for alpha objective (maximization formulated as minimization)
// p: params dict, dir: descent direction for -objective, dirDeriv: grad·dir (negative)
// Returns (wNew;objNew) where obj = neg alphaObj
alphaLineSearch:{[p;lo;hi;wCur;objCur;dir;dirDeriv]
    alpha:1f;
    wNew:projectConstrained[lo;hi;wCur + alpha * dir];
    objNew:neg alphaObj[p;wNew];
    cnt:0;
    while[(objNew > objCur + 1e-4 * alpha * dirDeriv) and (alpha > 1e-12) and (cnt < 30);
        alpha:0.5 * alpha;
        wNew:projectConstrained[lo;hi;wCur + alpha * dir];
        objNew:neg alphaObj[p;wNew];
        cnt+:1];
    (wNew;objNew)}

// One-shot alpha optimization
// alphas: list of signal tables ([] time; ricRoot; sig; prevSig)
// returns: ([] time; ricRoot; pxDiff)
// cfg: dict with optional keys:
//   rf        - annual risk-free rate (default 0)
//   lamCorr   - correlation penalty (default 0.1)
//   lamTO     - turnover penalty (default 0.1)
//   lamCVaR   - CVaR tail risk penalty (default 0; 0 = disabled)
//   lamDD     - CDaR drawdown penalty (default 0; 0 = disabled)
//   cvarAlpha - CVaR tail probability (default 0.05, worst 5%)
//   cdarAlpha - CDaR tail probability (default 0.05, worst 5%)
//   lo        - min weight (default 0)
//   hi        - max weight (default 1)
//   nIter     - gradient steps (default 500)
//   period    - function mapping time→period for return aggregation (default: none)
//               e.g. {"d"$x} to aggregate intraday→daily, {`week$x} for weekly
//   annFactor - annualization factor (default 252; set to 52 for weekly, etc.)
optimizeAlphas:{[alphas;returns;cfg]
    // Defaults
    rf:$[`rf in key cfg; cfg`rf; 0f];
    lamCorr:$[`lamCorr in key cfg; cfg`lamCorr; 0.1];
    lamTO:$[`lamTO in key cfg; cfg`lamTO; 0.1];
    lamCVaR:$[`lamCVaR in key cfg; cfg`lamCVaR; 0f];
    lamDD:$[`lamDD in key cfg; cfg`lamDD; 0f];
    cvarAlpha:$[`cvarAlpha in key cfg; cfg`cvarAlpha; 0.05];
    cdarAlpha:$[`cdarAlpha in key cfg; cfg`cdarAlpha; 0.05];
    nIter:$[`nIter in key cfg; cfg`nIter; 500];
    af:$[`annFactor in key cfg; cfg`annFactor; annFactor];
    hasPeriod:`period in key cfg;

    // Compute alpha returns matrix
    arTable:alphaReturns[alphas;returns];

    // Period aggregation: sum alpha returns within each period for Sharpe computation
    // Turnover penalty still uses full-resolution signal deltas (deltaCtx below)
    if[hasPeriod;
        periodFn:cfg`period;
        aCols:cols[arTable] except `time;
        aggCols:aCols!({(sum;x)} each aCols);
        arTable:?[arTable; (); enlist[`time]!enlist (periodFn;`time); aggCols]];

    R:fromTable delete time from arTable;
    n:count R;

    lo:$[`lo in key cfg; cfg`lo; n#0f];
    hi:$[`hi in key cfg; cfg`hi; n#1f];
    lo:$[0>type lo; n#lo; lo];
    hi:$[0>type hi; n#hi; hi];

    // Build delta matrix for portfolio-level turnover (full resolution, not aggregated)
    deltaCtx:alphaDeltas alphas;

    // Run optimization
    opts:`lo`hi`nIter`af`lamCVaR`lamDD`cvarAlpha`cdarAlpha!(lo;hi;nIter;af;lamCVaR;lamDD;cvarAlpha;cdarAlpha);
    optimizeAlphasCore[R;deltaCtx;rf;lamCorr;lamTO;opts]}

// Core optimizer (works on pre-computed data)
// R: K rows × T cols (one row per alpha)
// deltaCtx: from alphaDeltas — aligned signal change matrix
// opts: `lo`hi`nIter`af dict
optimizeAlphasCore:{[R;deltaCtx;rf;lamCorr;lamTO;opts]
    lo:opts`lo; hi:opts`hi; nIter:opts`nIter; af:opts`af;
    n:count R;
    C:.kdbtools.covmat flip R;
    mu:avg each R;

    // Correlation matrix (scale-invariant diversification penalty)
    vols:sqrt C @' til n;
    Corr:C % vols *\: vols;
    zeroIdx:where 0=vols;
    if[0 < count zeroIdx; Corr:@[Corr;zeroIdx;:;(count[zeroIdx])#enlist n#0f]];

    // Tail risk penalty parameters
    lamCVaR:$[`lamCVaR in key opts; opts`lamCVaR; 0f];
    lamDD:$[`lamDD in key opts; opts`lamDD; 0f];
    cvarAlpha:$[`cvarAlpha in key opts; opts`cvarAlpha; 0.05];
    cdarAlpha:$[`cdarAlpha in key opts; opts`cdarAlpha; 0.05];

    // Pack params (includes annFactor for objective/gradient functions)
    p:`mu`C`Corr`rf`deltaCtx`lamCorr`lamTO`lamCVaR`lamDD`cvarAlpha`cdarAlpha`annFactor!(
        mu;C;Corr;rf;deltaCtx;lamCorr;lamTO;lamCVaR;lamDD;cvarAlpha;cdarAlpha;af);
    // Attach R matrix only when tail risk penalties are enabled (avoids memory cost otherwise)
    if[(lamCVaR > 0) or lamDD > 0; p[`R]:R];

    // Analytical tangency portfolio as starting point (ignores penalties)
    excessMu:mu - rf % af;
    tpState:`w`r`p!(n#1f%n; excessMu - .kdbtools.mm[C;n#1f%n]; excessMu - .kdbtools.mm[C;n#1f%n]);
    cgStep:{[C;s]
        Ap:.kdbtools.mm[C;s`p];
        a:(sum s[`r]*s`r) % sum s[`p]*Ap;
        wNew:s[`w] + a * s`p;
        rNew:s[`r] - a * Ap;
        beta:(sum rNew*rNew) % sum s[`r]*s`r;
        pNew:rNew + beta * s`p;
        `w`r`p!(wNew;rNew;pNew)};
    tpState:50 cgStep[C]/tpState;
    tpw:tpState`w;
    tpw:$[0 < sum tpw; tpw % sum tpw; n#1f%n];

    // Initialize with multiple starting points, keep best
    starts:(n#1f%n;                              // Equal weight
            lo + (hi-lo) * n?1f;                 // Random in bounds
            lo + 0.5 * hi - lo;                  // Midpoint
            tpw);                                // Tangency portfolio
    starts:projectConstrained[lo;hi] each starts;
    startObjs:alphaObj[p;] each starts;
    w:starts startObjs?max startObjs;

    // State (maximize objective, formulated as minimize -objective for L-BFGS)
    negObj:neg max startObjs;
    grad0:neg alphaGrad[p;w];  // Gradient of -objective
    state:`w`obj`iter`converged`grad`sHist`yHist!(w;negObj;0;0b;grad0;();());
    lbfgsMem:10;

    // L-BFGS step with steepest descent fallback
    step:{[p;lo;hi;lbfgsMem;s]
        if[s`converged; :s];
        wCur:s`w;
        objCur:s`obj;
        gradCur:s`grad;

        // Projected gradient convergence check
        atLo:wCur <= lo + 1e-12;
        atHi:wCur >= hi - 1e-12;
        projGrad:gradCur;
        projGrad:?[atLo and projGrad > 0; 0f; projGrad];
        projGrad:?[atHi and projGrad < 0; 0f; projGrad];
        pgNorm:sqrt sum projGrad*projGrad;
        if[pgNorm < 1e-10; :s,enlist[`converged]!enlist 1b];

        // Try L-BFGS direction
        rawDir:lbfgsDir[s`sHist;s`yHist;gradCur];
        isDesc:(sum rawDir * gradCur) > 0;
        lDir:$[isDesc; neg rawDir; neg gradCur];
        dn:sqrt sum lDir*lDir;
        if[dn < 1e-15; :s,enlist[`converged]!enlist 1b];
        lDir:lDir % dn;
        dd:sum gradCur * lDir;

        res:alphaLineSearch[p;lo;hi;wCur;objCur;lDir;dd];
        wNew:res 0; objNew:res 1;

        // Steepest descent fallback
        if[objNew >= objCur;
            sdDir:neg gradCur;
            sdNorm:sqrt sum sdDir*sdDir;
            if[sdNorm > 1e-15;
                sdDir:sdDir % sdNorm;
                sdDD:sum gradCur * sdDir;
                res:alphaLineSearch[p;lo;hi;wCur;objCur;sdDir;sdDD];
                wNew:res 0; objNew:res 1]];

        wDiff:max abs wNew - wCur;
        if[(objNew >= objCur) or (wDiff < 1e-12);
            :s,enlist[`converged]!enlist 1b];

        // Update L-BFGS history
        gradNew:neg alphaGrad[p;wNew];
        sk:wNew - wCur;
        yk:gradNew - gradCur;
        syk:sum sk * yk;
        sHist:s`sHist; yHist:s`yHist;
        $[syk > 1e-10 * (sqrt sum sk*sk) * sqrt sum yk*yk;
            [sHist:$[lbfgsMem <= count sHist; 1 _ sHist; sHist],enlist sk;
             yHist:$[lbfgsMem <= count yHist; 1 _ yHist; yHist],enlist yk];
            [sHist:(); yHist:()]];

        `w`obj`iter`converged`grad`sHist`yHist!(wNew;objNew;s[`iter]+1;0b;gradNew;sHist;yHist)};

    state:nIter step[p;lo;hi;lbfgsMem]/state;
    w:state`w;

    // Compute final metrics
    corrConc:sum w * .kdbtools.mm[Corr;w];
    portTO:portfolioTO[deltaCtx;w];

    res:`weights`sharpe`return`volatility`objective`corrConcentration`portfolioTO`converged`iterations!(
        w;
        sqrt[af] * (sum[w*mu] - rf % af) % sqrt sum w * .kdbtools.mm[C;w];
        af * sum w * mu;
        sqrt[af] * sqrt sum w * .kdbtools.mm[C;w];
        neg state`obj;
        corrConc;
        portTO;
        state`converged;
        state`iter);

    // Tail risk metrics (always reported if penalties were enabled)
    portRets:sum w * R;
    cumRets:sums portRets;
    res[`cvar]:portCVaR[cvarAlpha;w;R];
    res[`cdar]:CDaR[cdarAlpha;cumRets];
    res[`maxDD]:min drawdowns cumRets;
    res}

// Rolling alpha optimization
// Re-optimizes every `step` periods using a trailing `window` of data
optimizeAlphasRolling:{[alphas;returns;window;step;cfg]
    // Defaults
    rf:$[`rf in key cfg; cfg`rf; 0f];
    lamCorr:$[`lamCorr in key cfg; cfg`lamCorr; 0.1];
    lamTO:$[`lamTO in key cfg; cfg`lamTO; 0.1];
    lamCVaR:$[`lamCVaR in key cfg; cfg`lamCVaR; 0f];
    lamDD:$[`lamDD in key cfg; cfg`lamDD; 0f];
    cvarAlpha:$[`cvarAlpha in key cfg; cfg`cvarAlpha; 0.05];
    cdarAlpha:$[`cdarAlpha in key cfg; cfg`cdarAlpha; 0.05];
    nIter:$[`nIter in key cfg; cfg`nIter; 200];
    lo:$[`lo in key cfg; cfg`lo; 0f];
    hi:$[`hi in key cfg; cfg`hi; 1f];
    af:$[`annFactor in key cfg; cfg`annFactor; annFactor];
    hasPeriod:`period in key cfg;

    // Compute full alpha returns table
    arTable:alphaReturns[alphas;returns];

    // Period aggregation (same logic as optimizeAlphas)
    if[hasPeriod;
        periodFn:cfg`period;
        aCols:cols[arTable] except `time;
        aggCols:aCols!({(sum;x)} each aCols);
        arTable:?[arTable; (); enlist[`time]!enlist (periodFn;`time); aggCols]];

    times:arTable`time;
    R:fromTable delete time from arTable;
    n:count R;
    nT:count first R;

    lo:$[0>type lo; n#lo; lo];
    hi:$[0>type hi; n#hi; hi];

    // Build full delta context (full resolution for turnover)
    deltaCtx:alphaDeltas alphas;

    // Re-optimization indices
    optIdxs:window + step * til 1 + (nT - window) div step;
    optIdxs:optIdxs where optIdxs <= nT;

    // Pack context
    opts:`lo`hi`nIter`af`lamCVaR`lamDD`cvarAlpha`cdarAlpha!(lo;hi;nIter;af;lamCVaR;lamDD;cvarAlpha;cdarAlpha);
    ctx:`R`deltaCtx`times`window`rf`lamCorr`lamTO`opts!(R;deltaCtx;times;window;rf;lamCorr;lamTO;opts);

    // Run optimization at each point
    results:{[ctx;idx]
        R:ctx`R; times:ctx`times; window:ctx`window;
        startIdx:idx - window;
        subR:{[x;s;w] x s + til w}[;startIdx;window] each R;
        // Use full deltaCtx (turnover structure is stationary across windows)
        res:optimizeAlphasCore[subR;ctx`deltaCtx;ctx`rf;ctx`lamCorr;ctx`lamTO;ctx`opts];
        res,enlist[`time]!enlist times idx-1
    }[ctx] each optIdxs;

    // Format as table
    wCols:(`$"w",/:string til n);
    tab:([] time:{x`time} each results);
    tab:tab,'(flip wCols!flip {x`weights} each results);
    tab:tab,'([] sharpe:{x`sharpe} each results; portfolioTO:{x`portfolioTO} each results);
    tab}

// -----------------------------------------------------------------------------
// WALK-FORWARD ALPHA WEIGHT PROTOCOL
// -----------------------------------------------------------------------------
// Compare static in-sample, expanding, rolling, and blended weighting schemes.
// Intended for wide alpha return tables: ([] dt/time/date; alpha1; alpha2; ...)
// Returns summary ranking plus best series/weights for diagnostics.

awBoundVec_:{[x;n] $[0 > type x; n#"f"$x; "f"$x]}

awSanitizeVec_:{[x;fill]
    y:"f"$x;
    bad:(null y) | y=0w | y=-0w;
    @[y;where bad;:;"f"$fill]}

awNumericCols_:{[t;colsIn]
    m:meta t;
    exec c from m where (c in colsIn), t in "fegjih"}

awEnsureList_:{[x] $[0 > type x; enlist x; x]}

awPrepareWide_:{[t;cfg]
    if[98h <> type t; '"alphaWeightProtocol expects a table"];
    cs:cols t;
    timeCol:$[`timeCol in key cfg; cfg`timeCol; ::];
    if[(11h <> type timeCol) or 0 = count string timeCol;
        tcands:`time`dt`date;
        hits:tcands inter cs;
        if[0 = count hits; '"alphaWeightProtocol requires `time, `dt or `date (or cfg `timeCol)"];
        timeCol:first hits];
    if[not timeCol in cs; '"time column not found: ",string timeCol];
    ex:$[`excludeCols in key cfg; cfg`excludeCols; `symbol$()];
    ex:$[11h=type ex; ex; $[-11h=type ex; enlist ex; `symbol$()]];
    drop:distinct (enlist timeCol),ex;
    a0:cs except drop;
    aCols:awNumericCols_[t;a0];
    if[0 = count aCols; '"alphaWeightProtocol found no numeric alpha columns"];
    times:t timeCol;
    ord:iasc times;
    times:times ord;
    R0:"f"$value flip aCols#t;
    R:{[x;o] x o}[;ord] each awSanitizeVec_[;0f] each R0;
    `timeCol`alphaCols`times`R!(timeCol;aCols;times;R)}

awShrinkCov_:{[C;lambda;ridge]
    lam:0f | ("f"$lambda) & 1f;
    rg:"f"$ridge;
    n:count C;
    out:C;
    i:0;
    while[i < n;
        row:"f"$out i;
        d:"f"$C[i;i];
        d:$[(null d) | d=0w | d=-0w | d<=0f; 1e-10; d];
        row:(1f - lam) * row;
        row[i]+:(lam * d) + rg;
        out:@[out;i;:;row];
        i+:1];
    out}

awFitWeights_:{[Rwin;muShrink;covShrink;cfg]
    nA:count Rwin;
    if[nA < 1; :0#0f];
    nObs:$[nA > 0; count first Rwin; 0];
    if[nObs < 2; :nA#(1f % nA)];
    rw:awSanitizeVec_[;0f] each Rwin;
    mu:("f"$muShrink) * avg each rw;
    C:.kdbtools.covmat flip rw;
    C:awShrinkCov_[C;covShrink;cfg`ridge];
    wRaw:.kdbtools.mm[.kdbtools.minv C;mu];
    wRaw:awSanitizeVec_[wRaw;0f];
    lo:awBoundVec_[cfg`lo;nA];
    hi:awBoundVec_[cfg`hi;nA];
    w:projectConstrained[lo;hi;wRaw];
    if[sum abs w <= 0f; w:nA#(1f % nA)];
    w}

awCleanSample_:{[x;nonZeroOnly]
    y:"f"$x;
    y:y where not null y;
    if[nonZeroOnly; y:y where y<>0f];
    y}

awMetrics_:{[gross;net;turnover;jump;startIdx;af;nonZeroOnly]
    nAll:count net;
    if[startIdx >= nAll;
        :`n_obs`ann_ret`ann_vol`sharpe`gross_sharpe`win_rate`max_dd`avg_turnover`total_turnover`avg_w_jump`cost_ann!(
            0i;0n;0n;0n;0n;0n;0n;0n;0n;0n;0n)];
    idx:startIdx + til nAll - startIdx;
    rg:awCleanSample_[gross idx;nonZeroOnly];
    rn:awCleanSample_[net idx;nonZeroOnly];
    n:count rn;
    if[n < 2;
        :`n_obs`ann_ret`ann_vol`sharpe`gross_sharpe`win_rate`max_dd`avg_turnover`total_turnover`avg_w_jump`cost_ann!(
            n;0n;0n;0n;0n;0n;0n;avg turnover idx;sum turnover idx;avg jump idx;0n)];
    mu:avg rn;
    vol:dev rn;
    sh:$[vol > 1e-12; sqrt[af] * mu % vol; 0n];
    gmu:$[count rg > 0; avg rg; 0n];
    gvol:$[count rg > 1; dev rg; 0n];
    gsh:$[gvol > 1e-12; sqrt[af] * gmu % gvol; 0n];
    wr:avg rn > 0f;
    mdd:min drawdowns sums rn;
    avgTo:avg turnover idx;
    totTo:sum turnover idx;
    avgJ:avg jump idx;
    costAnn:af * (gmu - mu);
    `n_obs`ann_ret`ann_vol`sharpe`gross_sharpe`win_rate`max_dd`avg_turnover`total_turnover`avg_w_jump`cost_ann!(
        n;af*mu;sqrt[af]*vol;sh;gsh;wr;mdd;avgTo;totTo;avgJ;costAnn)}

awRunWF_:{[R;times;spec;cfg]
    mode:spec`mode;
    window:spec`window;
    rebalance:spec`rebalance;
    muShrink:spec`mu_shrink;
    covShrink:spec`cov_shrink;
    blendLambda:spec`blend_lambda;
    nA:count R;
    nT:$[nA > 0; count first R; 0];
    gross:nT#0n;
    net:nT#0n;
    turn:nT#0f;
    jmp:nT#0f;
    wTimes:0#times;
    wVals:();
    if[(nA < 1) | (nT < 2);
        m0:awMetrics_[gross;net;turn;jmp;0;cfg`annFactor;cfg`nonZeroOnly];
        :`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda`start_idx`weights`gross`net`turnover`jump`metrics!(
            mode;window;rebalance;muShrink;covShrink;blendLambda;0i;([] time:wTimes; weights:wVals);gross;net;turn;jmp;m0)];
    start:$[mode in `rolling`blend; (window | cfg`minObs); cfg`minObs];
    if[start >= nT; start:nT - 1];
    lo:awBoundVec_[cfg`lo;nA];
    hi:awBoundVec_[cfg`hi;nA];
    wCur:nA#0f;
    Rt:flip R;
    i:start;
    while[i < nT;
        if[(i = start) or 0 = (i - start) mod rebalance;
            wPrev:wCur;
            expR:{[x;n] x til n}[;i] each R;
            wExp:awFitWeights_[expR;muShrink;covShrink;cfg];
            wCur:$[mode~`expanding; wExp;
                    mode~`rolling;
                        [s:0 | i - window;
                         rollR:{[x;s;i] x s + til i-s}[;s;i] each R;
                         awFitWeights_[rollR;muShrink;covShrink;cfg]];
                    mode~`blend;
                        [s:0 | i - window;
                         rollR:{[x;s;i] x s + til i-s}[;s;i] each R;
                         wRoll:awFitWeights_[rollR;muShrink;covShrink;cfg];
                         projectConstrained[lo;hi;(1f-blendLambda)*wExp + blendLambda*wRoll]];
                    wExp];
            wCur:projectConstrained[lo;hi;awSanitizeVec_[wCur;0f]];
            dw:wCur - wPrev;
            turn[i]:sum abs dw;
            jmp[i]:max abs dw;
            wTimes,:enlist times i;
            wVals,:enlist wCur];
        g:sum wCur * Rt i;
        gross[i]:g;
        cp:"f"$cfg`costPerTurn;
        net[i]:g - cp * turn i;
        i+:1];
    m:awMetrics_[gross;net;turn;jmp;start;cfg`annFactor;cfg`nonZeroOnly];
    `mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda`start_idx`weights`gross`net`turnover`jump`metrics!(
        mode;window;rebalance;muShrink;covShrink;blendLambda;start;([] time:wTimes; weights:wVals);gross;net;turn;jmp;m)}

awRunStaticIS_:{[R;times;muShrink;covShrink;cfg]
    nA:count R;
    nT:$[nA > 0; count first R; 0];
    gross:nT#0n;
    net:nT#0n;
    turn:nT#0f;
    jmp:nT#0f;
    if[(nA < 1) or (nT < 2);
        m0:awMetrics_[gross;net;turn;jmp;0;cfg`annFactor;cfg`nonZeroOnly];
        :`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda`start_idx`weights`gross`net`turnover`jump`metrics!(
            `static_is;0i;0i;muShrink;covShrink;0f;0i;([] time:0#times; weights:());gross;net;turn;jmp;m0)];
    w:awFitWeights_[R;muShrink;covShrink;cfg];
    Rt:flip R;
    i:0;
    while[i < nT;
        gross[i]:sum w * Rt i;
        i+:1];
    turn[0]:sum abs w;
    jmp[0]:max abs w;
    cp:"f"$cfg`costPerTurn;
    net:gross - cp * turn;
    m:awMetrics_[gross;net;turn;jmp;0;cfg`annFactor;cfg`nonZeroOnly];
    `mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda`start_idx`weights`gross`net`turnover`jump`metrics!(
        `static_is;0i;0i;muShrink;covShrink;0f;0i;([] time:enlist first times; weights:enlist w);gross;net;turn;jmp;m)}

awRowFromRun_:{[run]
    m:run`metrics;
    `run_id`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda`n_obs`ann_ret`ann_vol`gross_sharpe`net_sharpe`win_rate`max_dd`avg_turnover`total_turnover`avg_w_jump`cost_ann!(
        run`run_id;
        run`mode;
        run`window;
        run`rebalance;
        run`mu_shrink;
        run`cov_shrink;
        run`blend_lambda;
        m`n_obs;
        m`ann_ret;
        m`ann_vol;
        m`gross_sharpe;
        m`sharpe;
        m`win_rate;
        m`max_dd;
        m`avg_turnover;
        m`total_turnover;
        m`avg_w_jump;
        m`cost_ann)}

// Main protocol runner
// Input:
//   t - wide alpha return table ([] dt/time/date; a1; a2; ...)
//   cfg optional keys:
//     `timeCol `excludeCols
//     `windows `rebalances `muShrinks `covShrinks `blendLambdas
//     `minObs `annFactor `costBps `lo `hi `ridge `nonZeroOnly
//     `includeStaticIS `baselineMode
//     `minNetSharpe `minSharpeUplift `watchSharpeFloor `watchUpliftFloor
//     `maxAvgTurnover `maxAvgWeightJump
//     `turnoverPenalty `instabilityPenalty
//
// Output dict keys:
//   `summary `best `series `weights `cfg `alphaCols `timeCol `baselineSharpe
alphaWeightProtocol:{[t;cfg]
    defaults:`timeCol`excludeCols`windows`rebalances`muShrinks`covShrinks`blendLambdas`minObs`annFactor`costBps`lo`hi`ridge`nonZeroOnly`includeStaticIS`baselineMode`minNetSharpe`minSharpeUplift`watchSharpeFloor`watchUpliftFloor`maxAvgTurnover`maxAvgWeightJump`turnoverPenalty`instabilityPenalty!(
        ::;
        `symbol$();
        126 252 504;
        5 20;
        0.25 0.5 1f;
        0.1 0.3 0.6;
        0.25 0.5 0.75;
        252i;
        annFactor;
        0f;
        0f;
        1f;
        1e-8;
        1b;
        1b;
        `expanding;
        0.5;
        0.05;
        0.2;
        -0.1;
        0.75;
        0.4;
        0.15;
        0.1);
    p:$[99h = type cfg; defaults,cfg; defaults];
    p[`costPerTurn]:("f"$p`costBps) * 1e-4;
    p[`windows]:"j"$awEnsureList_[p`windows];
    p[`rebalances]:"j"$awEnsureList_[p`rebalances];
    p[`muShrinks]:"f"$awEnsureList_[p`muShrinks];
    p[`covShrinks]:"f"$awEnsureList_[p`covShrinks];
    p[`blendLambdas]:"f"$awEnsureList_[p`blendLambdas];

    prep:awPrepareWide_[t;p];
    R:prep`R;
    times:prep`times;
    wins:p`windows;
    rebs:p`rebalances;
    mus:p`muShrinks;
    covs:p`covShrinks;
    blends:p`blendLambdas;

    runs:();
    rid:0i;

    iMu:0;
    while[iMu < count mus;
        mu:mus iMu;
        iCov:0;
        while[iCov < count covs;
            cv:covs iCov;
            if[p`includeStaticIS;
                run:awRunStaticIS_[R;times;mu;cv;p];
                run:run,enlist[`run_id]!enlist rid;
                runs,:enlist run;
                rid+:1];
            iRb:0;
            while[iRb < count rebs;
                rb:rebs iRb;
                spec:`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda!(`expanding;0i;rb;mu;cv;0f);
                run:awRunWF_[R;times;spec;p];
                run:run,enlist[`run_id]!enlist rid;
                runs,:enlist run;
                rid+:1;

                iW:0;
                while[iW < count wins;
                    w:wins iW;
                    spec:`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda!(`rolling;w;rb;mu;cv;0f);
                    run:awRunWF_[R;times;spec;p];
                    run:run,enlist[`run_id]!enlist rid;
                    runs,:enlist run;
                    rid+:1;

                    iB:0;
                    while[iB < count blends;
                        bl:blends iB;
                        spec:`mode`window`rebalance`mu_shrink`cov_shrink`blend_lambda!(`blend;w;rb;mu;cv;bl);
                        run:awRunWF_[R;times;spec;p];
                        run:run,enlist[`run_id]!enlist rid;
                        runs,:enlist run;
                        rid+:1;
                        iB+:1];
                    iW+:1];
                iRb+:1];
            iCov+:1];
        iMu+:1];

    if[0 = count runs; '"alphaWeightProtocol produced no runs"];
    rows:awRowFromRun_ each runs;
    summary:rows;
    summary:update objective:("f"$net_sharpe) - ("f"$p`turnoverPenalty) * ("f"$avg_turnover) - ("f"$p`instabilityPenalty) * ("f"$avg_w_jump) from summary;
    baseMode:$[11h=type p`baselineMode; p`baselineMode; `expanding];
    baseRows:select from summary where mode = baseMode;
    baseSharpe:$[count baseRows; max baseRows[`net_sharpe]; 0f];
    summary:update sharpe_vs_base:net_sharpe - baseSharpe from summary;
    summary:`objective xdesc summary;

    nSum:count summary;
    dec:nSum#`reject;
    i:0;
    while[i < nSum;
        sh:summary[`net_sharpe] i;
        up:summary[`sharpe_vs_base] i;
        to:summary[`avg_turnover] i;
        j:summary[`avg_w_jump] i;
        dec[i]:$[(sh >= p`minNetSharpe) & (up >= p`minSharpeUplift) & (to <= p`maxAvgTurnover) & (j <= p`maxAvgWeightJump); `accept;
                (sh >= p`watchSharpeFloor) & (up >= p`watchUpliftFloor); `watch;
                `reject];
        i+:1];
    summary:update decision:dec from summary;

    best:first summary;
    bestRun:runs best`run_id;
    series:([] time:times; gross:bestRun`gross; net:bestRun`net; turnover:bestRun`turnover; w_jump:bestRun`jump);

    `summary`best`series`weights`cfg`alphaCols`timeCol`baselineSharpe!(
        summary;
        best;
        series;
        bestRun`weights;
        p;
        prep`alphaCols;
        prep`timeCol;
        baseSharpe)}

// Alpha diagnostics report
alphaReport:{[alphas;returns;weights]
    arTable:alphaReturns[alphas;returns];
    R:fromTable delete time from arTable;
    n:count R;
    alphaTO:alphaTurnover alphas;
    deltaCtx:alphaDeltas alphas;
    C:.kdbtools.covmat flip R;
    mu:avg each R;
    vols:sqrt C @' til n;
    Corr:C % vols *\: vols;
    zeroIdx:where 0=vols;
    if[0 < count zeroIdx; Corr:@[Corr;zeroIdx;:;(count[zeroIdx])#enlist n#0f]];

    // Per-alpha stats
    sharpes:{[r] s:sqrt[annFactor] * avg[r] % dev r; $[null s;0f;s]} each R;
    names:`$"a",/:string til n;

    // Combined portfolio
    portRets:sum weights * R;
    portMu:avg portRets;
    portVol:dev portRets;
    portSharpeVal:sqrt[annFactor] * portMu % portVol;
    portTO:portfolioTO[deltaCtx;weights];
    sumAlphaTO:sum weights * alphaTO;

    // Tail risk metrics
    cumRets:sums portRets;
    cvar5:portCVaR[0.05;weights;R];
    cdar5:CDaR[0.05;cumRets];
    mdd:min drawdowns cumRets;

    -1 "=== Alpha Portfolio Report ===";
    -1 "";
    -1 "Per-Alpha Statistics:";
    perAlpha:([] name:names; weight:weights; sharpe:sharpes; avgTO:alphaTO; annVol:sqrt[annFactor]*vols);
    show perAlpha;
    -1 "";
    -1 "Correlation Matrix:";
    show Corr;
    -1 "";
    -1 "Combined Portfolio:";
    -1 "  Sharpe:         ",string portSharpeVal;
    -1 "  Return(ann):    ",string annFactor * portMu;
    -1 "  Vol(ann):       ",string sqrt[annFactor] * portVol;
    -1 "  Portfolio TO:   ",string portTO;
    -1 "  Sum-of-parts:   ",string sumAlphaTO;
    -1 "  TO netting:     ",string 1 - portTO % sumAlphaTO;
    -1 "";
    -1 "Tail Risk:";
    -1 "  CVaR(5%):       ",string cvar5;
    -1 "  CDaR(5%):       ",string cdar5;
    -1 "  Max Drawdown:   ",string mdd;
    -1 "";
    `perAlpha`combined`corrMatrix!(
        perAlpha;
        `sharpe`return`volatility`portfolioTO`sumOfPartsTO`nettingBenefit`cvar`cdar`maxDD!(
            portSharpeVal;annFactor*portMu;sqrt[annFactor]*portVol;portTO;sumAlphaTO;1-portTO%sumAlphaTO;cvar5;cdar5;mdd);
        Corr)}

// -----------------------------------------------------------------------------
// HIERARCHICAL / CLUSTERED RISK PARITY
// -----------------------------------------------------------------------------
// Based on López de Prado's Hierarchical Risk Parity (HRP) method
// Clusters assets/signals by correlation, then applies recursive bisection
// with inverse-variance weighting within clusters

// Correlation distance: sqrt(0.5 * (1 - correlation))
corrDistance:{[corr] sqrt 0.5 * 1 - corr}

// Compute distance matrix from correlation matrix
distanceMatrix:{[Corr]
    n:count Corr;
    D:corrDistance each Corr;
    {[D;i] D[i;i]:0f; D}/[D;til n]}

// Simple seriation: order items by similarity using greedy nearest-neighbor
// D: distance matrix
// Returns: ordered indices
seriation:{[D]
    n:count D;
    if[n<2; :til n];
    // Start with item 0
    order:enlist 0;
    remaining:1 _ til n;
    // Greedily add nearest item
    do[n-1;
        if[0=count remaining; :order];
        last_item:last order;
        dists:D[last_item] remaining;
        nearest_idx:dists?min dists;
        nearest:remaining nearest_idx;
        order,:nearest;
        remaining:remaining where not remaining = nearest];
    order}

// Recursive bisection for HRP weights
// C: covariance matrix
// items: list of indices in current cluster
// Returns: weights for items
recursiveBisection:{[C;items]
    n:count items;
    if[n=1; :enlist 1f];
    if[n=2;
        v1:C[items 0;items 0];
        v2:C[items 1;items 1];
        iv1:1 % v1 | 1e-10;
        iv2:1 % v2 | 1e-10;
        w1:iv1 % iv1 + iv2;
        :w1,1-w1];
    // Split in half
    mid:n div 2;
    left:mid#items;
    right:mid _ items;
    // Recursive weights
    wLeft:recursiveBisection[C;left];
    wRight:recursiveBisection[C;right];
    // Cluster variances
    Cleft:C[left;left];
    Cright:C[right;right];
    vLeft:sum wLeft * Cleft mmu wLeft;
    vRight:sum wRight * Cright mmu wRight;
    // Inverse variance allocation
    ivLeft:1 % vLeft | 1e-10;
    ivRight:1 % vRight | 1e-10;
    alphaLeft:ivLeft % ivLeft + ivRight;
    (alphaLeft * wLeft),(1 - alphaLeft) * wRight}

// Hierarchical Risk Parity weights
// R: return matrix (n x T)
hrpWeights:{[R]
    n:count R;
    if[n<2; :n#1f];
    C:.kdbtools.covmat flip R;
    vols:sqrt C @' til n;
    vols:vols | 1e-10;
    Corr:C % vols *\: vols;
    D:distanceMatrix[Corr];
    order:seriation[D];
    wOrdered:recursiveBisection[C;order];
    w:n#0f;
    w[order]:wOrdered;
    w}

// HRP optimizer
hrp:{[R]
    n:count R;
    C:.kdbtools.covmat flip R;
    w:hrpWeights[R];
    `weights`sharpe`return`volatility`method!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C];
        `hrp)}

// Silhouette score for clustering quality
// D: distance matrix
// assignments: cluster assignment for each item
// Returns: average silhouette score (-1 to 1, higher is better)
silhouetteScore:{[D;assignments]
    n:count assignments;
    clusters:group assignments;
    k:count clusters;
    if[k<2; :0f];  // Need at least 2 clusters

    // For each item, compute silhouette
    scores:{[D;clusters;assignments;i]
        myCluster:assignments i;
        myMembers:clusters myCluster;

        // a(i) = avg distance to items in same cluster
        if[1=count myMembers; :0f];  // Singleton cluster
        a:avg D[i] myMembers where not myMembers = i;

        // b(i) = min avg distance to items in other clusters
        otherClusters:key[clusters] except enlist myCluster;
        if[0=count otherClusters; :0f];
        // Use 'each' on cluster IDs, then index into clusters dict
        b:min {[D;clusters;i;cid] avg D[i] clusters cid}[D;clusters;i] each otherClusters;

        // silhouette = (b - a) / max(a, b)
        (b - a) % a | b | 1e-10
    }[D;clusters;assignments] each til n;

    avg scores}

// K-medoids clustering (returns assignments only)
// D: distance matrix
// k: number of clusters
// Returns: list of cluster assignments
kmedoids:{[D;k]
    n:count D;
    k:k & n;
    if[k<2; :n#0];

    // Initialize: pick k items with maximum distance spread
    medoids:enlist 0;
    do[k-1;
        minDists:{[D;medoids;i] min D[i] medoids}[D;medoids] each til n;
        minDists[medoids]:0f;
        medoids,:minDists?max minDists];

    // Assign each item to nearest medoid
    assignments:{[D;medoids;i] medoids D[i;medoids]?min D[i] medoids}[D;medoids] each til n;

    // Iterate to improve
    do[10;
        newMedoids:{[D;members]
            if[0=count members; :-1];
            costs:{[D;m;members] sum D[m] members}[D;;members] each members;
            members costs?min costs
        }[D]'[group assignments];
        medoids:asc value newMedoids;
        assignments:{[D;medoids;i] medoids D[i;medoids]?min D[i] medoids}[D;medoids] each til n];

    assignments}

// Find optimal number of clusters using silhouette method
// D: distance matrix
// minK: minimum clusters (default 2)
// maxK: maximum clusters to try (default: n/2 capped at 10)
// Returns: optimal k
optimalK:{[D;minK;maxK]
    n:count D;
    minK:$[null minK; 2; minK | 2];           // at least 2
    maxK:$[null maxK; (n div 2) & 10; maxK];
    maxK:maxK & n-1;                          // can't exceed n-1
    minK:minK & maxK;                         // minK can't exceed maxK
    if[maxK<2; :1];

    // Try k from minK to maxK
    ks:minK + til 1 + maxK - minK;
    scores:{[D;k]
        assignments:kmedoids[D;k];
        silhouetteScore[D;assignments]
    }[D] each ks;

    // Return k with highest silhouette
    ks scores?max scores}

// Find minimum k such that no signal weight exceeds maxWeight
// R: return matrix
// C: covariance matrix
// D: distance matrix
// maxWeight: maximum allowed weight per signal (e.g., 0.15 for 15%)
// maxK: maximum k to try
// Returns: k that satisfies constraint, or maxK if not achievable
optimalKByWeight:{[R;C;D;maxWeight;maxK]
    n:count R;
    maxK:$[null maxK; n-1; maxK & n-1];
    if[maxK < 2; :2];

    // Helper to compute weights for a given k
    computeWeights:{[R;C;D;k]
        assignments:kmedoids[D;k];
        clusters:group assignments;
        actualK:count clusters;
        if[actualK < 2; :n#1f%n];  // fallback to equal weight

        withinWeights:{[C;members]
            if[1=count members; :enlist 1f];
            clusterVols:sqrt C[members;members] @' til count members;
            normalize 1f % clusterVols | 1e-10
        }[C] each value clusters;

        clusterReturns:{[R;members;w] sum w * R members}[R]'[value clusters;withinWeights];

        // Covariance of cluster returns
        clusterC:$[1 = actualK;
            enlist enlist var first clusterReturns;
            .kdbtools.covmat flip clusterReturns];
        clusterVols:sqrt clusterC @' til actualK;
        acrossWeights:normalize 1f % clusterVols | 1e-10;

        // Final weights
        w:(count R)#0f;
        {[w;acrossW;withinW;members]
            w[members]:acrossW * withinW;
            w
        }/[w;acrossWeights;withinWeights;value clusters]
    }[R;C;D];

    // Try increasing k until constraint is satisfied
    k:2;
    while[k <= maxK;
        w:computeWeights k;
        if[max[w] <= maxWeight; :k];
        k+:1];

    maxK}  // Return maxK if constraint not achievable

// Clustered Risk Parity with automatic or fixed number of clusters
// Uses k-medoids clustering then risk parity within/across clusters
// R: return matrix
// k: number of clusters (0 or 0N for automatic selection)
// Returns dict with weights and cluster assignments
clusteredRiskParity:{[R;k]
    n:count R;

    C:.kdbtools.covmat flip R;
    vols:sqrt C @' til n;
    Corr:C % vols *\: vols;
    zeroIdx:where 0=vols;
    if[0 < count zeroIdx; Corr:@[Corr;zeroIdx;:;(count[zeroIdx])#enlist n#0f]];

    // Distance matrix
    D:distanceMatrix[Corr];

    // Auto-select k if 0 or null (use minK=2, maxK=default)
    k:$[(k=0) or null k; optimalK[D;0N;0N]; k & n];
    if[k<2; :riskParity[R;0;0]];

    // Run k-medoids clustering
    assignments:kmedoids[D;k];

    // Group items by cluster
    clusters:group assignments;
    clusterIds:key clusters;

    // Within-cluster weights (inverse volatility)
    withinWeights:{[C;members]
        if[1=count members; :enlist 1f];
        clusterVols:sqrt C[members;members] @' til count members;
        normalize 1f % clusterVols | 1e-10
    }[C] each value clusters;

    // Cluster-level covariance
    clusterReturns:{[R;members;w] sum w * R members}[R]'[value clusters;withinWeights];
    clusterC:.kdbtools.covmat flip clusterReturns;
    clusterVols:sqrt clusterC @' til k;

    // Across-cluster weights (inverse volatility)
    acrossWeights:normalize 1f % clusterVols | 1e-10;

    // Combine: final weight = across_weight * within_weight
    w:n#0f;
    w:{[w;acrossW;withinW;members]
        w[members]:acrossW * withinW;
        w
    }/[w;acrossWeights;withinWeights;value clusters];

    `weights`sharpe`return`volatility`k`silhouette`clusters`clusterWeights`method!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C];
        k;
        silhouetteScore[D;assignments];
        assignments;
        acrossWeights;
        `clusteredRP)}

// Auto-clustered RP (automatic k selection)
// R: return matrix
// Returns dict with weights, optimal k, and silhouette score
autoClusteredRP:{[R] clusteredRiskParity[R;0]}

// Clustered optimization with Sharpe maximization
// R: return matrix
// k: number of clusters (0 for auto)
// withinMethod: `rp (risk parity) or `sharpe (max Sharpe) within clusters
// acrossMethod: `rp (risk parity) or `sharpe (max Sharpe) across clusters
// Returns dict with weights and cluster info
clusteredOptimize:{[R;k;withinMethod;acrossMethod]
    n:count R;

    C:.kdbtools.covmat flip R;
    vols:sqrt C @' til n;
    Corr:C % vols *\: vols;
    zeroIdx:where 0=vols;
    if[0 < count zeroIdx; Corr:@[Corr;zeroIdx;:;(count[zeroIdx])#enlist n#0f]];

    // Distance matrix
    D:distanceMatrix[Corr];

    // Auto-select k if 0 or null (use minK=2, maxK=default)
    k:$[(k=0) or null k; optimalK[D;0N;0N]; k & n];
    if[k<2; :$[withinMethod~`sharpe; maxSharpe[R;0f;1000]; riskParity[R;0;0]]];

    // Run k-medoids clustering
    assignments:kmedoids[D;k];
    clusters:group assignments;

    // Within-cluster weights
    withinWeights:$[withinMethod~`sharpe;
        // Max Sharpe within each cluster
        {[R;members]
            if[1=count members; :enlist 1f];
            Rsub:R members;
            // Use random search max Sharpe (1000 iterations for small clusters)
            res:maxSharpe[Rsub;0f;1000];
            res`weights
        }[R] each value clusters;
        // Risk parity (inverse vol) within each cluster
        {[C;members]
            if[1=count members; :enlist 1f];
            clusterVols:sqrt C[members;members] @' til count members;
            normalize 1f % clusterVols | 1e-10
        }[C] each value clusters];

    // Cluster-level returns (weighted by within-cluster weights)
    clusterReturns:{[R;members;w] sum w * R members}[R]'[value clusters;withinWeights];

    // Across-cluster weights
    acrossWeights:$[acrossMethod~`sharpe;
        // Max Sharpe across clusters
        {[clusterReturns;k]
            if[k<2; :enlist 1f];
            res:maxSharpe[clusterReturns;0f;1000];
            res`weights
        }[clusterReturns;k];
        // Risk parity across clusters
        {[clusterReturns;k]
            clusterC:.kdbtools.covmat flip clusterReturns;
            clusterVols:sqrt clusterC @' til k;
            normalize 1f % clusterVols | 1e-10
        }[clusterReturns;k]];

    // Combine: final weight = across_weight * within_weight
    w:n#0f;
    w:{[w;acrossW;withinW;members]
        w[members]:acrossW * withinW;
        w
    }/[w;acrossWeights;withinWeights;value clusters];

    methodName:`$"clustered_",string[withinMethod],"_",string acrossMethod;

    `weights`sharpe`return`volatility`k`silhouette`clusters`clusterWeights`withinMethod`acrossMethod`method!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C];
        k;
        silhouetteScore[D;assignments];
        assignments;
        acrossWeights;
        withinMethod;
        acrossMethod;
        methodName)}

// Convenience wrappers for common clustered optimization patterns
clusteredSharpe:{[R;k] clusteredOptimize[R;k;`sharpe;`sharpe]}      // Max Sharpe within and across
clusteredSharpeRP:{[R;k] clusteredOptimize[R;k;`sharpe;`rp]}        // Max Sharpe within, RP across
clusteredRPSharpe:{[R;k] clusteredOptimize[R;k;`rp;`sharpe]}        // RP within, max Sharpe across

// Config-based clustered optimization
// R: return matrix
// cfg: dict with keys: `k`minK`maxK`maxWeight`within`across
//      k: fixed clusters (0 or null for auto)
//      minK: minimum clusters for auto (default 2)
//      maxK: maximum clusters for auto (default n/2, max 10)
//      maxWeight: max weight per signal (e.g., 0.15 for 15%) - overrides silhouette-based selection
//      within: `rp or `sharpe (default `rp)
//      across: `rp or `sharpe (default `rp)
clusteredOptimizeCfg:{[R;cfg]
    n:count R;
    k:$[`k in key cfg; cfg`k; 0];
    minK:$[`minK in key cfg; cfg`minK; 0N];
    maxK:$[`maxK in key cfg; cfg`maxK; 0N];
    maxWeight:$[`maxWeight in key cfg; cfg`maxWeight; 0N];
    withinMethod:$[`within in key cfg; cfg`within; `rp];
    acrossMethod:$[`across in key cfg; cfg`across; `rp];

    C:.kdbtools.covmat flip R;
    vols:sqrt C @' til n;
    Corr:C % vols *\: vols;
    zeroIdx:where 0=vols;
    if[0 < count zeroIdx; Corr:@[Corr;zeroIdx;:;(count[zeroIdx])#enlist n#0f]];

    D:distanceMatrix[Corr];

    // Auto-select k: by maxWeight constraint, or by silhouette with minK/maxK
    k:$[(k=0) or null k;
        $[not null maxWeight;
            optimalKByWeight[R;C;D;maxWeight;maxK];  // weight-based selection
            optimalK[D;minK;maxK]];                  // silhouette-based selection
        k & n];
    if[k<2; :$[withinMethod~`sharpe; maxSharpe[R;0f;1000]; riskParity[R;0;0]]];

    assignments:kmedoids[D;k];
    clusters:group assignments;

    // Within-cluster weights
    withinWeights:$[withinMethod~`sharpe;
        {[R;members]
            if[1=count members; :enlist 1f];
            res:maxSharpe[R members;0f;1000];
            res`weights
        }[R] each value clusters;
        {[C;members]
            if[1=count members; :enlist 1f];
            clusterVols:sqrt C[members;members] @' til count members;
            normalize 1f % clusterVols | 1e-10
        }[C] each value clusters];

    clusterReturns:{[R;members;w] sum w * R members}[R]'[value clusters;withinWeights];

    acrossWeights:$[acrossMethod~`sharpe;
        {[clusterReturns;k]
            if[k<2; :enlist 1f];
            res:maxSharpe[clusterReturns;0f;1000];
            res`weights
        }[clusterReturns;k];
        {[clusterReturns;k]
            clusterC:.kdbtools.covmat flip clusterReturns;
            clusterVols:sqrt clusterC @' til k;
            normalize 1f % clusterVols | 1e-10
        }[clusterReturns;k]];

    w:n#0f;
    w:{[w;acrossW;withinW;members]
        w[members]:acrossW * withinW;
        w
    }/[w;acrossWeights;withinWeights;value clusters];

    methodName:`$"clustered_",string[withinMethod],"_",string acrossMethod;

    `weights`sharpe`return`volatility`k`minK`maxK`silhouette`clusters`clusterWeights`withinMethod`acrossMethod`method!(
        w;
        portSharpe[w;R;C;0f];
        annFactor * portRet[w;R];
        sqrt[annFactor] * portVol[w;C];
        k;
        minK;
        maxK;
        silhouetteScore[D;assignments];
        assignments;
        acrossWeights;
        withinMethod;
        acrossMethod;
        methodName)}

// Convenience wrapper for list of alpha signal tables
// Each table in the list is one alpha's signal+returns
// Assumes each table has a 'ret' column for the alpha's returns
// signalTables: list of tables, each with `ret column
// k: number of clusters (0 for full HRP)
clusteredAlphaRP:{[signalTables;k]
    // Extract return columns from each table
    R:{x`ret} each signalTables;
    $[k>0;
        clusteredRiskParity[R;k];
        hrp[R]]}

// Clustered RP directly from return matrix with names
// R: return matrix (each row = one signal's returns)
// names: signal names (optional, for labeling)
// k: number of clusters (0 for full HRP)
clusteredRPNamed:{[R;names;k]
    result:$[k>0; clusteredRiskParity[R;k]; hrp[R]];
    if[0<count names;
        result:result,enlist[`weightsBySignal]!enlist names!result`weights];
    result}

// Verify Sharpe calculation - shows step-by-step computation
// R: return matrix, w: weights from optimizer
// Returns dict with all intermediate values for debugging
verifySharpe:{[R;w]
    // Portfolio returns (time series)
    portRets:sum w * R;
    T:count portRets;

    // Statistics using different std methods
    avgRet:avg portRets;
    stdDev:dev portRets;                              // Q's dev uses n (population)
    stdSdev:sdev portRets;                            // sdev uses n-1 (sample)

    // What optimizer uses (covariance matrix with n-1)
    C:.kdbtools.covmat flip R;
    stdCov:portVol[w;C];                              // sqrt(w'Cw), C uses n-1

    -1 "=== SHARPE VERIFICATION ===";
    -1 "Portfolio returns (first 10): ",", " sv string 10#portRets;
    -1 "Periods: ",string T;
    -1 "";
    -1 "Mean return: ",string avgRet;
    -1 "";
    -1 "Volatility methods:";
    -1 "  dev (n divisor):     ",string stdDev;
    -1 "  sdev (n-1 divisor):  ",string stdSdev;
    -1 "  sqrt(w'Cw) [optimizer uses this]: ",string stdCov;
    -1 "";
    -1 "Annualized Sharpe (252 days):";
    -1 "  Using dev:           ",string annFactor * avgRet % sqrt[annFactor] * stdDev;
    -1 "  Using sdev:          ",string annFactor * avgRet % sqrt[annFactor] * stdSdev;
    -1 "  Using sqrt(w'Cw):    ",string annFactor * avgRet % sqrt[annFactor] * stdCov;
    -1 "  Optimizer reports:   ",string portSharpe[w;R;C;0f];
    -1 "";

    `portRets`avgRet`stdDev`stdSdev`stdCov`sharpeOptimizer!(
        portRets;avgRet;stdDev;stdSdev;stdCov;portSharpe[w;R;C;0f])}

// Compute Sharpe with custom annualization factor
// af: annualization factor (252=daily, 52=weekly, 12=monthly, 1=already annualized)
sharpeCustom:{[R;w;af]
    portRets:sum w * R;
    avgRet:avg portRets;
    stdRet:sdev portRets;  // use sdev (n-1) for consistency with covariance
    (af * avgRet) % sqrt[af] * stdRet}

\d .

// Load message
-1 "Loaded .optimizer namespace v0.2.0";
-1 "Functions: maxSharpe, maxSharpeConstrained, maxSharpeSLSQP, maxSharpeHybrid, maxMedianSharpe";
-1 "          riskParity, minVariance, minCDaR, maxCER, minEntropicRisk, maxKappa, maxSortino";
-1 "Run .optimizer.help[] for usage or .optimizer.example[] for demo";
