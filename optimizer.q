...
optimalK:{[D;maxK]
    n:count D;
    maxK:$[null maxK; (n div 2) & 10; maxK & n-1];
    if[maxK<2; :1];

    // Try k from 2 to maxK
    ks:2 + til maxK - 1;
    scores:{[D;k]
...
clusteredRiskParity:{[R;k]
    ...
    // Auto-select k if 0 or null
    k:$[(k=0) or null k; optimalK[D;0N]; k & n];
...
clusteredOptimize:{[R;k;withinMethod;acrossMethod]
    ...
    // Auto-select k if 0 or null
    k:$[(k=0) or null k; optimalK[D;0N]; k & n];
    if[k<2; :$[withinMethod~`sharpe; maxSharpe[R;0f;0]; riskParity[R;0;0]]];
