...
def fit_surface_per_expiry(rows: List[Dict[str, str]]) -> Dict[Tuple[str, float], Dict[str, float]]:
    ...
    for exp, exp_rows in by_exp.items():
        ...
        if len(ks_sorted) < 2:
            k0 = float(ks_sorted[0]) if len(ks_sorted) else 0.0
            w0 = float(ws_sorted[0]) if len(ws_sorted) else 1.0e-8
            ks_sorted = np.array([k0 - 1.0e-6, k0 + 1.0e-6])
            ws_sorted = np.array([w0, w0])
...
def fit_surface_rows(rows: List[Dict[str, object]]) -> VolSurface:
    ...
    for exp, exp_rows in by_exp.items():
        ...
        ks_sorted = ks_arr[order]
        ws_sorted = ws_arr[order]
        wts_sorted = wts_arr[order]
...
def annotate_surface_df(
    ...
    df["edge_per_vega"] = df["edge"] / df["vega"].where(df["vega"].abs() > 1.0e-8)
...
def fit_surface_df(
    ...
    for key, idx in grouped.groups.items():
        ...
        if len(k_sorted) < 2:
            k0 = float(k_sorted[0]) if len(k_sorted) else 0.0
            w0 = float(w_sorted[0]) if len(w_sorted) else 1.0e-8
            k_sorted = np.array([k0 - 1.0e-6, k0 + 1.0e-6])
            w_sorted = np.array([w0, w0])
            wts_sorted = np.array([1.0, 1.0])
