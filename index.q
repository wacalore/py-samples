...
buildIndexComposition:{[bonds;asOfDate;indexName;prices;outstandings]
    ...
    // Add outstanding amounts (use auction typical size as proxy if not provided)
    // For now, assume equal weighting if not provided
    eligible:update outstanding:1e9 from eligible;  // Default $1B
    if[not outstandings ~ (::);
        eligible:update outstanding:outstandings cusip from eligible where cusip in key outstandings];

    // Calculate yields (assume par pricing: ytm = coupon)
    eligible:update ytm:coupon from eligible;

    // Calculate durations
    eligible:update modDuration:.tsyindex.bondDuration[asOfDate;;]'[maturityDate;coupon;ytm] from eligible;

    // Calculate market values
    eligible:update marketValue:outstanding * cleanPrice % 100 from eligible;
    ...
    select cusip, securityType, coupon, maturityDate, issueDate, origTerm,
           remYears, bucket, cleanPrice, ytm, outstanding, marketValue,
...
forecastMonth:{[bonds;currentComp;targetMonth;indexName;asOfDate]
    ...
    if[0 < count newIssues;
        ...
        newRows:([]
            ...
            outstanding:newIssues`typicalSize;
...
extensionFromPrices:{[bonds;priceData;indexName]
    ...
    // Get outstandings from bonds (use publicFloat if available)
    outstandings:$[`publicFloat in cols bonds;
        bonds[`cusip]!bonds`publicFloat;
        bonds[`cusip]!bonds`outstanding];
    // Fill nulls with $20B default
    outstandings:?[null outstandings; 20e9; outstandings];

    // Process each date
    results:{[bonds;priceData;outstandings;indexName;asOfDate]
        // Get prices for this date as dict cusip->price
        dayPrices:exec cusip!price from priceData where date = asOfDate;

        // Build composition with market prices
        comp:.tsyindex.buildIndexComposition[bonds;asOfDate;indexName;dayPrices;outstandings];
        if[0 = count comp; :()];
    ...
    }[bonds;priceData;outstandings;indexName] each dates;
