...
mi:{[x;y;J;K]
  mask:(not null x) & not null y;
  x:x where mask; y:y where mask;
  n:count x;
  if[n < J * K; :0n];
  bx:qbin[J;x]; by:qbin[K;y];
  // Build J×K joint count matrix
  jnt:(J;K)#(J * K)#0;
  i:0; while[i < n;
    jnt[bx i;by i]+:1;
    i+:1];
  ...
  j:0; while[j < J;
    k:0; while[k < K;
      ...
      k+:1];
    j+:1];
  // Miller-Madow bias correction: subtract (m-1)/(2*N)
  s - (m - 1) % 2 * n}

// Local MI per x-bin ("inefficiency intensity")
// Returns J-length vector: KL divergence of conditional Y|X=j from marginal Y
localMI:{[x;y;J;K]
  mask:(not null x) & not null y;
  x:x where mask; y:y where mask;
  n:count x;
...
// Map local MI back to each observation (same length as x)
localMIvec:{[x;y;J;K] lmi:localMI[x;y;J;K]; lmi qbin[J;x]}

// Rolling MI over sliding window of size w
// Returns n-length vector with nulls for first w-1 positions
rmi:{[w;x;y;J;K]
  n:count x;
  r:n#0n;
  i:w; while[i <= n;
    r[i-1]:mi[x (i-w)+til w; y (i-w)+til w; J; K];
    i+:1];
  r}

// MI profile table: per-bin diagnostics
// Returns table with xbin, localMI, margX (marginal), avgY (conditional mean), cnt
miProfile:{[x;y;J;K]
  mask:(not null x) & not null y;
  xf:x where mask; yf:y where mask;
  n:count xf;
  if[n < J * K; :([] xbin:til J; localMI:J#0n; margX:J#0n; avgY:J#0n; cnt:J#0)];
  bx:qbin[J;xf]; by:qbin[K;yf];
  // Build J×K joint count matrix
  jnt:(J;K)#(J * K)#0;
  i:0; while[i < n;
    jnt[bx i;by i]+:1;
  ...
  j:0; while[j < J;
    if[nj[j] > 0;
      ...
      lmi[j]:s];
    j+:1];
  // Conditional mean of Y per bin
  aY:J#0n;
  j:0; while[j < J;
    idx:where bx = j;
    if[0 < count idx; aY[j]:avg yf idx];
    j+:1];
  ([] xbin:til J; localMI:lmi; margX:nj % n; avgY:aY; cnt:nj)}
...
ridgerFast:{[n;X;y;lam]
    nObs:count y;
    X:`float$X; y:`float$y;  // Ensure float for matrix ops
    p:count first X;
...
// Rolling Ridge with intercept - adds column of 1s and doesn't regularize intercept
ridgerFastIntercept:{[n;X;y;lam]
    nObs:count y;
    X:`float$X; y:`float$y;  // Ensure float for matrix ops
    X1:{(1f,x)} each X;  // Add intercept column
    p:count first X1;
...
// Rolling Ridge with intercept - parallel version
ridgerFastInterceptParallel:{[n;X;y;lam]
    nObs:count y;
    X:`float$X; y:`float$y;  // Ensure float for matrix ops
    X1:{(1f,x)} each X;
    p:count first X1;
...
// Rolling Ridge Helper with intercept
rollingRidgeHelperIntercept:{[xc;ycol;w;lam;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    betas:ridgerFastIntercept[w;X;y;lam];  // Returns [intercept, b1, b2, ...] per window
    // Expecting nf+1 coefficients (intercept + features)
    cleanBeta:{[nf;b]
    ...
    g,'flip (enlist[`b_intercept]!enlist intercepts),bcols,'(enlist`yhat)!enlist pred}

// Rolling Ridge Helper with intercept - parallel version
rollingRidgeHelperInterceptParallel:{[xc;ycol;w;lam;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    betas:ridgerFastInterceptParallel[w;X;y;lam];
    cleanBeta:{[nf;b]
        $[(count b)<>nf+1; (nf+1)#0n;
          any null b; (nf+1)#0n;
          any (b=0w) or b=-0w; (nf+1)#0n;
...
// The intercept term is NOT regularized
// @param t      - table (must be sorted by bycol, time)
// @param bycol  - symbol: column to group by (e.g. `sym)
// @param xc     - list of symbols: feature columns
// @param ycol   - symbol: target column
// @param window - integer: rolling window size
// @param lam    - float: ridge regularization parameter (applied to features only, not intercept)
rollingRidgeTableIntercept:{[t;bycol;xc;ycol;window;lam]
    ...
    result:$[nGroups >= 4;
        // Many groups: parallelize across groups
        raze rollingRidgeHelperIntercept[xc;ycol;window;lam] peach groups;
        // Few groups: parallelize within each group
...
// @param n - window size
// @param X - list of feature vectors (each row is one observation's features)
// @param y - target vector
// @param lam - ridge regularization parameter
// @return list of (beta; mu; sigma) tuples for each position
ridgerInterceptZscore:{[n;X;y;lam]
    nObs:count y;
    X:`float$X; y:`float$y;
    p:count first X;
    idx:swin[n;til nObs];
    computeWindow:{[X;y;lam;p;i]
        if[any null i; :((p+1)#0n; p#0n; p#0n)];
        Xw:X i;
        yw:y i;
        // Z-score features within window
        Xflip:flip Xw;
        mu:avg each Xflip;
        sigma:{$[0<x;x;1e-10]} each dev each Xflip;
        Xz:flip (Xflip - mu) % sigma;
        ...
        reg:lam * eye p+1;
        reg[0;0]:0f;
        beta:.[{(minv x) mmu y};(XtX + reg; Xty);{(p+1)#0n}];
        (beta; mu; sigma)
    }[X;y;lam;p];
    computeWindow each idx}

// Parallel version of ridgerInterceptZscore
ridgerInterceptZscoreParallel:{[n;X;y;lam]
    nObs:count y;
    X:`float$X; y:`float$y;
    p:count first X;
    idx:swin[n;til nObs];
    ...
    computeWindow peach idx}

// Rolling Ridge Helper with intercept and z-scoring
// Betas are in z-scored feature space
// Predictions use previous window's normalization params (no lookahead)
rollingRidgeHelperInterceptZscore:{[xc;ycol;w;lam;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    results:ridgerInterceptZscore[w;X;y;lam];
    prevResults:prev results;
    // Prediction: z-score current X using prev window's mu/sigma, then apply prev betas
    pred:{[nf;x;r]
        if[not 3=count r; :0n];
        beta:r 0; mu:r 1; sigma:r 2;
        if[(count beta)<>nf+1; :0n];
        if[any null beta; :0n];
        if[(count mu)<>nf; :0n];
        if[any null mu; :0n];
        xz:(x - mu) % sigma;
        beta[0] + sum xz * 1_ beta
    }[nf]'[X;prevResults];
    // Extract components for output
    betas:results[;0];
    cleanBeta:{[nf;b] $[(count b)<>nf+1; (nf+1)#0n; b]}[nf] each betas;
    intercepts:cleanBeta[;0];
    featBetas:1_/:cleanBeta;
    bcols:(`$"b_",/:string xc)!flip featBetas;
    g,'flip (enlist[`b_intercept]!enlist intercepts),bcols,'(enlist`yhat)!enlist pred}

// Parallel version of rolling ridge helper with z-scoring
rollingRidgeHelperInterceptZscoreParallel:{[xc;ycol;w;lam;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    results:ridgerInterceptZscoreParallel[w;X;y;lam];
    prevResults:prev results;
    pred:{[nf;x;r]
        if[not 3=count r; :0n];
        beta:r 0; mu:r 1; sigma:r 2;
...
// Rolling ridge regression with intercept AND z-scoring by group
// Features are z-scored within each rolling window before fitting
// This handles non-stationarity and ensures equal ridge penalty across features
// @param t      - table (must be sorted by bycol, time)
// @param bycol  - symbol: column to group by (e.g. `sym)
// @param xc     - list of symbols: feature columns
// @param ycol   - symbol: target column
// @param window - integer: rolling window size
// @param lam    - float: ridge regularization parameter
// @return original table + b_intercept + b_<feature> columns + yhat
//         Note: betas are in z-scored feature space
...
// PCA via power iteration (no QML dependency)
// Computes top k principal components of feature matrix X
// @param X - matrix (rows=observations, cols=features)
// @param k - number of components to extract
// @return (loadings; scores; explainedVar; mu; sigma)
pcaSimple:{[X;k]
    X:`float$X;
    n:count X;
    p:count first X;
    k:k&p;
    // Center and scale (z-score each column)
    mu:avg each flip X;
    sigma:{$[0<x;x;1e-10]} each dev each flip X;
    Xz:flip ((flip X) - mu) % sigma;
    // Covariance matrix
    C:((flip Xz) mmu Xz) % n-1;
    // Power iteration for top eigenvector
    getTopEigenvec:{[C;nIter]
        p:count C;
        v:p?1.0;
        v:v % sqrt sum v*v;
        do[nIter; v2:C mmu v; v:v2 % sqrt sum v2*v2];
        eigenval:sum v * C mmu v;
        (eigenval; v)};
    // Extract k components via deflation
    loadings:();
    eigvals:();
    Cwork:C;
    do[k;
        res:getTopEigenvec[Cwork;100];
        eigvals,:res 0;
        loadings,:enlist res 1;
        Cwork:Cwork - (res 0) * (res 1) */:\: res 1];
    loadings:flip loadings;
    totalVar:sum {x[y;y]}[C] each til p;
    expVar:eigvals % totalVar;
    scores:Xz mmu loadings;
    (loadings; scores; expVar; mu; sigma)}

// Rolling Ridge with PCA dimensionality reduction
// @param n - window size
// @param X - feature matrix (list of row vectors)
// @param y - target vector
// @param lam - ridge lambda
// @param nComp - number of PCA components to keep
ridgerPCA:{[n;X;y;lam;nComp]
    nObs:count y;
    X:`float$X; y:`float$y;
    p:count first X;
    nComp:nComp & p;
    idx:swin[n;til nObs];
    computeWindow:{[X;y;lam;p;nComp;i]
        if[any null i; :((nComp+1)#0n; (p;nComp)#0n; p#0n; p#0n)];
        Xw:X i;
        yw:y i;
        // PCA on window features
        pcaRes:pcaSimple[Xw;nComp];
        loadings:pcaRes 0;
        scores:pcaRes 1;
        mu:pcaRes 3;
        sigma:pcaRes 4;
        // Fit ridge on PC scores
        X1:{(1f,x)} each scores;
        XtX:(flip X1) mmu X1;
        Xty:(flip X1) mmu yw;
        reg:lam * eye nComp+1;
        reg[0;0]:0f;
        beta:.[{(minv x) mmu y};(XtX + reg; Xty);{(1+x)#0n}[nComp]];
        (beta; loadings; mu; sigma)
    }[X;y;lam;p;nComp];
    computeWindow each idx}

// Rolling Ridge Helper with PCA
rollingRidgeHelperPCA:{[xc;ycol;w;lam;nComp;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    results:ridgerPCA[w;X;y;lam;nComp];
    prevResults:prev results;
    pred:{[nf;nComp;x;r]
        if[not 4=count r; :0n];
        beta:r 0; loadings:r 1; mu:r 2; sigma:r 3;
        if[(count beta)<>nComp+1; :0n];
        if[any null beta; :0n];
        if[(count mu)<>nf; :0n];
        xz:(x - mu) % sigma;
        pc:xz mmu loadings;
        beta[0] + sum pc * 1_ beta
    }[nf;nComp]'[X;prevResults];
    betas:results[;0];
    cleanBeta:{[nc;b] $[(count b)<>nc+1; (nc+1)#0n; b]}[nComp] each betas;
    intercepts:cleanBeta[;0];
    pcBetas:1_/:cleanBeta;
    bcols:(`$"b_PC",/:string 1+til nComp)!flip pcBetas;
    g,'flip (enlist[`b_intercept]!enlist intercepts),bcols,'(enlist`yhat)!enlist pred}

// Rolling ridge regression with PCA dimensionality reduction
// Reduces features to principal components within each window before fitting
// Helps with multicollinearity and produces more stable betas
// @param t      - table (must be sorted by bycol, time)
// @param bycol  - symbol: column to group by
// @param xc     - list of symbols: feature columns
// @param ycol   - symbol: target column
// @param window - integer: rolling window size
// @param lam    - float: ridge regularization parameter
// @param nComp  - integer: number of PCA components to keep
// @return original table + b_intercept + b_PC1..b_PCn columns + yhat
...
// Classify observations into volatility regimes
// Uses rolling std of target, classifies as high/low relative to expanding median
// @param y - target series
// @param volWindow - window for volatility calculation
// @return vector of regime labels (0=low vol, 1=high vol)
classifyVolRegime:{[y;volWindow]
    y:`float$y;
    vol:mdev[volWindow;y];
    n:count y;
    // Expanding median (no lookahead)
    expMedian:{[v;i] $[i<2; 0n; med v til i]}[vol] each til n;
    regime:`int$(vol > expMedian);
    regime[til volWindow]:0Ni;
    regime}

// Rolling Ridge with separate models per volatility regime
ridgerRegime:{[n;X;y;lam;volWindow]
    nObs:count y;
    X:`float$X; y:`float$y;
    p:count first X;
    regimes:classifyVolRegime[y;volWindow];
    idx:swin[n;til nObs];
    // Fit ridge on subset
    fitSubset:{[Xw;yw;idx;lam;p]
        if[5>count idx; :((p+1)#0n; p#0n; p#0n)];
        Xr:Xw idx;
        yr:yw idx;
        Xflip:flip Xr;
        mu:avg each Xflip;
        ...
        reg:lam * eye p+1;
        reg[0;0]:0f;
        beta:.[{(minv x) mmu y};(XtX + reg; Xty);{(1+x)#0n}[p]];
        (beta; mu; sigma)};
    computeWindow:{[X;y;regimes;lam;p;fitSubset;i]
        if[any null i; :((p+1)#0n; (p+1)#0n; 0Ni; p#0n; p#0n; p#0n; p#0n)];
        Xw:X i;
        yw:y i;
        rw:regimes i;
        currRegime:last rw;
        idx0:where rw=0;
        idx1:where rw=1;
        res0:fitSubset[Xw;yw;idx0;lam;p];
        res1:fitSubset[Xw;yw;idx1;lam;p];
        (res0 0; res1 0; currRegime; res0 1; res0 2; res1 1; res1 2)
    }[X;y;regimes;lam;p;fitSubset];
    results:computeWindow each idx;
    (results; regimes)}

// Rolling Ridge Helper with regime switching
rollingRidgeHelperRegime:{[xc;ycol;w;lam;volWindow;g]
    X:flip g xc;
    y:g ycol;
    nf:count xc;
    res:ridgerRegime[w;X;y;lam;volWindow];
    results:res 0;
    regimes:res 1;
    prevResults:prev results;
    pred:{[nf;x;regime;r]
        if[not 7=count r; :0n];
        beta0:r 0; beta1:r 1;
        mu0:r 3; sigma0:r 4; mu1:r 5; sigma1:r 6;
        if[null regime; :0n];
        $[regime=0;
            [if[(count beta0)<>nf+1; :0n];
             if[any null beta0; :0n];
             if[any null mu0; :0n];
             xz:(x - mu0) % sigma0;
             beta0[0] + sum xz * 1_ beta0];
            [if[(count beta1)<>nf+1; :0n];
             if[any null beta1; :0n];
             if[any null mu1; :0n];
             xz:(x - mu1) % sigma1;
             beta1[0] + sum xz * 1_ beta1]]
    }[nf]'[X;regimes;prevResults];
    betas0:results[;0];
    betas1:results[;1];
    cleanBeta:{[nf;b] $[(count b)<>nf+1; (nf+1)#0n; b]}[nf];
    betas0Clean:cleanBeta each betas0;
    betas1Clean:cleanBeta each betas1;
    intLo:betas0Clean[;0];
    intHi:betas1Clean[;0];
    featBetasLo:1_/:betas0Clean;
    featBetasHi:1_/:betas1Clean;
    bcolsLo:(`$"b_",/:string[xc],\:"_loVol")!flip featBetasLo;
    bcolsHi:(`$"b_",/:string[xc],\:"_hiVol")!flip featBetasHi;
    regimeCol:(enlist`volRegime)!enlist regimes;
    intCols:`b_int_loVol`b_int_hiVol!(intLo;intHi);
    g,'flip regimeCol,intCols,bcolsLo,bcolsHi,'(enlist`yhat)!enlist pred}

// Rolling ridge regression with separate models per volatility regime
// Fits separate models for high-vol and low-vol periods
// @param t         - table (must be sorted by bycol, time)
// @param bycol     - symbol: column to group by
// @param xc        - list of symbols: feature columns
// @param ycol      - symbol: target column
// @param window    - integer: rolling window size for regression
// @param lam       - float: ridge regularization parameter
// @param volWindow - integer: window for volatility calculation
// @return original table + volRegime + betas for both regimes + yhat
...
rollingRidgeN:{[t;bycol;retcol;xc;N;window;lam]
    ...
    // Recompute yhat = sum(lagged_betas * features) for all rows
    // CRITICAL: betas[k] uses fwdRetN[k] = ret[k+1..k+N], which is known at time k+N
    // So at time t, the most recent USABLE beta is betas[t-N-1]
    // Using betas[t-1] would be look-ahead bias (fwdRetN[t-1] requires ret up to t+N-1)
    // Lag by N+1 to ensure no future data is used
    lagN:N+1;
    // First lag the betas, then multiply by features
    laggedBcols:`$"__lag_",/:string xc;
    t:![t; (); by; laggedBcols!{(xprev;x;y)}'[lagN;bcols]];
    // Now compute yhat as sum of lagged_beta * feature
    terms:{(*;x;y)}'[laggedBcols;xc];
    yexpr:terms 0;
...
bocpd:{[x;hazard;mu0;kappa0;alpha0;beta0]
    ...
    i:0; while[i < T;
        ...
        i+:1];
    // cpScore: how many times higher than baseline (1/hazard) is cpProb
    cpScore:cpP % H;
    // cpDetect: 1 when runLength drops significantly (new regime detected)
    rlPrev:prev rl;
    cpDetect:(rl < 5i) and (rlPrev > 10i);
    `cpProb`cpScore`runLength`cpDetect`postMean`postVar!(cpP;cpScore;rl;cpDetect;pMu;pVar)}
