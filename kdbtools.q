...
// Fast MSE split finder using cumulative sums
...
bestSplitFeatFast:{[x;y;minLeaf]
    ...
    xs:x ord;
    ys:y ord;
    // Find split points (where consecutive x values differ)
    diffs:differ xs;
    splitIdx:where diffs;
    if[0=count splitIdx; :(0n;0w)];
    // Filter by minLeaf constraint
    validIdx:splitIdx where (splitIdx >= minLeaf) & (splitIdx <= n - minLeaf);
    if[0=count validIdx; :(0n;0w)];
    // Cumulative sums for O(1) variance calculation
    cumSum:sums ys;
    cumSumSq:sums ys * ys;
    totalSum:last cumSum;
    totalSumSq:last cumSumSq;
    // Compute weighted MSE for each valid split
    // Left: indices 0..i-1, Right: indices i..n-1
    // var(y) = E[y²] - E[y]² = sumSq/n - (sum/n)²
    calcMSE:{[cumSum;cumSumSq;totalSum;totalSumSq;n;i]
        sumL:cumSum i-1;
        sumSqL:cumSumSq i-1;
        nL:i;
        nR:n - i;
        ...
        varL:(sumSqL % nL) - (sumL % nL) xexp 2;
        varR:(sumSqR % nR) - (sumR % nR) xexp 2;
        (nL * varL) + nR * varR
    }[cumSum;cumSumSq;totalSum;totalSumSq;n];
    mseScores:calcMSE each validIdx;
    bestLocalIdx:mseScores?min mseScores;
    bestSplitIdx:validIdx bestLocalIdx;
    // Threshold is midpoint between xs[bestSplitIdx-1] and xs[bestSplitIdx]
    thresh:0.5 * xs[bestSplitIdx-1] + xs bestSplitIdx;
    (thresh; mseScores bestLocalIdx)}

// Fast Gini split finder for classification using cumulative class counts
bestSplitFeatFastCls:{[x;y;minLeaf]
    ...
    validIdx:splitIdx where (splitIdx >= minLeaf) & (splitIdx <= n - minLeaf);
    if[0=count validIdx; :(0n;0w)];
    // Build cumulative class count matrix for O(1) Gini calculation
    classes:asc distinct ys;
    k:count classes;
    // One-hot encode and cumsum
    classIdx:classes?ys;
    cumCounts:(sums each (til k) =\: classIdx);  // k x n matrix of cumulative counts
    totalCounts:cumCounts[;n-1];
    // Compute Gini at each split using cumulative counts
    // gini = 1 - sum(p_i^2) where p_i = count_i / n
    calcGiniFromCounts:{[cumCounts;totalCounts;n;k;i]
        countsL:cumCounts[;i-1];
        ...
        gR:1 - sum pR*pR;
        ((nL%n)*gL) + (nR%n)*gR
    }[cumCounts;totalCounts;n;k];
    giniScores:calcGiniFromCounts each validIdx;
    bestLocalIdx:giniScores?min giniScores;
    bestSplitIdx:validIdx bestLocalIdx;
    thresh:0.5 * xs[bestSplitIdx-1] + xs bestSplitIdx;
    (thresh; giniScores bestLocalIdx)}
...
// Predict using fitted tree model
dtPredict:{[model;X]
...
gbFit:{[X;y;nTrees;maxDepth;minLeaf;lr]
    ...
    init:avg y;
    preds:n#init;
    // Pre-allocate tree list (avoids O(n²) concatenation)
    trees:nTrees#enlist (::);
    idx:til n;
    i:0;
    do[nTrees;
        ...
        treePreds:dtPredictOne[tree] each X;
        preds+:lr * treePreds;
        trees[i]:treeModel;
        i+:1
    ];
...
dtPredictBatch:{[trees;X]
    {[tree;X] dtPredictOne[tree`tree] each X}[;X] each trees}

// Predict with gradient boosting regression model (optimized)
gbPredict:{[model;X]
    init:model`init;
    lr:model`lr;
    // Vectorized: accumulate predictions as we go
    n:count X;
    preds:n#init;
    {[X;lr;preds;tree]
        preds + lr * dtPredictOne[tree`tree] each X
...
gbcFit:{[X;y;nTrees;maxDepth;minLeaf;lr]
    ...
    do[nTrees;
        prob:sigmoid F;
        // Negative gradient of log-loss = y - prob
        negGrad:y - prob;
...
gbcFitNewton:{[X;y;nTrees;maxDepth;minLeaf;lr]
    ...
    do[nTrees;
        ...
        grad:y - prob;
        hess:prob * 1 - prob;
        // Build tree structure on gradients
        tree:dtBuildIdx[X;grad;idx;maxDepth;minLeaf;`reg];
        // Recompute leaf values using Newton step: sum(grad)/sum(hess)
...
gbcPredictProb:{[model;X]
    ...
    F:n#init;
    F:{[X;lr;F;tree]
        F + lr * dtPredictOne[tree`tree] each X
    }[X;lr]/[F;model`trees];
    sigmoid F}
...
gbmcPredictProb:{[model;X]
    ...
    F:{[X;lr;K;F;roundTrees]
        // For each class, add tree predictions to F
        newF:F;
        k:0;
        do[K;
            treePreds:dtPredictOne[roundTrees[k]`tree] each X;
