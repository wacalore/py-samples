...
msRegInit:{[X;y;lam]
    p:count first X;
    bAll:ridge[X;y;lam];
    resid:y - first (enlist bAll) mmu flip X;
    absR:abs resid;
    medR:med absR;
    ...
    if[(count[idx0]<2*p) or count[idx1]<2*p;
        s0:dev y[idx0]; s1:dev y[idx1];
        :`b0`b1`s0`s1`p00`p11!(bAll;bAll;s0|1e-6;s1|1e-6;0.95;0.95)];
    b0:ridge[X idx0;y idx0;lam];
    b1:ridge[X idx1;y idx1;lam];
    r0:y[idx0] - first (enlist b0) mmu flip X idx0;
    r1:y[idx1] - first (enlist b1) mmu flip X idx1;
    s0:sqrt avg r0*r0;
    s1:sqrt avg r1*r1;
...
msRegEM:{[X;y;lam;maxIter;tol;params0]
    ...
    while[(iter<maxIter) and not converged;
        ...
        prevLL:ll;
        if[not converged;
            // M-step: WLS betas (sqrt-weighting)
            sqw0:sqrt fp0|1e-10; sqw1:sqrt fp1|1e-10;
            b0:ridge[sqw0*'X;sqw0*y;lam];
            b1:ridge[sqw1*'X;sqw1*y;lam];
            // M-step: sigmas (mmu for speed)
            r0:y - first (enlist b0) mmu Xt;
            r1:y - first (enlist b1) mmu Xt;
            s0:sqrt(sum[fp0*r0*r0]%sum fp0)|1e-6;
            s1:sqrt(sum[fp1*r1*r1]%sum fp1)|1e-6;
            // M-step: transition probs
            pfp0:neg[1]_ fp0; nfp0:1_ fp0;
            pfp1:neg[1]_ fp1; nfp1:1_ fp1;
