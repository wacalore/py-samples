...
// Prepare table for multi-period rolling ridge regression
// Adds forward N-period return column and lags features by N-1
// to eliminate lookahead bias in training data
// @param t      - table (must be sorted by bycol, time)
// @param bycol  - group column (e.g. `sym)
// @param retcol - return column (simple returns)
// @param xc     - list of feature columns to lag
// @param N      - forward return horizon (periods)
// @return table with `fwdRetN target column and lagged features
fwdTarget:{[t;bycol;retcol;xc;N]
    bc:$[11h=type bycol; bycol; enlist bycol];
    by:bc!bc;
    t:![t; (); by; (enlist `fwdRetN)!enlist (`.kdbtools.fwdReturn;N;retcol)];
    if[N > 1; t:![t; (); by; xc!{(xprev;x;y)}[N-1] each xc]];
    t}
