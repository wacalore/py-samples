...
alphaOptimize:{[alphas;R;p;pcLimits;cfg]
    ...
    Rmat:0f^Rmat;  // fill nulls with 0 to prevent mmu propagation
    ...
    // Pad with null (unconstrained) if fewer limits than PCs — do NOT cycle
    padK:{[k;v] @[k#0n; til min(k;count v); :; v]};
    pcLo:pcHi:k#0n;
    $[(99h=type pcLimits) & 0=count pcLimits;
        ::;  // ()!() - no constraints
      (99h=type pcLimits) & 0<count pcLimits;
        [lo:pcLimits`lo; hi:pcLimits`hi;
         if[0<count lo; pcLo:padK[k;lo]]; if[0<count hi; pcHi:padK[k;hi]]];
      -9h=type pcLimits;
    ...
    while[i < nIter;
        ...
        score:$[objective=`maxSharpe; (ret - cfg`rf) % vol + 1e-10;
                ...
                (ret - cfg`rf) % vol + 1e-10];  // default to Sharpe
...
alphaProjectPC:{[alloc;alphaPCExp;pcLo;pcHi;maxIter]
    ...
    // Short-circuit: if no constraints, return immediately
    if[all null pcLo,pcHi; :alloc];
