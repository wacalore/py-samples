...
alphaOptimize:{[alphas;R;p;pcLimits;cfg]
    ...
    pcLo:pcHi:k#0n;
    $[99h=type pcLimits;
        [pcLo:k#pcLimits`lo; pcHi:k#pcLimits`hi];
      -9h=type pcLimits;
...
alphaListOptimize:{[alphaList;alphaNames;pcLimits;cfg]
    defaults:`weightMethod`sigCol`retCol`symCol`dtCol`prevSigCol`k`objective`nIter!(
        `avg;`sig;`pxDiff;`sym;`dt;`prevSig;0;`maxSharpe;5000);
    cfg:defaults,cfg;

    n:count alphaList;
    if[n<>count alphaNames; '"alphaList and alphaNames must have same length"];

    // Get all unique symbols across all alphas (hardcoded to `sym column for simplicity)
    allSyms:asc distinct raze {distinct x`sym} each alphaList;

    // Compute characteristic weights for each alpha (uses `sig and `sym columns)
    computeWeights:{[t;allSyms] w:exec avg sig by sym from t; w:allSyms#(allSyms!count[allSyms]#0f),w; w%sum abs w};
    charWeights:computeWeights[;allSyms] each alphaList;
    alphas:alphaNames!charWeights;

    // Compute alpha returns time series with proper column names
    // Using functional select to name the column after the alpha
    computeAlphaRet:{[t;nm] ?[t;();(enlist`dt)!enlist`dt;(enlist nm)!enlist(sum;(*;`prevSig;`pxDiff))]};
    alphaRetTables:computeAlphaRet'[alphaList;alphaNames];

    // Build asset returns: pivot pxDiff by sym
    combined:raze alphaList;
    assetRetLong:0!select last pxDiff by dt, sym from combined;  // unkey

    // Pivot to wide format
    allDates:asc distinct assetRetLong`dt;
    R:exec allSyms#(sym!pxDiff) by dt from assetRetLong;
    // Convert keyed dict to table
    keyDates:key R;
    valMatrix:value each value R;
    R:([] dt:keyDates);
    R:R,'flip allSyms!flip valMatrix;
    ...
    // Add alpha return stats to result
    // alphaRetTables are keyed tables with correct column names
    alphaRetWide:([] dt:allDates);
...
alphaListInfo:{[alphaList;alphaNames]
    ...
    {[t;nm;i]
        ...
        -1 "  Symbols: ",", " sv string asc distinct t`sym;
        -1 "  Dates:   ",string[min t`dt]," to ",string max t`dt;
    }'[alphaList;alphaNames;til count alphaList];
    -1 "";}
...
usage:{[]
    ...
    -1 "  alphaListInfo[alphaList;names]                  - Display alpha table info";
