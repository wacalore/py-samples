...
// oisRateTable: ([] date; tenors; rates) - long format table
//   date: valuation date
//   tenors: symbol like `1D`1W`1M`3M`6M`1Y`2Y...
//   rates: OIS par rate
// Returns dict: date -> oisCurve
buildOISCurves:{[oisRateTable]
    dates:asc distinct oisRateTable`date;

    // Build curve for each date
    buildOne:{[tbl;dt]
        // Get rates for this date, sorted by tenor
        dayRates:select from tbl where date=dt;
        // Convert tenors to year fractions and sort
        yf:.swaps.tenorsToYF dayRates`tenors;
        ord:iasc yf;
        yf:yf ord;
        rates:(dayRates`rates) ord;
        // Bootstrap
        oisBootstrap[yf;rates;dt]
    }[oisRateTable;];

    dates!buildOne each dates}
...
exampleOIS:{[]
    ...
    -1 "Step 1: Create OIS Rate Table (long format)";
    -1 "";

    // Build rate table: ([] date; tenors; rates)
    // Each date gets all tenors with their rates
    oisRateTable:raze {[d;t;r] ([] date:count[t]#d; tenors:t; rates:r)}[;tenors;baseRates] each dates;

    -1 "  OIS Rate Table (first 10 rows):";
    show 10#oisRateTable;
    -1 "  ...";
    -1 "  Total rows: ",string count oisRateTable;
    -1 "";
