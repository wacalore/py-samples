...
switchPointsMarket:{[settleDate;deliveryDate;futuresPrice;bonds;repoRate]
    ...
    result:{[s;deliv;fp;repo;ctdDV01;ctdCF;ctdNB;row]
        ...
        // Net basis sensitivity differential (futures-equivalent DV01 difference)
        // Positive means bond duration > CTD duration
        nbSensDiff:(bondDV01 % bond`cf) - (ctdDV01 % ctdCF);
        ...
        switchRate:$[abs[nbSensDiff] > 0.001; neg nbGap % nbSensDiff; 0n];
        `sym`switchBps`netBasis`impliedRepo`bond!(row`sym;switchRate * 10000;bondNB;bondIR;bond)
...
// Option-adjusted invoice spread using MARKET prices
// Uses implied repo ranking and includes timing option
...
// Returns standard spread, option breakdown (quality + timing), and OAS
oasInvoiceSpreadMarket:{[curve;delivery;futuresPrice;ctdBond;basket;repoRate;vol;freq]
    ...
    `invoiceYield`swapRate`invoiceSpread`spreadBps`qualityOption`timingOption`totalOption`optionAdjBps`oas`oasBps`netBasis!(
        invYld;
        swpRt;
        spread;
        spreadBps;
...
// Full OAS analysis using MARKET prices
...
oasAnalysisMarket:{[curve;delivery;futuresPrice;basket;repoRate;vol;freq]
    ...
    ctdIR:(first ranked)`impliedRepo;
    // Calculate OAS
    oas:oasInvoiceSpreadMarket[curve;delivery;futuresPrice;ctdBond;basket;repoRate;vol;freq];
    // Switch points using market prices
    switches:switchPointsMarket[settleDate;lastDelivery;futuresPrice;basket;repoRate];
    // Package results
    `oas`ctdBond`ctdImpliedRepo`ranking`switchPoints`vol`settleDate`firstDelivery`lastDelivery!(
        oas;ctdBond;ctdIR;ranked;switches;vol;settleDate;firstDelivery;lastDelivery)}
...
// basketTable: table with date, deliveryCode, sym, coupon, maturity, cf (all deliverable bonds)
// bondPx: ([] date; sym; cleanPrice) - daily bond prices, joined to basket
//         OR (::) to use cleanPrice from basketTable directly
...
dailyOASFromCTD:{[ctdTable;basketTable;bondPx;curves;repoRate;vol;freq]
    ...
    // Join bondPx to basketTable if provided
    basket:$[(bondPx~(::)) or (0=count bondPx);
        // No bondPx - use basketTable as-is (must have cleanPrice)
        basketTable;
        // Join bondPx to basketTable on (date, sym)
        basketTable lj `date`sym xkey select date, sym, cleanPrice from bondPx];
    ...
    curveType:$[99h=type curves;
                    // Dict - check if keys are dates or curve keys
                    $[all (key curves) in dates; `dateDict; `singleCurve];
                (type curves) in 100 104h; `builder;
                98h=type curves; `table;
                `singleCurve];
    ...
    hasBasketCleanPrice:`cleanPrice in cols basket;

    // Pack context
    ctx:`curvesByDate`basketTable`freq`hasCtdCleanPrice`hasBasketCleanPrice`isOIS!(
        curvesByDate;basket;frq;hasCtdCleanPrice;hasBasketCleanPrice;isOIS);
