...
invoiceYieldStreet:{[settleDate;maturityDate;coupon;cleanPrice]
    // Generate coupon dates (May 15 and Nov 15 for Treasuries)
    // Determine coupon months from maturity date
    matMonth:`mm$maturityDate;
    matDay:`dd$maturityDate;
    cpnMonths:$[matMonth in 1 2 3 4 5 6; (5;11); (5;11)];  // Simplify: assume 5/15 and 11/15
    if[matMonth in 7 8 9 10 11 12; cpnMonths:(5;11)];

    // Find previous and next coupon dates
    matYear:`year$maturityDate;
    settleYear:`year$settleDate;

    // Generate all possible coupon dates from settle year to maturity
    years:settleYear + til 1 + matYear - settleYear;
    allCpnDates:raze {[y;m;d] ("D"$string[y],".05.",string[d];"D"$string[y],".11.",string[d])}[;cpnMonths 0;matDay] each years;
    allCpnDates:asc distinct allCpnDates;
...
oisBootstrap:{[tenors;parRates;curveDate]
...
// Convenience: build OIS curves for a date range
// Returns dict: date -> oisCurve
buildOISCurves:{[tenors;oisRates;dates]
    rateType:$[
        (type oisRates) in 9 -9h; `static;
        99h=type oisRates; `dict;
        98h=type oisRates; `table;
        `static];

    $[rateType=`static;
        dates!{[t;r;d] oisBootstrap[t;r;d]}[tenors;oisRates;] each dates;
      rateType=`dict;
        dates!{[t;r;d] oisBootstrap[t;r d;d]}[tenors;oisRates;] each dates;
      rateType=`table;
        rateCols:(cols oisRates) except `date;
        ratesByDate:exec date!flip rateCols#/:oisRates from oisRates;
        dates!{[t;r;d] oisBootstrap[t;r d;d]}[tenors;ratesByDate;] each dates;
      dates!{[t;r;d] oisBootstrap[t;r;d]}[tenors;oisRates;] each dates]}
...
exampleOIS:{[]
    ...
    // Define OIS tenors (28 tenors from 1D to 10Y)
    tenors:`1D`1W`2W`3W`1M`2M`3M`4M`5M`6M`7M`8M`9M`10M`11M`1Y`1.25Y`1.5Y`1.75Y`2Y`3Y`4Y`5Y`6Y`7Y`8Y`9Y`10Y;

    // Sample OIS rates (USD SOFR par rates)
    baseRates:0.0364 0.0367 0.0369 0.0368 0.0367 0.0368 0.0367 0.0366 0.0365 0.0364 0.0362 0.036 0.0358 0.0356 0.0354 0.0353 0.0347 0.0344 0.0343 0.0343 0.0346 0.0351 0.0357 0.0364 0.0371 0.0377 0.0383 0.0389;

    // Create sample dates
    dates:("D"$"2026.01.21") + til 5;

    -1 "Step 1: Define OIS Curve Inputs";
    -1 "  Tenors: ",", " sv string 10#tenors;
    -1 "  (28 tenors total from 1D to 10Y)";
    -1 "";
    ...
    -1 "Step 3: Build OIS Curves for Each Date";
    -1 "  oisCurves:.invoicepricer.buildOISCurves[tenors;rates;dates]";
    -1 "";

    // Build curves (using static rates - same for all dates in this example)
    oisCurves:buildOISCurves[tenors;baseRates;dates];
    ...
    -1 "Workflow:";
    -1 "  1. Build OIS curves: oisCurves:.invoicepricer.buildOISCurves[tenors;rates;dates]";
...
usage:{[]
    ...
    -1 ".invoicepricer.buildOISCurves[tenors;oisRates;dates]";
    -1 "    // tenors: `1D`1W`1M`3M`6M`1Y`2Y...  oisRates: static vector or dict (date->rates)";
