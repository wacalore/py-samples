...
simAtIdx:{[cfg;prices;i] px:prices til i+1; p:$[`vol in key cfg;cfg;cfg,enlist[`vol]!enlist dev 1_deltas px]; r:simSignalFast[px;cfg`sigFn;cfg`nPaths;cfg`nSteps;cfg`method;p]; `currentSig`mean`std`p5`p50`p95`probUp`probCross0!(r`currentSig;last r`meanByStep;last r`stdByStep;last r`p5;last r`p50;last r`p95;r`probUp;r`probCross0)}

// Helper: run sim for one sym's data (parallel over dates if cfg`parallel)
simOneSym:{[cfg;grp] dts:grp cfg`dt; prices:grp cfg`price; n:count prices; mh:cfg`minHist; idxs:mh+til n-mh; res:$[cfg`parallel; simAtIdx[cfg;prices;] peach idxs; simAtIdx[cfg;prices;] each idxs]; ([] dt:dts idxs; currentSig:res@\:`currentSig; mean:res@\:`mean; std:res@\:`std; p5:res@\:`p5; p50:res@\:`p50; p95:res@\:`p95; probUp:res@\:`probUp; probCross0:res@\:`probCross0)}
