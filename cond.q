...
simRolling:{[t;cfg] cfg:$[`parallel in key cfg;cfg;cfg,enlist[`parallel]!enlist 0b]; bySym:t group t cfg`sym; f:{[cfg;s;g] r:simOneSym[cfg;g]; update sym:s from r}[cfg]; results:$[cfg`parallel; f peach'[key bySym;value bySym]; f'[key bySym;value bySym]]; raze results}
