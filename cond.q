...
simRolling:{[t;cfg] cfg:$[`parallel in key cfg;cfg;cfg,enlist[`parallel]!enlist 0b]; bySym:t group t cfg`sym; f:{[cfg;s;g] r:simOneSym[cfg;g]; update sym:s from r}[cfg]; pairs:flip (key bySym;value bySym); results:$[cfg`parallel; {[f;p] f[p 0;p 1]}[f;] peach pairs; f'[key bySym;value bySym]]; raze results}
