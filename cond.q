...
simSignalChangeND:{[x;sigFn;nPaths;nSteps;method;params]
    ...
    // Generate multi-step paths
    paths:simPathsND[x;nPaths;nSteps;method;params];

    // Apply signal to each path at each step
    // pathSigs[i;j] = signal value for path i at step j
    // For each path, compute signal at cumulative extensions: x,path[0]; x,path[0..1]; etc.
    pathSigs:{[sigFn;x;path]
        {[sigFn;x;i;path] last sigFn x,i#path}[sigFn;x;;path] each 1 + til count path
    }[sigFn;x] each paths;

    // Statistics by step (pathSigs is nPaths x nSteps)
    byStep:flip pathSigs;  // transpose to nSteps x nPaths
    ...
    `currentSig`pathSigs`meanByStep`stdByStep`finalMean`finalStd!(
        currentSig;pathSigs;meanByStep;stdByStep;avg finalChanges;dev finalChanges)}
...
// nSteps   - number of forward steps
// method   - `rw, `ar1, or `boot
// params   - simulation params dict (if `vol not provided, estimated per sym)
...
simBySym:{[t;symCol;priceCol;sigFn;nPaths;nSteps;method;params]
    // Get unique syms and extract prices for each
    ...
    // Run simulation for each sym
    runOne:{[sigFn;nPaths;nSteps;method;params;prices] p:$[`vol in key params; params; params,enlist[`vol]!enlist dev 1_deltas prices]; r:simSignalChangeND[prices;sigFn;nPaths;nSteps;method;p]; pctl:simPercentiles[prices;sigFn;nPaths;nSteps;method;p]; finalSigs:last each r`pathSigs; currentSig:r`currentSig; probUp:(sum finalSigs > currentSig) % count finalSigs; probCross0:$[currentSig > 0; sum finalSigs < 0; sum finalSigs > 0] % count finalSigs; `currentSig`finalMean`finalStd`p5`p50`p95`probUp`probCross0!(currentSig;r`finalMean;r`finalStd;last pctl`p5;last pctl`p50;last pctl`p95;probUp;probCross0)};
    results:runOne[sigFn;nPaths;nSteps;method;params;] each priceVecs;
    // Build output table
...
// Flexible simulation from table - all config in dict
// t   - table with sym/date/price columns
// cfg - dict with keys:
//       Required: `sym`price`sigFn`nPaths`nSteps`method
//       Optional: `vol`dist`df`phi`window (for sim params)
// Example:
//   cfg:`sym`price`nPaths`nSteps`method`dist!(`sym;`close;500;10;`rw;`normal);
//   cfg[`sigFn]:{[x] rzscore[60;smooth[x;10]]};
//   .cond.simTable[t;cfg]
simTable:{[t;cfg]
...
// Convenience: simulate EMA z-score by sym
// cfg dict must include: `sym`price`emaHL`zWindow`nPaths`nSteps`method
simEmaZscoreBySym:{[t;cfg] cfg[`sigFn]:{[hl;w;x] rzscore[w;smooth[x;hl]]}[cfg`emaHL;cfg`zWindow]; simTable[t;cfg]}

// Convenience: simulate rolling z-score by sym
