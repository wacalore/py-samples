...
// Breakout-from-compression with trailing stop
// Detects Bollinger bandwidth compression, enters on band breakout,
// trails with volatility-scaled stop. Continuous position sizing tapers as price approaches stop.
// @param bbW - Bollinger band window
// @param bbK - Bollinger band std devs
// @param compPctl - compression threshold (bandwidth percentile below this = compressed)
// @param trailW - volatility window for trailing stop
// @param trailK - trailing stop multiplier (stop = best -/+ trailK * mdev)
// @param x - price series
breakoutTrail:{[bbW;bbK;compPctl;trailW;trailK;x] soBreakoutTrail_[x;bbW;bbK;compPctl;trailW;trailK]}
...
soFmet_:{[sym;ret;rsk;tm;ann;noPrev;sig]
    ...
    mask:(not null ps)&(not null ret)&(ps<>0f);
...
soBreakHold_:{[feat;w;holdN]
    ...
    while[i<n;
        ...
        i+:1];
    pos};
// Breakout-from-compression with trailing stop state machine
// Enters on Bollinger band breakout during bandwidth compression,
// exits via trailing stop with continuous position tapering.
// Compression is checked on the previous bar (the squeeze precedes the breakout).
// States: 0=flat, 1=long, -1=short
soBreakoutTrail_:{[feat;bbW;bbK;compPctl;trailW;trailK]
    n:count feat; if[n=0; :`float$()]; pos:n#0f;
    bbp:(.kdbtools.bbpos[bbW;bbK;]) feat;
    bw:(.kdbtools.bbWidth[bbW;bbK;]) feat;
    bwPctl:rrank[bbW;bw];
    vol:mdev[trailW;feat];
    st:0; best:0n; warmup:1+bbW|trailW; i:warmup;
    while[i<n;
        f:feat i; v:1e-10|vol i; bp:bbp i; wp:bwPctl i-1;
        if[(not null f) and not null bp;
            $[st = 0;
                $[(wp < compPctl) and bp >= 1f;  [st:1;  best:f];
                  (wp < compPctl) and bp <= -1f; [st:-1; best:f];
                  (::)];
              st = 1;
                [best:best|f; stop:best - trailK * v;
                 $[f <= stop; [st:0; best:0n];
                   pos[i]:1f & (f - stop) % trailK * v]];
              // st = -1
                [best:best&f; stop:best + trailK * v;
                 $[f >= stop; [st:0; best:0n];
                   pos[i]:neg 1f & (stop - f) % trailK * v]]]];
        i+:1];
    pos};
...
sigOpt:{[t;featureCol;cfg]
    ...
    // Breakout-from-compression with trailing stop
    g31:{[c;p] (soBreakoutTrail_[;`long$p`bbW;p`bbK;p`compPctl;`long$p`trailW;p`trailK];c`feat) fby c`sym}[ctx;];
    gens:(g1;g2;g3;g4;g5;g6;g7;g8;g9;g10;g11;g12;g13;g14;g15;g16;g17;g18;g19;g20;g21;g22;g23;g24;g25;g26;g27;g28;g29;g30;g31);
    ...
    spaces:(
        ...
        mks[`bbW`bbK`compPctl`trailW`trailK; 10 1.0 0.05 5 1.0; 100 3.0 0.4 50 5.0; 00000b; 10010b; noVld]); // breakoutTrail
    names:`zscore`rank`momentum`smoothMom`accel`emaCross`meanRev`volAdj`breakout`decay`ridge`ols`csZscore`csRank`smoothZscore`rsi`bbandPos`macdHist`slopeT`cci`stochastic`fisher`cmo`trix`kalmanResid`rsiRetrace`bbRetrace`zscoreSnap`breakHold`tensionSnap`breakoutTrail;
