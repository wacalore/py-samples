...
snapRevert:{[tension;trigger;mode;holdHL]
    ten:@[ffill tension;where null ffill tension;:;0f];
    trig:@[ffill trigger;where null ffill trigger;:;0f];
    lam:$[mode~`decay; exp neg log[2] % holdHL; 0f];
    init:(0f; 0f);
    step:$[mode~`track;
        {[s;t;tr]
            pos:s 0; esign:s 1; ts:signum t;
            $[(tr>0.5) and t<>0f;        (neg t; ts);
              (esign<>0f) and (ts<>esign) and ts<>0f; (0f;0f);
              esign<>0f;                  (neg t; esign);
                                          (0f;0f)]};
        {[lam;s;t;tr]
            pos:s 0; esign:s 1; ts:signum t;
            $[(tr>0.5) and t<>0f;        (neg t; ts);
              (esign<>0f) and (ts<>esign) and ts<>0f; (0f;0f);
              esign<>0f;                  (pos*lam; esign);
                                          (0f;0f)]}[lam]];
...
help:{[]
    ...
    -1 "  snapRevert[ten;trig;mode;hl]     - mean-revert on trigger, track or decay";
    -1 "";
