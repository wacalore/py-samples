...
// Always includes maturity as the final payment date
genPaySchedule:{[maturity;freq]
    dt:tenorToYF freq;
    n:1 | `long$maturity % dt;
    sched:dt * 1 + til n;
    // Ensure maturity is always the final payment:
    // - If last equals maturity (within tolerance): keep schedule
    // - If last > maturity: replace last with maturity
    // - If last < maturity: append maturity
    $[(abs last[sched] - maturity) < 1e-10; sched;
      last[sched] > maturity; @[sched;n-1;:;maturity];
      sched,maturity]}
...
// Evaluate cubic spline DERIVATIVE at points xi
// For spline: y = a + b*dx + c*dx^2 + d*dx^3
// Derivative: y' = b + 2*c*dx + 3*d*dx^2
cubicSplineDeriv:{[coeffs;xi]
    x:coeffs`x; b:coeffs`b; c:coeffs`c; d:coeffs`d;
    xi:`float$xi;
    idx:{[x;v] 0 | (count[x]-2) & x bin v}[x] each xi;
    dx:xi - x idx;
    b[idx] + dx * (2*c[idx] + dx * 3*d[idx])}
...
buildCurve:{[tenors;rates;config]
    ...
    zeroSpline:$[cfg[`interpMethod]~`smooth;
        ...
        ()!()];
    curve:`tenors`yearFracs`discountFactors`zeroRates`parRates`zeroSpline`fwdSpline`config!(labels;yf;df;zeros;rates;zeroSpline;()!();cfg);
    ...
    // Build forward rate spline for smooth forward interpolation
    curve[`fwdSpline]:buildFwdSpline[curve];
    curve}
...
rebuildFromZeros:{[curve]
    ...
    // Rebuild splines if using smooth interpolation
    if[cfg[`interpMethod]~`smooth;
        curve[`zeroSpline]:smoothingSpline[yf;zeros;cfg`smoothParam];
        curve[`fwdSpline]:buildFwdSpline[curve]];
    curve}
...
// Forward rate from t1 to t2 (simple compounded)
fwdRate:{[curve;t1;t2]
...
// f(t) = zero(t) + t * zero'(t) = -d(ln DF)/dt
// Uses forward polynomial/spline for smooth output when available
instFwdRate:{[curve;t]
    cfg:curve`config;
    im:cfg`interpMethod;
    // Use pre-computed forward curve if available
    if[(im~`smooth) and count curve`fwdSpline;
        fwd:curve`fwdSpline;
        // Check if polynomial or spline
        if[`poly~fwd`type;
            :sum fwd[`coeffs] * t xexp til count fwd`coeffs];
        // Otherwise it's a smoothing spline
        :smoothingSplineEval[fwd;enlist `float$t] 0];
    // Fallback: compute from zero spline derivative
    if[(im~`smooth) and count curve`zeroSpline;
        z:cubicSplineEval[curve`zeroSpline;enlist `float$t] 0;
        zDeriv:cubicSplineDeriv[curve`zeroSpline;enlist `float$t] 0;
        :z + t * zDeriv];
    // Fallback: numerical derivative with small dt
    dt:0.001;
    z1:interpZero[curve;t-dt];
    z2:interpZero[curve;t+dt];
    zDeriv:(z2-z1) % (2*dt);
    z:interpZero[curve;t];
    z + t * zDeriv}

// Compute raw instantaneous forward (without spline smoothing)
// Used internally to build the forward spline
instFwdRateRaw:{[curve;t]
    cfg:curve`config;
    im:cfg`interpMethod;
    // Compute from zero spline derivative
    if[(im~`smooth) and count curve`zeroSpline;
        z:cubicSplineEval[curve`zeroSpline;enlist `float$t] 0;
        zDeriv:cubicSplineDeriv[curve`zeroSpline;enlist `float$t] 0;
        :z + t * zDeriv];
...
// Fit polynomial of degree n to data using least squares
// Returns coefficients [c0, c1, c2, ..., cn] for c0 + c1*x + c2*x^2 + ...
polyFit:{[x;y;n]
    // Build Vandermonde matrix: each row is [1, x, x^2, ..., x^n]
    X:flip x xexp/: til n+1;
    // Solve normal equations: (X'X)c = X'y
    Xt:flip X;
    XtX:Xt mmu X;
    Xty:Xt mmu y;
    inv[XtX] mmu Xty}

// Build forward rate curve from polynomial fit
// Uses polynomial instead of spline to avoid oscillations
// Only builds polynomial for dense grids (>=30 points); sparse grids use zero derivative
buildFwdSpline:{[curve]
    cfg:curve`config;
    if[not cfg[`interpMethod]~`smooth; :()!()];
    yf:curve`yearFracs;
    validYf:yf where yf > 0.1;
    n:count validYf;
    // Only use polynomial for dense grids (avoids wild extrapolation with sparse data)
    // For sparse curves (<30 points), instFwdRate will use zero spline derivative
    if[n < 30; :()!()];
    rawFwds:instFwdRateRaw[curve;] each validYf;
    // Polynomial degree: cap at min(8, n-2)
    maxDeg:$[`fwdPolyDeg in key cfg; cfg`fwdPolyDeg; 8];
    deg:(n - 2) & maxDeg;
    deg:2 | deg;  // minimum degree 2
    coeffs:polyFit[validYf;rawFwds;deg];
    `type`coeffs`xmin`xmax!(`poly;coeffs;min validYf;max validYf)}
...
keyRateDV01:{[curve;notional;fixedRate;maturity;freq;isReceiver]
    ...
    sens:relevantIdx!{[curve;calcPV;notional;fixedRate;dcfs;payDates;isReceiver;idx]
        ...
        (pvDn - pvUp) % 2
    }[curve;calcPV;notional;fixedRate;dcfs;payDates;isReceiver] peach relevantIdx;
...
scenarioPnL:{[curve;notional;fixedRate;maturity;freq;isReceiver;shocksBps]
    ...
    shocksBps!calcPnL peach shocksBps}
...
carryRollTable:{[curve;notional;fixedRate;maturity;freq;isReceiver;horizons]
    ...
    results:carryRoll[curve;notional;fixedRate;maturity;freq;isReceiver;] peach horizons;
    // Build table
    ([] horizon:horizons;
