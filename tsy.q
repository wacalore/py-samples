...
parseBillResponse:{[data]
    ...
    cusips:`$rawTable`cusip;
    secTypes:`$rawTable`securityType;
    // Bills have discount rate instead of coupon
    discRates:0.01 * "F"$rawTable`interestRate;
    issueDates:"D"$10#/:rawTable`issueDate;
    matDates:"D"$10#/:rawTable`maturityDate;
    origTerms:.tsy.parseSecurityTerm each rawTable`securityTerm;
    auctionDates:"D"$10#/:rawTable`auctionDate;
...
fetchAllBills:{[startYear]
    ...
    bills:select from ((`cusip xgroup bills) @ \: `auctionDate xcols 0#bills) where i = (first;i) fby cusip;
...
// Can take: startYear (int), or startDate/endDate (dates)
// buildBillCache[2020]                     - from 2020 to present
// buildBillCache["D"$"2020-01-01"; (::)]   - from date to present
// buildBillCache["D"$"2020-01-01"; "D"$"2024-12-31"]  - date range
buildBillCache:{[startDate;endDate]
    // Handle single-arg case (just startYear)
    if[(endDate ~ (::)) and (-6h = type startDate);
        // Single year argument (legacy behavior)
        bills:.tsy.fetchAllBills[startDate];
        .tsy.saveBillCache bills;
        :bills];

    // Handle date range
    startDate:$[-6h = type startDate; "D"$(string startDate),".01.01"; startDate];
    endDate:$[endDate ~ (::); .z.d; endDate];
    endDate:$[-6h = type endDate; "D"$(string endDate),".12.31"; endDate];
...
findOnTheRun:{[bonds;bills;asOfDate]
    ...
    otrBonds:select cusip, securityType, coupon, maturityDate, origTerm, auctionDate
        from activeBonds
        where auctionDate = (max;auctionDate) fby origTerm;

    otrBills:select cusip, securityType, discountRate, maturityDate, origTerm, auctionDate, price
        from activeBills
        where auctionDate = (max;auctionDate) fby origTerm;

    // Build result table
    result:([] tenor:`symbol$(); cusip:`symbol$(); securityType:`symbol$();
        coupon:`float$(); maturityDate:`date$(); auctionDate:`date$();
        remainingYears:`float$(); price:`float$());

    // Add bonds (2Y, 3Y, 5Y, 7Y, 10Y, 20Y, 30Y)
    bondTenors:`2Y`3Y`5Y`7Y`10Y`20Y`30Y;
    {[result;otrBonds;asOfDate;t]
        b:select from otrBonds where origTerm = t;
        if[0 = count b; :result];
        b:first b;
        row:([] tenor:enlist t; cusip:enlist b`cusip; securityType:enlist b`securityType;
            coupon:enlist b`coupon; maturityDate:enlist b`maturityDate;
            auctionDate:enlist b`auctionDate;
            remainingYears:enlist (b[`maturityDate] - asOfDate) % 365f;
            price:enlist 0n);  // Price needs to be provided separately
        result,row
    }[;otrBonds;asOfDate]/[result;bondTenors];
...
buildOTRCurveOne:{[bonds;bills;asOfDate;priceDict]
    ...
    // Add bills to OTR
    activeBills:select from bills where issueDate <= asOfDate, maturityDate > asOfDate;
    otrBills:select cusip, securityType, discountRate, maturityDate, origTerm, auctionDate
        from activeBills
        where auctionDate = (max;auctionDate) fby origTerm;

    billTenors:`4W`8W`13W`17W`26W`52W;
    billRows:{[otrBills;asOfDate;priceDict;t]
        ...
        yld:$[not null px;
            billYieldFromPrice[px;daysToMat];
            billBEY[b`discountRate;daysToMat]];
        `tenor`cusip`securityType`coupon`maturityDate`auctionDate`remainingYears`price`yield!
            (t;b`cusip;b`securityType;0f;b`maturityDate;b`auctionDate;
             daysToMat%365f;px;yld)
    }[otrBills;asOfDate;priceDict;] each billTenors;
    billRows:billRows where not (::)~/: billRows;
    billTable:$[count billRows; flip billRows;
        ([] tenor:`symbol$(); cusip:`symbol$(); securityType:`symbol$();
           coupon:`float$(); maturityDate:`date$(); auctionDate:`date$();
           remainingYears:`float$(); price:`float$(); yield:`float$())];
...
buildOTRCurve:{[bonds;bills;asOfDate;prices]
    ...
    priceCol:$[99h <> type prices; `price;
               `cleanPrice in cols prices; `cleanPrice;
               `price];

    // Multi-date mode: asOfDate is (::) and prices has date column
    if[(asOfDate ~ (::)) and (99h = type prices) and (`date in cols prices);
