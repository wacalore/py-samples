...
dailyCTD:{[baskets;prices;futures;curves;repoRate]
    ...
    // 100h = lambda, 104h = projection (partial application)
    curveType:$[99h=type curves; `dict;           // static curve dict
                (type curves) in 100 104h; `builder;  // function or projection
                98h=type curves; `table;           // table with date column
                `dict];                            // default to static

    // Curve getter
    getCurve:$[curveType=`dict; {[c;dt] c}[curves;];
               curveType=`builder; curves;
               curveType=`table; {[t;dt] first exec curve from t where date=dt}[curves;];
               {[c;dt] c}[curves;]];

    // Repo getter
    getRepo:{[r;dt]
        $[-9h=type r; r;                                          // float scalar
          99h=type r; r`rate;                                     // dict with `rate
          98h=type r; first exec rate from r where date=dt;       // table
          r]                                                      // default
    }[repoRate;];

    // Detect futures table format (deliveryCode vs contract+deliveryMonth)
    hasDC:`deliveryCode in cols futures;

    // Get futures price for a delivery code and date
    getFutPrice:$[hasDC;
        {[f;dc;dt] first exec price from f where date=dt, deliveryCode=dc}[futures;;];
        {[f;dc;dt]
            c:`$3#string dc;                                      // extract contract from code
            m:"M"$-3_ 2_string dc;                                // extract month (hacky but works)
            first exec price from f where date=dt, contract=c, deliveryMonth=m
        }[futures;;]];

    // Pack context
    ctx:`baskets`prices`getCurve`getRepo`getFutPrice!
        (baskets;prices;getCurve;getRepo;getFutPrice);

    // Analyze one delivery code for one date
    analyzeOne:{[ctx;dc;dt]
        // Get basket for this delivery code
        basket:select from ctx[`baskets] where deliveryCode=dc;
        if[0=count basket; :()];

        // Extract contract and delivery month
        c:first basket`contract;
        dm:first basket`deliveryMonth;
        deliveryDate:firstDeliveryDay[dm];

        // Get curve and repo for this date
        curve:ctx[`getCurve][dt];
        if[99h<>type curve; :()];  // skip if no curve (curve is a dict)

        repo:ctx[`getRepo][dt];
        if[null repo; :()];

        // Get futures price
        futPrice:ctx[`getFutPrice][dc;dt];
        if[null futPrice; :()];

        // Get bond prices and convert to CTD format
        dayPrices:exec cusip!price from ctx[`prices] where date=dt;
        ctdBonds:toCtdBonds[basket;dayPrices];
        if[0=count ctdBonds; :()];

        // Run CTD analysis
        results:.ctd.ctdAnalysis[curve;dt;deliveryDate;futPrice;repo;ctdBonds];

        // Add identifiers
        update date:dt, deliveryCode:dc, contract:c, deliveryMonth:dm from results
    }[ctx;;];

    // Generate all deliveryCode/date combinations
    combos:(cross)[deliveryCodes;dates];

    // Run analysis (could use peach for parallelism)
    results:raze analyzeOne ./: combos;

    // Sort by date, contract, deliveryMonth
...
runDailyCTD:{[bonds;startYear;endYear;prices;futures;curves;repoRate]
