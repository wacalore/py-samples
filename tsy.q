...
parseSecurityTerm:{[term]
    ...
    // For "X-Year Y-Month" format, take just the first number before "-Year" or "-year"
    // Split on " " and take first part, then extract number
    firstPart:first " " vs t;  // e.g., "19-Year" from "19-Year 10-Month"
    n:"I"$firstPart where firstPart in "0123456789";
    `$string[n],suffix}
...
updateCache:{[]
    ...
    // Add missing columns to old cache if needed (schema migration)
    if[not `offeringAmount in cols existing;
        existing:update offeringAmount:0n, totalAccepted:0n, outstanding:0n, somaHoldings:0n, publicFloat:0n from existing];
    ...
    combined:select securityType:first securityType, coupon:first coupon, issueDate:first issueDate,
                    ...
                    offeringAmount:sum offeringAmount, totalAccepted:sum totalAccepted,
                    outstanding:last outstanding, somaHoldings:last somaHoldings, publicFloat:last publicFloat
...
parseApiResponse:{[data]
    ...
    // Extract auction amounts (new fields for index weighting)
    // offeringAmount: amount offered at auction
    // totalAccepted: actual amount sold
    // currentlyOutstanding: total outstanding (for reopenings)
    // somaHoldings: Fed/SOMA holdings (deduct for public float)
    offeringAmts:"F"$rawTable`offeringAmount;
    totalAccepted:"F"$rawTable`totalAccepted;
    outstanding:"F"$rawTable`currentlyOutstanding;
    somaHoldings:"F"$rawTable`somaHoldings;
    // Use totalAccepted if outstanding is null, else use outstanding
    outstanding:?[null outstanding; totalAccepted; outstanding];
    // Calculate public float (outstanding minus SOMA holdings)
    publicFloat:outstanding - ?[null somaHoldings; 0f; somaHoldings];
    publicFloat:publicFloat|0f;  // Floor at 0
    ...
    ([] cusip:cusips; securityType:secTypes; coupon:coupons;
        ...
        offeringAmount:offeringAmts; totalAccepted:totalAccepted;
        outstanding:outstanding; somaHoldings:somaHoldings; publicFloat:publicFloat)}
...
fetchAllSecurities:{[startYear]
    ...
    // Combine and deduplicate by CUSIP
    // For reopenings: take last values (cumulative outstanding, latest SOMA holdings)
...
loadBondCache:{[]
    ...
    // Try formats: 12 cols (with SOMA), 10 cols (outstanding only), 7 cols (original)
    bonds:@[{("SSFDDSDFFFFF";enlist csv) 0: x};path;
           {[p;e] @[{("SSFDDSDFFF";enlist csv) 0: x};p;
                   {[p2;e2] ("SSFDDSD";enlist csv) 0: p2}[p;]]}[path;]];
...
loadBillCache:{[]
    ...
    bills:("SSFDDSDF";enlist csv) 0: path;
...
// =============================================================================
// FISCAL DATA AUCTION API
// =============================================================================
// Per-auction data from api.fiscaldata.treasury.gov
// This API provides offering_amt for each auction (critical for reopenings)
// Unlike TreasuryDirect API which gives cumulative outstanding

fiscalDataAuctionUrl:"https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/accounting/od/auctions_query"
auctionCacheFile:"tsy_auctions.csv"

// Fetch auction data for a date range
// Returns raw data with per-auction offering amounts
fetchAuctions:{[startDate;endDate]
    sd:$[10h = type startDate; startDate; ssr[string startDate;".";"-"]];
    ed:$[10h = type endDate; endDate; ssr[string endDate;".";"-"]];
    // Filter for Notes and Bonds only
    filter:"security_type:in:(Note,Bond),auction_date:gte:",sd,",auction_date:lte:",ed;
    url:fiscalDataAuctionUrl,"?format=json&page%5Bsize%5D=10000&filter=",filter;
    result:.j.k httpGet url;
    result`data}

// Fetch auctions for a specific month
fetchAuctionsForMonth:{[m]
    startDate:firstOfMonth m;
    endDate:lastOfMonth m;
    .tsy.fetchAuctions[startDate;endDate]}

// Fetch auctions that issue in a specific month (regardless of auction date)
// This is what we need for extension calculations
fetchAuctionsIssuingInMonth:{[m]
    // Need to look at auctions from prior month that may settle in target month
    startDate:firstOfMonth m - 1;
    endDate:lastOfMonth m;
    sd:ssr[string startDate;".";"-"];
    ed:ssr[string endDate;".";"-"];

    issueStart:`$ssr[string firstOfMonth m;".";"-"];
    issueEnd:`$ssr[string lastOfMonth m;".";"-"];

    // Filter for Notes and Bonds, auction in prior month or target month
    filter:"security_type:in:(Note,Bond),auction_date:gte:",sd,",auction_date:lte:",ed;
    url:fiscalDataAuctionUrl,"?format=json&page%5Bsize%5D=10000&filter=",filter;
    result:.j.k httpGet url;
    data:result`data;

    // Further filter to only those issuing in target month
    // Use symbol comparison for proper lexicographic ordering
    if[0 < count data;
        issueDateSyms:`$data[;`issue_date];
        mask:(issueDateSyms >= issueStart) and (issueDateSyms <= issueEnd);
        data:data where mask];
    data}

// Parse Fiscal Data auction response to standard format
parseAuctionResponse:{[data]
    if[0 = count data; :([] cusip:`symbol$(); securityType:`symbol$();
        coupon:`float$(); auctionDate:`date$(); issueDate:`date$();
        maturityDate:`date$(); origTerm:`symbol$(); offeringAmt:`float$();
        totalAccepted:`float$(); isReopening:`boolean$(); announcementDate:`date$())];

    rawTable:flip data;

    cusips:`$rawTable`cusip;
    secTypes:`$rawTable`security_type;
    // int_rate is in percent (e.g., "4.500")
    coupons:0.01 * "F"$rawTable`int_rate;
    auctionDates:"D"$10#/:rawTable`auction_date;
    issueDates:"D"$10#/:rawTable`issue_date;
    matDates:"D"$10#/:rawTable`maturity_date;
    // security_term is like "2-Year", "10-Year", "9-Year 10-Month" (reopened 10Y)
    // Use security_term (current remaining term) as that's what matters for bucket assignment
    // Note: For reopenings, security_term shows current remaining maturity
    origTerms:.tsy.parseSecurityTerm each rawTable`security_term;
    // offering_amt is the public offering amount (excludes SOMA add-on)
    offeringAmts:"F"$rawTable`offering_amt;
    // total_accepted is the total issued (includes SOMA add-on) - use this for outstanding calculation
    totalAcceptedAmts:"F"$rawTable`total_accepted;
    // Fall back to offeringAmt if totalAccepted is null
    totalAcceptedAmts:?[null totalAcceptedAmts; offeringAmts; totalAcceptedAmts];
    // reopening is "Yes" or "No" - convert to boolean
    isReopen:"Yes"~/:rawTable`reopening;
    // announcemt_date (note API typo) - when auction was announced
    announceDates:"D"$10#/:rawTable`announcemt_date;

    ([] cusip:cusips; securityType:secTypes; coupon:coupons;
        auctionDate:auctionDates; issueDate:issueDates;
        maturityDate:matDates; origTerm:origTerms; offeringAmt:offeringAmts;
        totalAccepted:totalAcceptedAmts; isReopening:isReopen; announcementDate:announceDates)}

// Get auctions issuing in a month with offering amounts
// This is the main function for extension calculations
getAuctionsForExtension:{[targetMonth]
    raw:.tsy.fetchAuctionsIssuingInMonth[targetMonth];
    .tsy.parseAuctionResponse raw}

// Fetch all auction data from startYear to present
fetchAllAuctions:{[startYear]
    startYear:$[startYear ~ (::); 2018; startYear];
    endYear:`year$.z.d;

    -1 "=== Fetching Auction Data ===";
    -1 "Date range: ",string[startYear]," to ",string endYear;
    -1 "";
    ...
    data:raze {[y]
        ...
        ed:string[y],"-12-31";
        -1 "  ",string[y],"...";
        .tsy.fetchAuctions[sd;ed]
    } each years;

    -1 "  Total: ",string[count data]," records";
    auctions:.tsy.parseAuctionResponse data;
    auctions:`auctionDate xasc auctions;
    -1 "Parsed: ",string[count auctions]," auctions";
    auctions}

// Save auction cache
saveAuctionCache:{[auctions]
    filepath:cacheDir,auctionCacheFile;
    path:`$":",filepath;
    (path) 0: csv 0: auctions;
    -1 "Saved ",string[count auctions]," auctions to ",filepath;
    auctions}
...
loadAuctionCache:{[]
    filepath:cacheDir,auctionCacheFile;
    path:`$":",filepath;
    if[()~key path; '"Auction cache not found at ",filepath,". Run .tsy.buildAuctionCache[] first"];
    // Format: cusip,securityType,coupon,auctionDate,issueDate,maturityDate,origTerm,offeringAmt,totalAccepted,isReopening,announcementDate
    auctions:("SSFDDDSFFBD";enlist csv) 0: path;
    -1 "Loaded ",string[count auctions]," auctions from cache";
    auctions}

// Build and save auction cache
buildAuctionCache:{[startYear]
    auctions:.tsy.fetchAllAuctions[startYear];
    .tsy.saveAuctionCache auctions;
    auctions}

// Update auction cache with new data
updateAuctionCache:{[]
    existing:.tsy.loadAuctionCache[];
    lastDate:max existing`auctionDate;

    -1 "=== Updating Auction Cache ===";
    -1 "Last auction in cache: ",string lastDate;
    -1 "";
    ...
    -1 "Fetching new data...";
    newRaw:.tsy.fetchAuctions[startDate;endDate];
    newAuctions:$[0 < count newRaw; .tsy.parseAuctionResponse newRaw; 0#existing];
    -1 "  New auctions: ",string count newAuctions;

    combined:existing,newAuctions;
    combined:`auctionDate xasc combined;

    newCount:(count combined) - count existing;
    -1 "";
    -1 "Added ",string[newCount]," new auctions";
    -1 "Total auctions: ",string count combined;

    .tsy.saveAuctionCache combined;
    combined}

// Query auctions from cache for bonds issuing in a specific month
// Returns per-auction data with offering amounts
auctionsIssuingInMonth:{[auctions;targetMonth]
    select from auctions where (`month$issueDate) = targetMonth}

// =============================================================================
// SOMA HOLDINGS (Federal Reserve)
// =============================================================================
// Fetch historical SOMA holdings from NY Fed API
// API: https://markets.newyorkfed.org/api/soma/tsy/get/all/asof/YYYY-MM-DD.json

somaApiUrl:"https://markets.newyorkfed.org/api/soma/tsy/get/all/asof/"
somaCacheFile:"tsy_soma.csv"

// Fetch SOMA holdings for a specific date
// Returns table with cusip, parValue, percentOutstanding, maturityDate, securityType, coupon
fetchSomaAsOf:{[dt]
    dtStr:ssr[string dt;".";"-"];
    url:somaApiUrl,dtStr,".json";
    result:.j.k httpGet url;
    if[not `soma in key result; :([] cusip:`symbol$())];
    if[not `holdings in key result`soma; :([] cusip:`symbol$())];
    holdings:result[`soma]`holdings;
    if[0 = count holdings; :([] cusip:`symbol$())];

    // Filter to NotesBonds only (exclude Bills, TIPS, FRN)
    secTypes:holdings[;`securityType];
    notesBonds:holdings where secTypes like "NotesBonds";
    if[0 = count notesBonds; :([] cusip:`symbol$())];

    cusips:`$notesBonds[;`cusip];
    parValues:"F"$notesBonds[;`parValue];
    pctOut:"F"$notesBonds[;`percentOutstanding];
    matDates:"D"$10#/:notesBonds[;`maturityDate];
    coupons:"F"$notesBonds[;`coupon];
    asOfDates:count[cusips]#dt;

    ([] asOfDate:asOfDates; cusip:cusips; parValue:parValues;
        percentOutstanding:pctOut; maturityDate:matDates; coupon:coupons)}

// Get available SOMA snapshot dates (Wednesdays, weekly)
// Returns list of dates from startDate to endDate
somaSnapshotDates:{[startDate;endDate]
    // SOMA data is published weekly on Thursdays for Wednesday close
    // Generate all Wednesdays in range
    allDates:startDate + til 1 + endDate - startDate;
    // Wednesday = day 4 in q (0=Sat, 1=Sun, 2=Mon, ...)
    wednesdays:allDates where 4 = allDates mod 7;
    wednesdays}

// Fetch SOMA holdings for a date range (weekly snapshots)
// Returns table with all holdings across dates
fetchSomaRange:{[startDate;endDate]
    dates:somaSnapshotDates[startDate;endDate];
    -1 "Fetching SOMA holdings for ",string[count dates]," weeks...";
    holdings:raze {[dt]
        h:@[fetchSomaAsOf; dt; {[e] ([] cusip:`symbol$())}];
        if[0 < count h; -1 "  ",string[dt],": ",string[count h]," holdings"];
        h
    } each dates;
    holdings}

// Build and save SOMA cache from startYear to present
buildSomaCache:{[startYear]
    startDate:"D"$string[startYear],".01.01";
    endDate:.z.d;
    -1 "=== Building SOMA Holdings Cache ===";
    -1 "Date range: ",string[startDate]," to ",string endDate;
    -1 "";

    holdings:.tsy.fetchSomaRange[startDate;endDate];

    -1 "";
    -1 "Total records: ",string count holdings;
    -1 "Date range: ",string[min holdings`asOfDate]," to ",string max holdings`asOfDate;
    -1 "Unique CUSIPs: ",string count distinct holdings`cusip;

    .tsy.saveSomaCache holdings;
    holdings}

// Save SOMA cache
saveSomaCache:{[holdings]
    filepath:cacheDir,somaCacheFile;
    path:`$":",filepath;
    (path) 0: csv 0: holdings;
    -1 "Saved ",string[count holdings]," SOMA records to ",filepath;
    holdings}
...
loadSomaCache:{[]
    filepath:cacheDir,somaCacheFile;
    path:`$":",filepath;
    if[()~key path; '"SOMA cache not found at ",filepath,". Run .tsy.buildSomaCache[] first"];
    holdings:("DSFFDDF";enlist csv) 0: path;
    -1 "Loaded ",string[count holdings]," SOMA records from cache";
    holdings}

// Get SOMA holdings as of a specific date (uses cache, finds nearest prior date)
// Returns dict cusip -> parValue
somaHoldingsAsOf:{[somaCache;asOfDate]
    // Find the most recent snapshot on or before asOfDate
    availDates:asc distinct somaCache`asOfDate;
    priorDates:availDates where availDates <= asOfDate;
    if[0 = count priorDates; :()!()];  // No data before this date
    snapshotDate:last priorDates;
    exec cusip!parValue from somaCache where asOfDate = snapshotDate}

// Get public float (outstanding - SOMA) as of date
// Returns dict cusip -> publicFloat
publicFloatAsOf:{[somaCache;auctions;asOfDate]
    // Get outstanding from auctions
    outstanding:.tsyindex.outstandingAsOf[auctions;asOfDate];
    // Get SOMA holdings
    soma:somaHoldingsAsOf[somaCache;asOfDate];
    // Calculate public float = outstanding - soma
    cusips:key outstanding;
    pf:cusips!{[out;soma;c]
        o:out c;
        s:$[c in key soma; soma c; 0f];
        o - s
    }[outstanding;soma;] each cusips;
    // Floor at 0
    pf:cusips!0f|pf cusips;
    pf}

// Update SOMA cache with new data since last date
updateSomaCache:{[]
    existing:.tsy.loadSomaCache[];
    lastDate:max existing`asOfDate;

    -1 "=== Updating SOMA Cache ===";
    -1 "Last snapshot in cache: ",string lastDate;
    -1 "";
    ...
    newHoldings:.tsy.fetchSomaRange[startDate;endDate];
    if[0 = count newHoldings;
        -1 "No new data available";
        :existing];

    combined:existing,newHoldings;
    combined:`asOfDate`cusip xasc combined;

    -1 "";
    -1 "Added ",string[count newHoldings]," new records";
    -1 "Total records: ",string count combined;

    .tsy.saveSomaCache combined;
    combined}
