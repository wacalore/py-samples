...
// Generate bond cash flow dates from settlement to maturity (vectorized, no loop)
genBondCFDates:{[settleDate;maturityDate;couponFreq]
    ...
    nPeriods:1 + `int$(maturityDate - settleDate) % monthsPerPeriod * 30;
    nPeriods:1 | nPeriods;  // At least 1
    // Use month arithmetic (vectorized)
    matMonth:`month$maturityDate;
    matDay:`dd$maturityDate;
    // Generate months: maturity, maturity-6m, maturity-12m, ...
    cfMonths:matMonth - monthsPerPeriod * til nPeriods;
    // Convert to dates (1st of month + day offset - 1)
    dates:("d"$cfMonths) + matDay - 1;
    // Filter to dates > settleDate and sort ascending
    asc dates where dates > settleDate}

// Batch generate CF dates for multiple bonds (fully vectorized)
genBondCFDatesBatch:{[settleDate;maturities;couponFreq]
    monthsPerPeriod:$[couponFreq~`SA;6;couponFreq~`Q;3;couponFreq~`A;12;6];
    // For each bond, generate its CF dates
    genBondCFDates[settleDate;;couponFreq] each maturities}
...
// ytm is annual yield with SEMI-ANNUAL compounding (bond equivalent yield)
priceBondFromYield:{[settleDate;maturityDate;coupon;couponFreq;faceValue;ytm]
    ...
    // Time to each cash flow in semi-annual periods
    cfYFs:(cfDates - settleDate) % 365f;
    cfPeriods:cfYFs * 2;  // Convert years to semi-annual periods

    // Discount at YTM (semi-annual compounding): df = (1 + y/2)^(-periods)
    y2:ytm % 2;
    dfs:xexp[1 + y2; neg cfPeriods];

    sum cfs * dfs}

// Yield to maturity (solve for yield given price)
// Uses Newton-Raphson with analytical duration - quadratic convergence
// Returns SEMI-ANNUAL bond equivalent yield (market convention)
// Input: cleanPrice (quoted price, excluding accrued interest)
yieldToMaturity:{[settleDate;maturityDate;coupon;couponFreq;faceValue;cleanPrice]
    if[settleDate >= maturityDate; :0f];

    // Generate cash flows once
    cfDates:genBondCFDates[settleDate;maturityDate;couponFreq];
    if[0 = count cfDates; :coupon];
    ...
    // Time to each cash flow in semi-annual periods (for semi-annual compounding)
    cfYFs:(cfDates - settleDate) % 365f;
    cfPeriods:cfYFs * 2;  // Convert years to semi-annual periods

    // YTM solves for: PV of cash flows = dirty price
    // Dirty price = clean price + accrued interest
    ai:accruedInterest[settleDate;maturityDate;coupon;couponFreq;faceValue];
    dirtyPrice:cleanPrice + ai;

    // Price and duration at given yield (semi-annual compounding)
    // ytm is annual yield, y2 = ytm/2 is semi-annual yield
    // df = 1 / (1 + y2)^periods
    priceDur:{[cfs;cfPeriods;ytm]
        y2:ytm % 2;                              // Semi-annual yield
        dfs:xexp[1 + y2; neg cfPeriods];         // (1 + y2)^(-periods)
        pv:sum cfs * dfs;
        // dP/dy = dP/dy2 * dy2/dy = dP/dy2 * 0.5
        // dP/dy2 = -sum(cfs * periods * (1+y2)^(-periods-1))
        //        = -sum(cfs * periods * dfs / (1+y2))
        dur:neg 0.5 * sum (cfs * cfPeriods * dfs) % (1 + y2);
        (pv;dur)
    }[cfs;cfPeriods;];

    // Initial guess: improved approximation formula
    n:(maturityDate - settleDate) % 365f;
    annualCoupon:coupon * faceValue;
    ytm:(annualCoupon + ((faceValue - cleanPrice) % n)) % ((0.4 * faceValue) + (0.6 * cleanPrice));
    ytm:0.001 | ytm;  // floor at 0.1%

    // Newton-Raphson: x_{n+1} = x_n - f(x_n) / f'(x_n)
    // Solve: PV(ytm) = dirtyPrice
    do[10;
        pd:priceDur ytm;
        pv:pd 0; dur:pd 1;
        f:pv - dirtyPrice;
        if[abs[dur] < 1e-14; :ytm];
        ytm:ytm - (f % dur)];
    ytm}
...
conversionFactor:{[deliveryDate;maturityDate;coupon;couponFreq]
    ...
    // Round down to nearest quarter (use div for floor division, not % which is regular division)
    quarters:monthsToMat div 3;
...
// Returns basis in decimal (price points)
grossBasis:{[bondCleanPrice;futuresPrice;cf]
    bondCleanPrice - (futuresPrice * cf)}

// Gross basis in 32nds (market convention for Treasury basis)
grossBasis32:{[bondCleanPrice;futuresPrice;cf]
    (bondCleanPrice - (futuresPrice * cf)) * 32}
...
couponPaymentsBetween:{[settleDate;deliveryDate;maturityDate]
    ...
    // Use genBondCFDates which correctly handles coupon date generation
    // Look back far enough to capture all coupons in the period
    cfDates:genBondCFDates[settleDate - 365;maturityDate;`SA];
    // Count coupons strictly after settle and on or before delivery
    count cfDates where (cfDates > settleDate) & cfDates <= deliveryDate}
...
// bondAISettle: accrued interest at settle date (for dirty price)
// bondAIDelivery: accrued interest at delivery date (for invoice price)
impliedRepoRate:{[settleDate;deliveryDate;bondCleanPrice;bondAISettle;bondAIDelivery;futuresPrice;cf;couponAccrual]
    daysToDelivery:deliveryDate - settleDate;
    if[daysToDelivery <= 0; :0f];

    // Dirty price at settle = what you pay to buy the bond
    dirtyPrice:bondCleanPrice + bondAISettle;

    // Invoice price at delivery = what you receive when delivering
    invoicePrice:(futuresPrice * cf) + bondAIDelivery;

    // Solve for repo rate:
    // DirtyPrice * (1 + r * days/360) = InvoicePrice + CouponIncome
    // r = (InvoicePrice + CouponIncome - DirtyPrice) / (DirtyPrice * days/360)

    numerator:invoicePrice + couponAccrual - dirtyPrice;
...
analyzeBond:{[curve;settleDate;deliveryDate;futuresPrice;repoRate;bond]
    ...
    // Accrued interest at settle date (for dirty price)
    aiSettle:accruedInterest[settleDate;maturity;coupon;couponFreq;faceValue];
    dirtyPrice:cleanPrice + aiSettle;

    // Accrued interest at delivery date (for invoice price)
    ...
    // Implied repo - use AI at settle for dirty price, AI at delivery for invoice
    ir:impliedRepoRate[settleDate;deliveryDate;cleanPrice;aiSettle;aiDelivery;futuresPrice;cf;couponIncome];

    // Invoice price (what short receives upon delivery) - uses AI at delivery
    invoicePrice:(futuresPrice * cf) + aiDelivery;
    ...
    `sym`coupon`maturity`cleanPrice`dirtyPrice`ai`cf`grossBasis`carry`netBasis`impliedRepo`invoicePrice`ytm!(
        sym;coupon;maturity;cleanPrice;dirtyPrice;aiSettle;cf;gb;carry;nb;ir;invoicePrice;ytm)}
...
// =============================================================================
// VECTORIZED CTD ANALYSIS (OPTIMIZED)
// =============================================================================

// Vectorized accrued interest - works on vectors
// Uses same logic as scalar version but batched
accruedInterestVec:{[settleDate;maturities;coupons;faceValue]
    // Just call scalar version for each - it's already fast
    // The optimization is avoiding multiple calls in analyzeBond
    accruedInterest[settleDate;;;`SA;faceValue]'[maturities;coupons]}

// Vectorized YTM - processes multiple bonds
yieldToMaturityVec:{[settleDate;maturities;coupons;cleanPrices;faceValue]
    // Process each bond (still iterative but fewer function calls)
    {[sd;mat;cpn;px;fv]
        yieldToMaturity[sd;mat;cpn;`SA;fv;px]
    }[settleDate;;;;faceValue]'[maturities;coupons;cleanPrices]}

// Single-pass bond analysis - generates CF dates once, computes all metrics
analyzeBondFast:{[settleDate;deliveryDate;futuresPrice;repoRate;bond]
    sym:bond`sym;
    coupon:bond`coupon;
    maturity:bond`maturity;
    cleanPrice:bond`cleanPrice;
    ...
    // Generate CF dates ONCE and reuse for all calculations
    cfDates:genBondCFDates[settleDate - 365;maturity;`SA];
    cpnPayment:faceValue * coupon * 0.5;

    // Accrued interest at settle (using pre-computed cfDates)
    prevCpn:max cfDates where cfDates <= settleDate;
    nextCpn:min cfDates where cfDates > settleDate;
    ai:cpnPayment * (settleDate - prevCpn) % nextCpn - prevCpn;
    dirtyPrice:cleanPrice + ai;

    // Accrued interest at delivery (for carry calculation)
    prevCpnDel:max cfDates where cfDates <= deliveryDate;
    nextCpnDel:min cfDates where cfDates > deliveryDate;
    aiDelivery:cpnPayment * (deliveryDate - prevCpnDel) % nextCpnDel - prevCpnDel;
    aiChange:aiDelivery - ai;

    // Gross basis
    gb:cleanPrice - (futuresPrice * cf);

    // Coupon count between settle and delivery
    nCpns:sum (cfDates > settleDate) and cfDates <= deliveryDate;
    couponIncome:nCpns * cpnPayment;

    // Carry = AI change + coupon income - financing cost (matches bondCarryFull)
    daysToDelivery:deliveryDate - settleDate;
    financingCost:dirtyPrice * repoRate * daysToDelivery % 360;
    carry:aiChange + couponIncome - financingCost;

    // Net basis
    nb:gb - carry;

    // Implied repo - use AI at settle for dirty price, AI at delivery for invoice
    ir:impliedRepoRate[settleDate;deliveryDate;cleanPrice;ai;aiDelivery;futuresPrice;cf;couponIncome];

    // Invoice price (uses AI at delivery date, not settle)
    invoicePrice:(futuresPrice * cf) + aiDelivery;

    // YTM (uses its own CF dates for forward dates only)
    ytm:yieldToMaturity[settleDate;maturity;coupon;`SA;faceValue;cleanPrice];

    `sym`coupon`maturity`cleanPrice`dirtyPrice`ai`cf`grossBasis`carry`netBasis`impliedRepo`invoicePrice`ytm!(
        sym;coupon;maturity;cleanPrice;dirtyPrice;ai;cf;gb;carry;nb;ir;invoicePrice;ytm)}

// Vectorized CTD analysis - uses fast single-pass bond analysis
// Uses peach when called directly, each when called from dailyCTD (which has outer peach)
ctdAnalysisVec:{[curve;settleDate;deliveryDate;futuresPrice;repoRate;bonds]
    // Use fast single-pass analysis for each bond
    // Note: Using each here since dailyCTD uses peach at delivery code level
    results:analyzeBondFast[settleDate;deliveryDate;futuresPrice;repoRate;] each bonds;
...
carryAnalysis:{[settleDate;deliveryDate;ctdBond;futuresPrice;repoRate]
    ...
    // Accrued interest
    aiSettle:accruedInterest[settleDate;maturity;coupon;`SA;faceValue];
    aiDelivery:accruedInterest[deliveryDate;maturity;coupon;`SA;faceValue];
    dirtyPrice:cleanPrice + aiSettle;
    ...
    implRepo:impliedRepoRate[settleDate;deliveryDate;cleanPrice;aiSettle;aiDelivery;futuresPrice;cf;couponAccrual];
...
// Uses market prices as base - shifts YTM of each bond by the same amount
qualityBreakeven:{[curve;settleDate;deliveryDate;futuresPrice;repoRate;bonds]
    // Current CTD analysis (using market prices)
    ...
    // Calculate each bond's YTM from market price (for consistent yield-based shifts)
    bondsWithYTM:{[s;b]
        ytm:yieldToMaturity[s;b`maturity;b`coupon;`SA;100f;b`cleanPrice];
        b,enlist[`ytm]!enlist ytm
    }[settleDate;] each bonds;

    // Test yield shifts from -100bp to +100bp
    shifts:-100 -75 -50 -25 0 25 50 75 100;

    // Reprice each bond by shifting its YTM, keeping market-based consistency
    findCTDAtShift:{[curve;settle;delivery;fut;repo;bondsWithYTM;shift]
        // Shift each bond's YTM and reprice
        repricedBonds:{[s;shift;b]
            newYTM:b[`ytm] + shift % 10000;
            newYTM:0.0001 | newYTM;  // Floor at 1bp to avoid negative yields
            // priceBondFromYield returns DIRTY price, subtract AI for clean price
            dirtyPrice:priceBondFromYield[s;b`maturity;b`coupon;`SA;100f;newYTM];
            ai:accruedInterest[s;b`maturity;b`coupon;`SA;100f];
            newPrice:dirtyPrice - ai;
            b,enlist[`cleanPrice]!enlist newPrice
        }[settle;shift;] each bondsWithYTM;
        // Run CTD analysis
        results:ctdAnalysis[curve;settle;delivery;fut;repo;repricedBonds];
        ctd:first exec sym from results where isCTD;
        `shift`ctd!(shift;ctd)
    };

    shiftResults:findCTDAtShift[curve;settleDate;deliveryDate;futuresPrice;repoRate;bondsWithYTM;] peach shifts;
...
// Find nearest switch point using binary search (OPTIMIZED)
// Returns exact breakeven (within 1bp) and direction
// maxShift: maximum bp to search (default 300bp)
//
// Optimizations vs full ctdAnalysis approach:
// 1. Pre-compute all static bond properties (AI, CF, days to delivery)
// 2. Fast CTD check: only compute implied repo (skip net basis, carry, etc.)
// 3. Coarse-then-fine search: 25bp grid first, then binary search near switches
// 4. Vectorized implied repo calculation
nearestSwitchPoint:{[curve;settleDate;deliveryDate;futuresPrice;repoRate;bonds;maxShift]
    maxShift:$[null maxShift; 300; maxShift];
    daysToDelivery:deliveryDate - settleDate;
    if[daysToDelivery <= 0; :`currentCTD`nearest`direction`newCTD`switchUp`switchDown`newCTDUp`newCTDDown`maxShiftSearched!(`;0Ni;`;`;0Ni;0Ni;`;`;maxShift)];

    // Pre-compute static bond properties (these don't change with yield shifts)
    bondInfo:{[settle;delivery;b]
        ytm:yieldToMaturity[settle;b`maturity;b`coupon;`SA;100f;b`cleanPrice];
        aiSettle:accruedInterest[settle;b`maturity;b`coupon;`SA;100f];
        aiDelivery:accruedInterest[delivery;b`maturity;b`coupon;`SA;100f];
        // Coupon income = number of coupons paid × coupon payment
        numCoupons:couponPaymentsBetween[settle;delivery;b`maturity];
        cpnIncome:numCoupons * 100f * b[`coupon] * 0.5;  // Semi-annual coupon on $100 face
        `sym`coupon`maturity`cf`ytm`aiSettle`aiDelivery`cpnIncome!(
            b`sym;b`coupon;b`maturity;b`cf;ytm;aiSettle;aiDelivery;cpnIncome)
    }[settleDate;deliveryDate;] each bonds;

    // Fast CTD finder: compute implied repo for all bonds, return sym with max
    // Takes pre-computed bondInfo and a yield shift in bp
    fastCTD:{[fut;daysToDeliv;settle;bondInfo;shift]
        // Reprice each bond at shifted yield and compute implied repo
        repos:{[fut;days;settle;shift;bi]
            newYTM:bi[`ytm] + shift % 10000;
            newYTM:0.0001 | newYTM;
            // Price bond at new yield (dirty price)
            dirtyPrice:priceBondFromYield[settle;bi`maturity;bi`coupon;`SA;100f;newYTM];
            cleanPrice:dirtyPrice - bi`aiSettle;
            // Implied repo calculation (inlined for speed)
            invoicePrice:(fut * bi`cf) + bi`aiDelivery;
            numerator:invoicePrice + bi[`cpnIncome] - (cleanPrice + bi`aiSettle);
            repo:numerator % ((cleanPrice + bi`aiSettle) * days % 360);
            repo
        }[fut;daysToDeliv;settle;shift;] each bondInfo;
        // Return sym of bond with highest implied repo
        bondInfo[repos?max repos]`sym
    }[futuresPrice;daysToDelivery;settleDate;bondInfo];

    // Current CTD (at shift=0)
    currentCTD:fastCTD 0;

    // Coarse search: find regions where CTD might switch (25bp increments)
    coarseStep:25;
    coarseShiftsUp:coarseStep * 1 + til `int$maxShift % coarseStep;
    coarseShiftsDown:neg coarseStep * 1 + til `int$maxShift % coarseStep;

    // Find coarse switch regions
    findCoarseSwitch:{[fastCTD;currentCTD;step;shifts]
        if[0 = count shifts; :0Ni];
        ctds:fastCTD each shifts;
        switchIdx:first where ctds <> currentCTD;
        if[null switchIdx; :0Ni];
        // Return the shift just before the switch (for binary search bounds)
        $[switchIdx = 0; 0; shifts switchIdx - 1]
    };

    coarseSwitchUp:findCoarseSwitch[fastCTD;currentCTD;coarseStep;coarseShiftsUp];
    coarseSwitchDown:findCoarseSwitch[fastCTD;currentCTD;coarseStep;coarseShiftsDown];

    // Binary search within coarse region (if switch found)
    // Invariant: CTD at lo != CTD at hi, we narrow to find exact boundary
    binarySearch:{[fastCTD;currentCTD;lo;hi]
        if[null lo; :0Ni];
        if[null hi; :0Ni];
        // Verify switch exists in range
        ctdLo:fastCTD lo;
        ctdHi:fastCTD hi;
        if[ctdLo = ctdHi; :0Ni];
        // Binary search to find exact switch point
        // Compare to ctdLo (not currentCTD) to handle both directions correctly
        while[1 < abs hi - lo;
            mid:`int$(lo + hi) % 2;
            ctdMid:fastCTD mid;
            $[ctdMid = ctdLo; lo:mid; hi:mid]
        ];
        // Return the boundary where CTD differs from currentCTD
        $[ctdHi = currentCTD; lo; hi]
    };

    // Fine-tune switch points with binary search
    switchUp:$[null coarseSwitchUp; 0Ni;
               binarySearch[fastCTD;currentCTD;coarseSwitchUp;coarseSwitchUp + coarseStep]];
    switchDown:$[null coarseSwitchDown; 0Ni;
                 binarySearch[fastCTD;currentCTD;coarseSwitchDown - coarseStep;coarseSwitchDown]];

    // Get new CTD at switch points
    newCTDUp:$[null switchUp; `; fastCTD switchUp];
    newCTDDown:$[null switchDown; `; fastCTD switchDown];

    // Find nearest
    nearest:$[(null switchUp) and null switchDown; 0Ni;
              null switchUp; switchDown;
              null switchDown; switchUp;
              (abs switchUp) < abs switchDown; switchUp; switchDown];

    direction:$[null nearest; `; nearest > 0; `up; `down];
    newCTD:$[null nearest; `; nearest > 0; newCTDUp; newCTDDown];

    `currentCTD`nearest`direction`newCTD`switchUp`switchDown`newCTDUp`newCTDDown`maxShiftSearched!(
        currentCTD;nearest;direction;newCTD;switchUp;switchDown;newCTDUp;newCTDDown;maxShift)}
...
usage:{[]
    ...
    -1 ".ctd.impliedRepoRate[settleDate;deliveryDate;bondPrice;aiSettle;aiDelivery;futuresPrice;cf;couponAccrual]";
...
help:{[]
    ...
    -1 "  impliedRepoRate[settle;delivery;price;aiSettle;aiDelivery;fut;cf;cpnAccr] - Implied repo";
    -1 "";
    -1 "CTD ANALYSIS:";
