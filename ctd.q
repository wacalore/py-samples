...
// Find nearest switch point using binary search
...
nearestSwitchPoint:{[curve;settleDate;deliveryDate;futuresPrice;repoRate;bonds;maxShift]
    ...
    // Calculate each bond's YTM from market price
    bondsWithYTM:{[s;b]
        ytm:yieldToMaturity[s;b`maturity;b`coupon;`SA;100f;b`cleanPrice];
        b,enlist[`ytm]!enlist ytm
    }[settleDate;] each bonds;

    // Helper: find CTD at a given shift
    ctdAtShift:{[curve;settle;delivery;fut;repo;bondsWithYTM;shift]
        repricedBonds:{[s;shift;b]
            newYTM:b[`ytm] + shift % 10000;
            newYTM:0.0001 | newYTM;
            dirtyPrice:priceBondFromYield[s;b`maturity;b`coupon;`SA;100f;newYTM];
            ai:accruedInterest[s;b`maturity;b`coupon;`SA;100f];
            b,enlist[`cleanPrice]!enlist dirtyPrice - ai
        }[settle;shift;] each bondsWithYTM;
        results:ctdAnalysis[curve;settle;delivery;fut;repo;repricedBonds];
        first exec sym from results where isCTD
    }[curve;settleDate;deliveryDate;futuresPrice;repoRate;bondsWithYTM];

    // Binary search for switch point in one direction
    // Returns null if no switch within range
    binarySearch:{[ctdAtShift;currentCTD;lo;hi]
        // Check if switch exists in range
        ctdLo:ctdAtShift lo;
        ctdHi:ctdAtShift hi;
        if[ctdLo = ctdHi; :0Ni];  // No switch in this range

        // Binary search to find exact switch point
        while[1 < hi - lo;
            mid:`int$(lo + hi) % 2;
            ctdMid:ctdAtShift mid;
            $[ctdMid = currentCTD;
                lo:mid;
                hi:mid]
        ];
        // Return the point where switch occurs
        hi
    };

    // Search upward (yields rise)
    switchUp:binarySearch[ctdAtShift;currentCTD;0;maxShift];

    // Search downward (yields fall)
    switchDown:binarySearch[ctdAtShift;currentCTD;neg maxShift;0];

    // Get new CTD at switch points
    newCTDUp:$[null switchUp; `; ctdAtShift switchUp];
    newCTDDown:$[null switchDown; `; ctdAtShift switchDown];
